"""v177: test maxnreg=168 and maxnreg=192 in the autotune space.

Dead end #53 (v139) tested maxnreg=128, which was never picked by
autotune. But 128 is an aggressive cap on SM89 (255 max regs/thread).
Intermediate caps like 168 (= 2/3 of max) or 192 (= 3/4) weren't
explored — they might be a sweet spot where the compiler spills
slightly but gains occupancy.

If autotune picks 168 or 192 for any shape, that's a signal the
compiler currently over-uses registers for that shape. If autotune
always prefers None, confirms dead end #53 more broadly.

Zero-risk probe: adds 20 more configs to autotune space
(10 existing + 10 with maxnreg=168 + 10 with maxnreg=192 → 30 total).
Autotune selects the winner per shape; None is in the space so it
can still win.

Kernel code is byte-identical to v173. Only the autotune decorator
differs.
"""
import math
import torch
...
for s in [1, 2, 3, 4, 5]:
    for w in [4, 8]:
        for mnr in [None, 168, 192]:
            kwargs = {}
            if mnr is not None:
                kwargs["maxnreg"] = mnr
            _configs.append(triton.Config({}, num_stages=s, num_warps=w, **kwargs))
