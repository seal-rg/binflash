"""v15 precompute_trans_k — v4 + hoist `tl.trans(k)` into a named variable per iter.

In v4 dQ inner, k is loaded as (D, BLOCK_N), qk uses it directly, dq update
uses tl.trans(k). Making trans(k) explicit (`k_t = tl.trans(k)`) may help the
compiler avoid recomputing the transpose or dual-mapping registers.

Also symmetric change in dK/dV inner: precompute `v_t = tl.trans(v)`.
...
def _bwd_dkdv_inner(
    ...
    for idx in tl.range(0, n_full, num_stages=3):
        ...
        # v15: precompute trans(p) and trans(v).
        p_do = p.to(do.dtype)
        p_t = tl.trans(p_do)
        v_t = tl.trans(v)
...
def _bwd_dq_inner(
    ...
    for idx in tl.range(0, n_full, num_stages=3):
        ...
        # v15: precompute trans(k) and trans(v) explicitly.
        k_t = tl.trans(k)  # (BLOCK_N, D)
        v_t = tl.trans(v)  # (D, BLOCK_N)
        ...
        dp = tl.dot(do, v_t).to(tl.float32)
        ds = p * (dp - d_i[:, None])
        dq = tl.dot(ds.to(k.dtype), k_t, dq)
