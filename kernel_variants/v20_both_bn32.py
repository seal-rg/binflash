"""v20 both_bn32 — v19 + dK/dV also uses BN=32 (and BM=64).

v19 used BM=128 BN=32 for dQ. v20 also changes dK/dV to BN=32 (K-outer kernel
with 32-wide K blocks). This halves the dk/dv accumulator size and p/ds
working tensors in dK/dV. Might free register pressure and improve throughput.
Needs block mask at (N/64, N/32) granularity for dK/dV (vs v19's (N/128, N/32)
for dQ which was different already).

v19 context:
v5/v17 both failed at BM=128 BN=64 by 4 bytes SMEM overflow on A100. Reducing
BN to 32 halves the p/ds working tensors (16KB each instead of 32KB) — net
SMEM savings ~32KB, more than the 4-byte shortfall. Should fit.

Adds a new (N/128, N/32) block mask granularity for the dQ kernel. dK/dV
unchanged at BM=BN=64.
...
_BWD_BLOCK_M = 64       # dK/dV kernel tile (Q-inner)
_BWD_BLOCK_N = 32       # v20: dK/dV also uses BN=32
_BWD_DQ_BLOCK_M = 128   # dQ kernel: 128×32
_BWD_DQ_BLOCK_N = 32
...
def _binflash_bwd(do, q, k, v, o, lse, mask, sm_scale, preproc=None):
    ...
    # v19: dQ uses (N/128, N/32) col_indices. New block mask built in preproc.
