"""Binary Block Masked Flash Attention — v127: partial hoist (mask + bm row).

v119 hoisted FOUR loop-invariants out of the inner while loop:
  1. K_row_base (HEAD_DIM-wide tensor)
  2. V_col_base (HEAD_DIM-wide tensor)
  3. Mask_row_base (BLOCK_M-wide tensor)
  4. bm_row_base (scalar)

v119 retest showed MIXED result — wins on longformer/causal D=128 +2-3%
but loss on sparse_10pct D=128 -1%. Hypothesis: hoisting the K/V row
bases creates more register live ranges across the loop, which hurts on
long-scan (sparse_10pct has 128 blocks/row) workloads where the compiler
would have hoisted internally anyway. On short-scan (longformer ~8
blocks/row), the manual hoist is a net win.

This version hoists ONLY the two cheap, clearly-invariant pointers:
  - Mask_row_base (small, used only in pass 2)
  - bm_row_base (scalar, used in both passes)
Leaves K/V pointer computation inline. Goal: keep the longformer win
without the sparse_10pct regression.
"""
...
def _binflash_fwd_inner(
    ...
    # Partial hoist: only mask row base + bm scalar, keep K/V pointer
    # computation inline so the compiler can schedule freely across the
    # long-scan loop without extra register live ranges.
...
def binflash_attention(
    ...
    """v114 + partial hoist (mask_row_base + bm_row_base only)."""
