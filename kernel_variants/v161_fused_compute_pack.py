"""v161: fused _compute_and_pack_mask kernel. After v160, profiling shows:
  _compute_block_mask (v159):    0.395ms  ← eliminated here
  _reduce_block_mask (v160):     ~0.05ms
  _pack_mask (v158):             1.032ms
  want_packed=True total:        1.39ms

_compute_block_mask and _pack_mask BOTH read the full (N,N) mask (256MB
at N=16384). They're called sequentially when want_packed=True, so the
256MB is read TWICE. Ada's 48MB L2 can't cache it between the two calls.

This version fuses them into one kernel that reads each tile once and
emits both outputs:
  - BN_CHUNKS bm_vals per program (block_mask)
  - (BLOCK_M, BN_CHUNKS*BLOCK_N/32) packed int32 words per program (mask_packed)

BN_CHUNKS=2 to keep the pack intermediate within register budget:
  tile int8:           (128, 128) = 16KB
  pack int32 inter:    (128, 4, 32) = 64KB
  acceptable on Ada (~100KB shmem, 255 regs/thread).

When want_packed=False the existing _compute_block_mask (BN_CHUNKS=8) is
still used — it's already optimized.

Kernel body UNCHANGED from v149/v158/v159/v160.
"""
...
def _compute_block_mask(
    ...
    """Compute-only variant for want_packed=False (v159)."""
    row = tl.program_id(0)
...
def _compute_and_pack_mask(
    ...
    """Fused compute + pack. Grid (nqb, nkb // BN_CHUNKS). Each program
    loads one (BLOCK_M, BN_CHUNKS*BLOCK_N) int8 tile and emits:
      - BN_CHUNKS bm_vals written to BlockMask row
      - (BLOCK_M, BN_CHUNKS*BLOCK_N/32) int32 packed words to Packed
    """
    row = tl.program_id(0)
    col_chunk = tl.program_id(1)
    ...
    tile_ptrs = Mask + offs_m[:, None] * stride_mask_m + offs_n[None, :] * stride_mask_n
    tile = tl.load(tile_ptrs)  # (BLOCK_M, BN_CHUNKS*BLOCK_N) int8

    # ── block_mask outputs ──
    tile_3d_bm = tl.reshape(tile, (BLOCK_M, BN_CHUNKS, BLOCK_N))
    tile_i32_bm = tile_3d_bm.to(tl.int32)
    chunk_sums_per_col = tl.sum(tile_i32_bm, axis=0)  # (BN_CHUNKS, BLOCK_N)
    chunk_sums = tl.sum(chunk_sums_per_col, axis=1)  # (BN_CHUNKS,)
    has_any = chunk_sums > 0
    has_all = chunk_sums == (BLOCK_M * BLOCK_N)
    bm_vals = has_any.to(tl.int8) + has_all.to(tl.int8)
    col_start = col_chunk * BN_CHUNKS + tl.arange(0, BN_CHUNKS)
    ...
    # ── mask_packed outputs ──
    # Reshape tile (BLOCK_M, BN_CHUNKS*BLOCK_N) → (BLOCK_M, WORDS, 32)
    # where WORDS = BN_CHUNKS*BLOCK_N / 32.
    WORDS: tl.constexpr = (BN_CHUNKS * BLOCK_N) // 32
    tile_3d_pk = tl.reshape(tile, (BLOCK_M, WORDS, 32))
    bits_i32 = tile_3d_pk.to(tl.int32)
    bit_weights = (1 << tl.arange(0, 32))  # (32,)
    weighted = bits_i32 * bit_weights[None, None, :]
    packed = tl.sum(weighted, axis=2)  # (BLOCK_M, WORDS) int32

    # Write packed to Packed[row_start:row_start+BLOCK_M, word_start:word_start+WORDS]
...
def _pack_mask_kernel(
    ...
    """Standalone pack kernel (v158) — still used when block_mask was
    already cached from a prior call and only the packed form is needed."""
...
def _fused_preproc(mask, block_m, block_n, want_packed=False):
    ...
    if want_packed:
        # v161: fused kernel emits both block_mask and mask_packed from one
        # tile read. Smaller BN_CHUNKS=2 to keep the int32 pack intermediate
        # within register budget at BLOCK_M=block_m.
        mask_packed = torch.empty((N, N // 32), dtype=torch.int32, device=mask.device)
        bn_chunks = 2
        while nkb % bn_chunks != 0 and bn_chunks > 1:
            bn_chunks //= 2
        if (bn_chunks * block_n) % 32 != 0:
            # fall back to separate kernels if the chunk width is not a
            # multiple of 32 (can't pack)
            _compute_block_mask[(nqb, nkb)](
                mask, block_mask,
                mask.stride(0), mask.stride(1), nkb,
                BLOCK_M=block_m, BLOCK_N=block_n, BN_CHUNKS=1,
            )
            mask_packed = _pack_mask(mask)
        else:
            _compute_and_pack_mask[(nqb, nkb // bn_chunks)](
                mask, block_mask, mask_packed,
                mask.stride(0), mask.stride(1),
...
# ─── Main attention kernel (UNCHANGED from v149/v158/v159/v160) ───
