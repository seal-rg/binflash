...
def _gathered_fwd_inner(
    ...
    # V208: disallow acc multi-buffer
    for idx in tl.range(0, n_full, disallow_acc_multi_buffer=True):
    ...
    for idx in tl.range(n_full, n_total, disallow_acc_multi_buffer=True):
        col_idx = tl.load(col_indices_ptr + idx)
        start_n = col_idx * BLOCK_N
        k_ptrs = K_base + offs_d[:, None] * stride_kk + (start_n + offs_bn)[None, :] * stride_kn
        k = tl.load(k_ptrs)
