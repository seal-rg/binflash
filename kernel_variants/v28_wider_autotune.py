"""v28 wider_autotune — v22 + expanded autotune space.

Current v22 (now prod) uses 18 autotune configs (stages [1..3] × warps [4,8] ×
maxnreg {None, 168, 192}). v10 tried 75 configs and was SLOWER (Triton picked
bad configs under noisy measurement). v28 tries MIDDLE ground: add maxnreg=128
and 144 (lower register cap) + warps=16 + stages=4 to the space.

Hypothesis: at BM=128 BN=32 the register pressure pattern differs from BM=64
BN=64 — persistent dq (64KB fp32) is in registers, so a lower maxnreg cap
might actually help by spilling some dq to SMEM and freeing registers for
better matmul issue.

If this is strictly worse than v22, it's a confirmation that Triton's autotune
doesn't benefit from a larger search for bwd.
...
# v28: expanded autotune space — add maxnreg=128 and maxnreg=144 (lower
# register caps that may help BM=128 BN=32 dQ where persistent dq is in
# registers). Also add num_warps=16 for BM=128 where more warps can help.
# Previous space: 3*2*3 = 18 configs. New: 4*3*5 = 60 configs (Triton auto-skips OOM).
for _s in [1, 2, 3, 4]:
    for _w in [4, 8, 16]:
        for _mnr in [None, 128, 144, 168, 192]:
            _kw = {}
            if _mnr is not None:
                _kw["maxnreg"] = _mnr
