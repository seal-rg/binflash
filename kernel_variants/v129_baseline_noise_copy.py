"""Binary Block Masked Flash Attention — v129: byte-identical copy of v114.

Diagnostic: measure the benchmark noise floor by comparing v114 against an
identical copy. Any "difference" here is pure autotune/thermal/scheduling
variance, not a real kernel effect.

This establishes the noise floor below which no recent kernel experiment
(v115-v128) can be trusted as a real win/loss. All micro-optimization
attempts over the past sessions have landed in the ±2-5% range; this
experiment reveals how much of that is actually mechanism vs. noise.
"""

# Literal copy of v114 structure with renamed kernel symbols so autotune
# caches are separate (otherwise Triton would share the cache entry and
# we'd measure the same compilation twice).
...
def _fused_preproc(mask, block_m, block_n):
    ...
    _compute_block_mask_v129[(nqb, nkb)](
        mask, block_mask,
...
def _binflash_fwd_v129(
    ...
    acc, l_i, m_i = _binflash_fwd_inner_v129(
        acc, l_i, m_i, q,
        K_base, V_base, Mask,
        stride_kk, stride_kn, stride_vk, stride_vn,
        stride_mask_m, stride_mask_n,
...
def binflash_attention(
    ...
    """v114 byte-identical copy (for noise-floor measurement)."""
