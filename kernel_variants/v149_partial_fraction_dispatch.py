"""Binary Block Masked Flash Attention — v149: partial_fraction dispatch for packed mask.

Probe of all benchmark patterns revealed two more "dispatch blind spots"
after v147/v148 fixes:

  sliding_window_128:   gap=0.000, density=0.023, partial_fraction=0.665
  causal_window_256:    gap=0.000, density=0.023, partial_fraction=0.667

These patterns have a contiguous diagonal band (gap_ratio=0, low density)
but most of their non-empty blocks ARE partial (pf=0.665/0.667). They
miss v148's `(density > 0.8 OR gap_ratio > 0.3)` trigger.

The root cause: `density` and `gap_ratio` were the wrong metrics to
dispatch on. The right metric is **partial_fraction** — the fraction
of non-empty blocks that are partial (bm_val==1). When this is high,
most scanned blocks need a mask load, and packing gives real bandwidth
savings.

v149 adds a third condition: `partial_fraction > 0.5`. Verified clean
separation on all probed patterns:
  - New packed: sliding_window_128 (pf=0.665), causal_window_256 (pf=0.667)
  - Still packed: sparse_10pct (pf=1.0), sparse_30pct (pf=1.0),
                  longformer (pf=0.776), cdw_sinks (pf=1.0)
  - Still regular: causal (pf=0.016), prefix_lm (pf=0.011),
                   log_tree (pf=0.023), block_diagonal (pf=0.000)

Preprocessing computes partial_fraction cheaply (one extra .sum() call).
"""
...
def _fused_preproc(mask, block_m, block_n, want_packed=False):
    ...
    # NEW (v149): partial_fraction = fraction of non-empty blocks that are partial
...
_PARTIAL_FRACTION_THRESHOLD = 0.5  # NEW in v149
...
def binflash_attention(
    ...
    """v148 + partial_fraction > 0.5 as third packing criterion."""
    ...
    if block_mask is None:
        ...
        if key == _bm_cache_key:
            ...
            bm128_bn64, fn, ln, gap_ratio, density, pf_128_64, _ = _fused_preproc(mask, 128, 64, want_packed=False)
            if D == 64 and gap_ratio > _GAP_RATIO_THRESHOLD and N % 64 == 0:
                ...
                # Default path BM=128 BN=64 — packed if ANY of:
                #   density > 0.8 (sparse_10pct/30pct)
                #   gap_ratio > 0.3 (D=128 longformer, cdw_sinks — v148)
                #   partial_fraction > 0.5 (sliding_window, causal_window — v149)
                block_mask, first_nz, last_nz = bm128_bn64, fn, ln
                use_packed = 1 if (
                    density > _DENSITY_THRESHOLD_PACKED
                    or gap_ratio > _GAP_RATIO_THRESHOLD
                    or pf_128_64 > _PARTIAL_FRACTION_THRESHOLD
                ) else 0
        ...
        raise NotImplementedError("External block_mask not supported in v149")
