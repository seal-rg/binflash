"""V195: V193 + single merged loop (revisits dead end #21).

Dead end #21 (v102) merged full+partial bodies in one scan loop with a
bm_val-dependent branch — was 5-22% slower because compiler couldn't
separate register allocation.

V195 attempts merged loop with gathered dispatch: the full/partial
boundary is explicit via n_full. The compiler may handle this cleaner
branch (loop counter vs runtime scalar) better than bm_val per iter.
"""
...
def _gathered_fwd_inner(
    ...
    # V195: Single merged loop over [0, n_total). is_full determined by position.
    # Revisits dead end #21 (merged pass was slower on scan-based v102).
    # Gathered dispatch makes the boundary explicit via n_full, which the
    # compiler may handle more cleanly than a bm_val-dependent branch.
