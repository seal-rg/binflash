...
def _bwd_dkdv_inner(
    ...
    for idx in tl.range(0, n_full):
        ...
        dv = tl.dot(tl.trans(p.to(do.dtype)), do, dv)  # v_acc28: keep bf16 dV

        dp = tl.dot(do, tl.trans(v)).to(tl.float32)
        ds = p * (dp - d_i[:, None])
        dk = tl.dot(tl.trans(ds), q.to(tl.float32), dk, input_precision='tf32x3')  # v_acc28: tf32x3 only on dK FULL
