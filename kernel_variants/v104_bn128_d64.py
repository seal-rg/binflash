"""Binary Block Masked Flash Attention — v104: BLOCK_N=128 at D=64.

Dead end #5 ruled out BN=128 at D=128 (shared memory). But at D=64,
shared memory usage is halved, making BN=128 feasible. Benefits:
- Half the loop iterations (N/128 vs N/64)
- Larger matmul tiles (better tensor core utilization?)
- Fewer block mask checks

Risk: bigger p tile (128×128 FP32 = 64KB) may cause register pressure.
Remove the `BLOCK_N <= HEAD_DIM` assertion to enable this config.
"""
...
def _binflash_fwd(
    ...
    # Note: removed `BLOCK_N <= HEAD_DIM` assertion to allow BN=128 at D=64
...
def binflash_attention(
    ...
    """Binary-block-masked flash attention (v104: BN=128 at D=64)."""
    global _bm_cache_key, _bm_cache_val
    B, H, N, D = q.shape
    # v104: use BN=128 at D=64 (was BN=64)
    if block_n is None:
        block_n = 128 if D == 64 else min(D, 64)
