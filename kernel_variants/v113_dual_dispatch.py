"""Binary Block Masked Flash Attention — v113: dual adaptive dispatch.

Extends v112 (BM=64 for gap-heavy patterns) with a second dispatch path:
BN=128 for fully-dense patterns (density > 0.9).

Dead end #22 (v104 BN=128) showed BN=128 at D=64 wins +4-12% on random
dense patterns (sparse_10pct, sparse_30pct) but loses on patterns with
actual block sparsity. Dispatching on density captures only the wins.

Three paths (all data-driven, not pattern-specific):
1. gap_ratio > 0.3 → BM=64 kernel (v112: longformer-like)
2. density > 0.9 AND D=64 → BN=128 kernel (v113 new: dense random)
3. default → BM=128, BN=64 kernel (v100 baseline)
"""
...
def _fused_preproc(mask, block_m, block_n):
    """Compute block_mask + bounds + gap_ratio + density."""
    N = mask.shape[0]
    nqb = N // block_m
    nkb = N // block_n
...
def binflash_attention(
    ...
    """Binary-block-masked flash attention with dual adaptive dispatch."""
    ...
    if block_mask is None:
        ...
        # External block_mask — use defaults
        nz = block_mask > 0
        nkb = nz.shape[1]
        col_indices = torch.arange(nkb, device=mask.device)
        has_any_in_row = nz.any(dim=1)
    ...
    if N % chosen_bm != 0 or N % chosen_bn != 0:
        raise ValueError(f"N ({N}) must be divisible by bm={chosen_bm} and bn={chosen_bn}")
