"""Binary Block Masked Flash Attention — v139: maxnreg in autotune configs.

Triton's `triton.Config` supports `maxnreg=N` to cap the per-thread
register count. None of the previous dead ends tested this knob.

At D=128 the kernel has high register pressure:
  - acc: 128*128*4 = 64KB F32
  - Q:   128*128*2 = 32KB BF16
  - unpack intermediate: 8192 int32 = 32KB (packed mask path)
Total: ~128KB across 256 threads = 500 B/thread = 125 fp32 regs/thread.

The SM can allocate up to ~255 regs per thread but lower counts can
enable higher occupancy (more program tiles resident). A lower maxnreg
forces the compiler to spill less-critical data to shared mem,
potentially unlocking better latency hiding via more concurrent
programs per SM.

Strategy: extend autotune with maxnreg ∈ {None, 128, 96}. None is the
default (no cap). Lower caps may help at D=128.

Config list grows from 10 to 30 configs — more compile time, but
autotune can pick the optimum. If None is always best, we learn that
maxnreg is neutral (expected).
"""
...
# Extended autotune with maxnreg variations
_CONFIGS = []
for s in [1, 2, 3, 4, 5]:
    for w in [4, 8]:
        for nreg in [None, 128]:  # None = no cap (default); 128 = tighter
            _CONFIGS.append(triton.Config({}, num_stages=s, num_warps=w, maxnreg=nreg))
...
def binflash_attention(
    ...
    """v132 + maxnreg autotune (configs include maxnreg=None and maxnreg=128)."""
    ...
    if block_mask is None:
        ...
        raise NotImplementedError("External block_mask not supported in v139")
