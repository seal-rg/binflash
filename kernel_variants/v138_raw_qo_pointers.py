"""Binary Block Masked Flash Attention — v138: raw pointers for Q/O.

Current v132 uses `tl.make_block_ptr` for Q (load) and O (store), but
raw pointer arithmetic for K/V/Mask/MaskPacked. Dead end #14 tested raw
mask pointers (neutral at D=128), but Q and O loads/stores specifically
haven't been measured against block_ptr.

Q is loaded once per program at the kernel start; O is stored once at
the end. If `make_block_ptr` has any per-program setup overhead beyond
raw pointer arithmetic, that overhead is amortized across many fewer
events than the per-block K/V loads — but there's exactly ONE such
event per program, so any overhead shows up cleanly.

Expected: neutral or tiny variance. But a clean experiment of a not-yet-
tested knob given how exhausted the search space is.
"""
...
def _binflash_fwd(
    ...
    offs_bm_full = start_m * BLOCK_M + tl.arange(0, BLOCK_M)
    offs_d = tl.arange(0, HEAD_DIM)
    # Raw pointer Q load
    q_ptrs = Q + qvk_offset + offs_bm_full[:, None] * stride_qm + offs_d[None, :] * stride_qk
    q = tl.load(q_ptrs)
    ...
    # Raw pointer O store
    o_ptrs = Out + qvk_offset + offs_bm_full[:, None] * stride_om + offs_d[None, :] * stride_on
...
def binflash_attention(
    ...
    """v132 + raw pointer arithmetic for Q load / O store."""
    ...
    if block_mask is None:
        ...
        raise NotImplementedError("External block_mask not supported in v138")
