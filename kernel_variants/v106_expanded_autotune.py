"""Binary Block Masked Flash Attention — v106: expanded autotune space.

Current: stages=[1..5] × warps=[4,8] = 10 configs (trimmed in v086 era).
Test: stages=[1..6] × warps=[2,4,8,16] = 24 configs.

Rationale: the current autotune space was determined before while-loops
and raw pointers (v100) + fused preprocessing (v103). The optimal
pipelining and warp counts may have changed with the new architecture.
"""
...
def binflash_attention(
    ...
    """Binary-block-masked flash attention with expanded autotune space."""
