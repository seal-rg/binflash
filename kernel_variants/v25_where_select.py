"""v25 where_select — v4 + select-style mask application in partial-block paths.

Current v4: `qk += tl.where(mask_, 0.0, -1.0e6)` materializes a (BM, BN) fp32
bias tile and adds it to qk.
v25: `qk = tl.where(mask_, qk, -1.0e6)` — one select op, no add. Semantics:
masked cells get exactly -1e6 (current: qk + -1e6, value close to -1e6 since
|qk| << 1e6).
Applied to bwd dQ and dK/dV partial-block loops. Fwd kernel unchanged (keeps
the v188 epilogue-adjacent form)."""
