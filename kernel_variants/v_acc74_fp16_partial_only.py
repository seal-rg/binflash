...
def _bwd_dkdv_inner(
    ...
    # Pass 2: partial Q-blocks — v_acc74: fp16 only on PARTIAL pass
    for idx in tl.range(n_full, n_total):
        row_idx = tl.load(ri_base + idx)
        m_cur = row_idx * BLOCK_M + offs_m
        q_ptrs = Q_base + m_cur[:, None] * stride_qm + offs_d[None, :] * stride_qk
        ...
        dv = tl.dot(tl.trans(p.to(tl.float16)), do.to(tl.float16), dv)  # v_acc74: fp16 PARTIAL only

        dp = tl.dot(do, tl.trans(v)).to(tl.float32)
        ds = p * (dp - d_i[:, None])
        dk = tl.dot(tl.trans(ds.to(tl.float16)), q.to(tl.float16), dk)  # v_acc74: fp16 PARTIAL only
