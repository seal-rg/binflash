"""Binary Block Masked Flash Attention — v135: expand autotune to stages 6,7.

v132's packed-mask path saves 7KB of shared memory per stage (8KB mask
tile → 1KB packed). This MIGHT unlock higher num_stages values than the
current autotune explores (1..5). Dead end #24 tried 1..6 with warps
2/4/8/16 and was neutral — but that was pre-v132, before the shmem
savings from packing made higher stages feasible.

v135 narrowly expands autotune to stages 1..7 × warps 4,8 (14 configs).
If any shape — especially the packed-path patterns (sparse_10pct/30pct
which use USE_PACKED=1) — picks stages=6 or 7 as the best config,
that's a new win. Otherwise it's just extra compile cost and we confirm
dead end #24 still holds.

Rationale: at packed D=128 BM=128 BN=64, pipeline working set per stage
is ~33KB (K 16KB + V 16KB + mask_packed 1KB). Theoretical max stages
~3x before 100KB limit. Adding stages 6,7 gives autotune room to
explore deeper pipelines if Triton's shmem math permits.
"""
...
# Extended autotune: stages 1..7 × warps 4,8 (14 configs)
@triton.autotune(
    configs=[triton.Config({}, num_stages=s, num_warps=w)
             for s in [1, 2, 3, 4, 5, 6, 7] for w in [4, 8]],
...
def binflash_attention(
    ...
    """v132 + expanded autotune (stages 1..7) for packed-path shmem headroom."""
    ...
    if block_mask is None:
        ...
        raise NotImplementedError("External block_mask not supported in v135")
