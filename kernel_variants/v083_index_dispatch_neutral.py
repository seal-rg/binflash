"""v083: Index dispatch kernel — NEUTRAL at kernel level.

Kernel-only comparison (precomputed block_mask passed to both):

D=64 N=8K:
  causal:  scan=2.665ms idx=2.574ms -> idx 1.04x faster
  sliding: scan=0.290ms idx=0.270ms -> idx 1.07x faster
  blkdiag: scan=0.191ms idx=0.210ms -> idx 0.91x SLOWER
  longfmr: scan=0.530ms idx=0.558ms -> idx 0.95x SLOWER
  csl_doc: scan=0.731ms idx=0.700ms -> idx 1.04x faster
  cdw_snk: scan=0.243ms idx=0.241ms -> idx 1.01x ~same

D=128 N=8K:
  causal:  scan=10.314ms idx=9.585ms -> idx 1.08x faster
  sliding: scan=1.228ms idx=1.182ms -> idx 1.04x faster
  blkdiag: scan=0.710ms idx=0.708ms -> idx ~same
  longfmr: scan=1.695ms idx=1.664ms -> idx 1.02x faster
  csl_doc: scan=3.200ms idx=2.873ms -> idx 1.11x faster
  cdw_snk: scan=1.073ms idx=0.980ms -> idx 1.10x faster

Decision: Index dispatch is roughly neutral at D=64, slightly better at D=128.
The big v082 win was from the CACHE, not the kernel change.
Keep cache, use scan-and-skip (simpler, no argsort needed).

This file contains the baseline kernel (same as v082_before_index_dispatch),
not the experimental index dispatch variant.
"""
