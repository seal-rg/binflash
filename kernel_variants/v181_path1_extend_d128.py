"""v181: extend path 1 (BM=64 BN=64 gap-heavy) to D=128.

Dead end #13 (v114) said BM=64 at D=128 regresses longformer by -16%
at N=8192 due to register pressure:
  Q (16KB) + acc (32KB FP32) + K/V/mask intermediates = too much
  registers without cap → spills → slower than BM=128 baseline.

Post-v177 (maxnreg) + v179 (stages [1..7]), register pressure is
bounded by the autotune cap. The compiler spills deterministically
rather than uncontrollably. This may unlock BM=64 at D=128 for gap-
heavy patterns (longformer D=128, cdw_sinks D=128) which currently
go through the default BM=128 path and are compute-heavy.

Current state at D=128 longformer N=16384 7B-long (v179 prod):
  ~1.62 ms (with BM=128 BN=64)

BM=64 path 1 at D=128 would have coarse unroll CF=4 or CF=8 same as
D=64. Register budget at maxnreg=192 × 4 warps × 32 × 4 = 98KB; at
warps=8 × 32 × 192 × 4 = 196KB. Both fit the ~96KB live state at
D=128 BM=64 BN=64.

Only dispatch change: `if D == 64 and gap_ratio > 0.3` → `if gap_ratio > 0.3`.
Kernel body, autotune space unchanged (uses production).
"""
import math
import torch
import triton
...
def _dispatch_and_preprocess(mask: torch.Tensor, D: int):
    ...
    # v181: path 1 extended to D=128 too (was D==64 only).
    if gap_ratio > _GAP_RATIO_THRESHOLD and N % 64 == 0:
