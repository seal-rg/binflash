"""Binary Block Masked Flash Attention — v103: fused preprocessing kernel.

Current preprocessing uses 4+ separate PyTorch ops (.any, .all, argmax, max),
each launching a CUDA kernel and reading the N×N mask. This version fuses
everything into a single Triton kernel: one pass over the mask computes
block_mask, first_nz, and last_nz.

For N=8192 (64MB mask): replaces 4 kernel launches + 128MB+ bandwidth
with 1 launch + 64MB bandwidth.
"""
...
def _compute_block_mask(
    ...
    """One program per block. Full 2D parallelism."""
    row = tl.program_id(0)
    col = tl.program_id(1)
    ...
    # Single sum → both has_any and has_all in one reduction
...
def fused_block_mask_preproc(mask, block_m, block_n):
    """Compute block_mask + first_nz + last_nz.

    block_mask via fused Triton kernel (parallel 2D grid),
    first_nz/last_nz via cheap PyTorch ops on the tiny block_mask tensor.
    """
    N = mask.shape[0]
    nqb = N // block_m
    nkb = N // block_n
    ...
    # Pass bool mask directly — Triton can load bool via tl.load
    ...
    # first_nz/last_nz on the tiny (nqb × nkb) block_mask (negligible cost)
    nz = block_mask > 0
...
# ─── Main attention kernel (same as v100) ───
...
def binflash_attention(
    ...
    """Binary-block-masked flash attention with fused preprocessing."""
    ...
    if block_mask is None:
        ...
        if key == _bm_cache_key:
            ...
            block_mask, first_nz, last_nz = fused_block_mask_preproc(mask, block_m, block_n)
            _bm_cache_key = key
