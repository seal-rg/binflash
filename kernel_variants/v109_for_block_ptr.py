"""Binary Block Masked Flash Attention — v109: for-loop + block_ptr + runtime bounds.

v100 uses while loops for early exit but had to use raw pointers (block_ptr
can't be used in while loops). However, Triton's for loops DO support
tensor pointers AND runtime bounds via `range(runtime_start, runtime_end)`.

Key insight: `for start_n in range(first_nz * BLOCK_N, (last_nz + 1) * BLOCK_N, BLOCK_N)`
gives us both:
- Early exit (only iterate [first_nz, last_nz])
- block_ptr with efficient tl.advance

Hypothesis: block_ptr's optimized address handling beats raw pointer
recomputation when we don't need while-loop semantics.
"""
...
def _binflash_fwd_inner(
    ...
    # Advance K, V to first_nz_col
    K_block_ptr = tl.advance(K_block_ptr, (0, first_nz_col * BLOCK_N))
    V_block_ptr = tl.advance(V_block_ptr, (first_nz_col * BLOCK_N, 0))

    # ── Pass 1: full blocks (bm_val==2) ──
    # for loop with runtime bounds — tests if this compiles to scf::ForOp
    first_n = first_nz_col * BLOCK_N
    end_n = (last_nz_col + 1) * BLOCK_N
    for start_n in range(first_n, end_n, BLOCK_N):
    ...
    # Reset K, V for pass 2 — advance back
    n_scanned = (last_nz_col + 1 - first_nz_col) * BLOCK_N
    K_block_ptr = tl.advance(K_block_ptr, (0, -n_scanned))
    V_block_ptr = tl.advance(V_block_ptr, (-n_scanned, 0))
    # Advance Mask to first_nz_col
    Mask_block_ptr = tl.advance(Mask_block_ptr, (0, first_nz_col * BLOCK_N))
...
def binflash_attention(
    ...
    """Binary-block-masked flash attention with for-loop + block_ptr + runtime bounds."""
