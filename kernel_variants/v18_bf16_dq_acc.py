"""v18 bf16_dq_acc — v4 + bf16 accumulator for dq, dk, dv (instead of fp32).

Halves SMEM/register usage for these accumulators. For A100, the dq accumulator
is 32KB fp32 at BLOCK_M=64 D=128 — bf16 reduces to 16KB.

Precision risk: ~256 accumulations (K-blocks visited worst case) with bf16's
~7 mantissa bits → relative error ~sqrt(256) * 2^-7 ≈ 1-2%. Still within
the 5e-2 absolute tolerance for most inputs, but edge cases may fail.
...
def _bwd_dkdv_kernel(
    ...
    dk = tl.zeros([BLOCK_N, HEAD_DIM], dtype=tl.bfloat16)  # v18: bf16 accumulator
    dv = tl.zeros([BLOCK_N, HEAD_DIM], dtype=tl.bfloat16)  # v18: bf16 accumulator
...
def _bwd_dq_kernel(
    ...
    dq = tl.zeros([BLOCK_M, HEAD_DIM], dtype=tl.bfloat16)  # v18: bf16 accumulator
