"""Binary Block Masked Flash Attention — v130: H as constexpr.

The kernel start has:
    qvk_offset = (off_hz // H) * stride_qz + (off_hz % H) * stride_qh

where H is runtime. GPU integer divmod is expensive (~10-20 cycles). If
H is power-of-2 and known at compile time, Triton can fold to shift/and
(~1 cycle each). Typical H values (4, 8, 16, 32) are all powers of 2.

Change: add H to the autotune key and make it `tl.constexpr`. Each
distinct H value gets its own compilation with constant-folded divmod.

Savings per program: ~15 cycles * 2 programs. Across 2048 programs:
~60μs total — small but measurable. Correctness unchanged.
"""
...
@triton.autotune(
    ...
    key=["N_CTX", "HEAD_DIM", "Z_TIMES_H", "BLOCK_M", "BLOCK_N", "H"],
)
@triton.jit
def _binflash_fwd(
    ...
    # H is constexpr => if H is power of 2, divmod folds to shift/and
...
def binflash_attention(
    ...
    """v114 + H as constexpr (constant-folds div/mod to shift/and)."""
