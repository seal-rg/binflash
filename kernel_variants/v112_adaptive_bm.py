"""Binary Block Masked Flash Attention — v112: adaptive BM dispatch (single kernel).

v110/v111 showed BM=64 wins 20%+ on longformer but loses 20% on dense.
The difference is the mask's GAP STRUCTURE:
- Longformer: has big gaps between global (col 0) and window blocks
- Causal/dense: non-empty blocks are contiguous, no gaps

This version measures gap ratio at preprocessing time and dispatches
to BM=64 for gap-heavy patterns, BM=128 for contiguous patterns.

Single kernel definition, BM passed as constexpr at call time. Triton
compiles specializations per BM value; the BM=128 path is equivalent
to v100 and doesn't introduce autotune variance vs the production kernel.
"""
...
def _fused_preproc(mask, block_m, block_n):
    ...
    # Gap ratio for adaptive dispatch
    ...
    return block_mask, first_nz, last_nz, avg_gap_ratio
...
@triton.autotune(
    ...
    key=["N_CTX", "HEAD_DIM", "Z_TIMES_H", "BLOCK_M"],
)
@triton.jit
def _binflash_fwd(
...
def binflash_attention(
    ...
    """Binary-block-masked flash attention with adaptive BM dispatch."""
    ...
    # Cache key includes N to invalidate on shape changes
    key = (mask.data_ptr(), mask.shape[0], mask.shape[1], block_n)
    if key == _bm_cache_key:
        block_mask, first_nz, last_nz, chosen_bm = _bm_cache_val
    else:
        # Compute BM=128 preproc first, check gap ratio
        bm128, fn128, ln128, gap128 = _fused_preproc(mask, 128, block_n)
        # If gap-heavy and N divisible by 64, switch to BM=64 preproc
        if gap128 > 0.3 and N % 64 == 0:
            block_mask, first_nz, last_nz, _ = _fused_preproc(mask, 64, block_n)
            chosen_bm = 64
        else:
            block_mask, first_nz, last_nz = bm128, fn128, ln128
            chosen_bm = 128
        _bm_cache_key = key
        _bm_cache_val = (block_mask, first_nz, last_nz, chosen_bm)
    ...
    _binflash_fwd[grid](
        ...
        B, H, N, B * H, HEAD_DIM=D, BLOCK_M=chosen_bm, BLOCK_N=block_n,
    )
    return o
