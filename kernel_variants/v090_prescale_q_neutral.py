"""v090: Q pre-scaling — NEUTRAL performance.

Pre-scale Q by sm_scale*log2(e) once before the loop instead of multiplying
qk by qk_scale per block. Saves 8192 FP32 muls per block but at production
scale (D=128) this is <0.1% of per-block time. Kept as code simplification.

7B-train causal: 8.780ms (was 8.712ms) -- within noise.
Mean errors slightly higher (0.000025 vs 0.000020) from FP16 rounding.

Kernel is identical to v091_before_raw_mask_ptr and v092_before_twoloop:
single-loop, 3-valued block mask, Q pre-scaled, no qk_scale in inner.
"""
