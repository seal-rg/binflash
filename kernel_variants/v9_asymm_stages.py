"""v9 asymm_stages — v4 with asymmetric tl.range(num_stages=...) per loop.

Full-block inner loops (no mask load/branch) get num_stages=4; partial-block
...
def _bwd_dkdv_inner(
    ...
    # Pass 1: full Q-blocks — no mask. v9: num_stages=4 (more pipelining on light body).
    for idx in tl.range(0, n_full, num_stages=4):
    ...
    # Pass 2: partial Q-blocks — packed or bool mask. v9: num_stages=2 (heavier body).
...
def _bwd_dq_inner(
    ...
    # Pass 1: full K-blocks — no mask. v9: num_stages=4.
    for idx in tl.range(0, n_full, num_stages=4):
    ...
    # Pass 2: partial K-blocks — packed or bool mask. v9: num_stages=2.
    for idx in tl.range(n_full, n_total, num_stages=2):
        col_idx = tl.load(ci_base + idx)
        start_n = col_idx * BLOCK_N
        k_ptrs = K_base + offs_d[:, None] * stride_kk + (start_n + offs_bn)[None, :] * stride_kn
