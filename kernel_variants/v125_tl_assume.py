"""Binary Block Masked Flash Attention — v125: tl.assume hints.

Triton 3.6 supports `tl.assume()` to provide compile-time invariants that
help optimization. The while-loop scan has several properties the compiler
can't deduce from the code alone:

  - first_nz_col >= 0 (always)
  - last_nz_col < N_CTX / BLOCK_N (always — max nkb)
  - first_nz_col <= last_nz_col + 1 (loaded from preprocessing that respects this)
  - bm_val ∈ {0, 1, 2} (ternary)

Providing these might let the compiler tighten the loop bounds check, avoid
overflow guards, and predict bm_val branches better.

Not in dead-ends list. Low-risk experiment.
"""
...
def _binflash_fwd_inner(
    ...
    # Compile-time invariants for the loop bounds
    nkb: tl.constexpr = N_CTX // BLOCK_N
    tl.assume(first_nz_col >= 0)
    tl.assume(last_nz_col < nkb)
    tl.assume(first_nz_col <= last_nz_col + 1)
...
def binflash_attention(
    ...
    """v114 + tl.assume hints on first_nz/last_nz loop bounds."""
