...
def _bwd_fused_kernel(
    ...
    for idx in tl.range(0, n_full):
        ...
        qk = tl.dot(q.to(tl.float32), tl.trans(k_scaled).to(tl.float32), input_precision='ieee')  # v_acc81: IEEE fp32 QK
...
def _bwd_dkdv_inner(
    ...
    for idx in tl.range(0, n_full):
        ...
        qk = tl.dot(q.to(tl.float32), tl.trans(k_scaled).to(tl.float32), input_precision='ieee')  # v_acc81: IEEE fp32 QK   # already log2-basis because k is pre-scaled
        p = tl.math.exp2(qk - lse_i[:, None])
...
def _bwd_dq_inner(
    ...
    for idx in tl.range(0, n_full, num_stages=1):  # v_acc76: ns=1 to reduce reg pressure
        ...
        qk = tl.dot(q_scaled.to(tl.float32), k.to(tl.float32), input_precision='ieee')  # v_acc81: IEEE fp32 QK in dQ
        p = tl.math.exp2(qk - lse_i[:, None])
