"""v162: speculative packed preproc — always run the fused compute+pack
kernel in the first preproc call so the default-path with-packing case
(sliding_window_128, causal_window_256) no longer falls back to the slow
split `_fused_preproc(want_packed=False) + _pack_mask()` route.

Prior layout (v161):
  Default+pack: 0.44ms (want_packed=False) + 1.03ms (standalone _pack_mask) = ~1.36ms
  Default+no-pack: 0.44ms
  Path 1/2: 0.44ms (call 1) + 0.52ms (call 2 with fused pack) = ~0.96ms

New layout (v162) — the first call always requests want_packed=True:
  Default+pack: 0.52ms (fused, block_mask+pack produced together)
  Default+no-pack: 0.52ms (+0.08ms regression — packed data discarded)
  Path 1/2: 0.52ms + 0.44ms (call 2 want_packed=False, reuses call 1's
            mask_packed which is independent of block_m/block_n) = ~0.96ms

Benchmark impact (estimated at N=16384, cache-miss preproc):
  sliding_window_128:  1.36ms → 0.52ms  (-0.84ms)
  causal_window_256:   1.36ms → 0.52ms  (-0.84ms)
  causal / block_diag / prefix_lm / log_tree / causal_doc_7:
                        0.44ms → 0.52ms  (+0.08ms each, within run-to-run noise)
  longformer / cdw_sinks / sparse_10pct / sparse_30pct:
                        ~0.96ms unchanged (paths 1/2 still 2 calls total)

Net across 10 patterns × 4 seq lens: strong positive at N≥16384; kernel
body is UNCHANGED from v149/.../v161 so no kernel timing risk. The
0.08ms regression on no-pack patterns is tiny relative to kernel time
(causal kernel at N=16384 is 7ms — 0.08ms is 1% end-to-end).
"""
...
def _pick_bn_chunks(block_m, block_n, nkb, target_tile_bytes=65536, start=8):
    """Adaptive BN_CHUNKS: cap per-program tile size so register/shmem
    pressure stays bounded. At BM=128 BN=128, BN_CHUNKS=8 would blow the
    budget (128KB tile); this clamps it down. Must also divide nkb.
    """
...
def _fused_preproc(mask, block_m, block_n, want_packed=False):
    ...
    if want_packed and (block_n % 32 == 0):
        ...
        # BN_CHUNKS=2 ceiling for the fused pack kernel to keep the pack
        # intermediate (BM, WORDS, 32) int32 within registers.
        ...
        # BN_CHUNKS up to 8 for compute-only path, clamped by tile budget.
...
# ─── Main attention kernel (UNCHANGED from v149/.../v161) ───
...
def _dispatch_and_preprocess(mask: torch.Tensor, D: int):
    """v162: the first `_fused_preproc` call ALWAYS requests packed output
    so the default-with-packing case (sliding_window / causal_window)
    produces `mask_packed` through the fused kernel (v161) instead of
    the slow standalone `_pack_mask()` fallback.

    For paths 1/2 (gap-heavy / dense random), the first call's
    `mask_packed` is reused verbatim in the second call (it is
    independent of `block_m`/`block_n`) — the second call only needs
    `want_packed=False` to get a new block_mask at the alternate tile
    shape, saving 0.4ms vs recomputing the pack twice.
    """
    N = mask.shape[0]
    # Call 1: speculative packed preproc at (BM=128, BN=64)
    bm128_bn64, fn, ln, gap_ratio, density, partial_fraction, mask_packed = _fused_preproc(
        mask, 128, 64, want_packed=True,
    )
    if D == 64 and gap_ratio > _GAP_RATIO_THRESHOLD and N % 64 == 0:
        block_mask, first_nz, last_nz, _, _, _, _ = _fused_preproc(
            mask, 64, 64, want_packed=False,
        )
    ...
    # Default path: reuse the metrics-pass block_mask. Drop mask_packed
    # if dispatch doesn't actually want it.
    use_packed = 1 if (
    ...
    if not use_packed:
        mask_packed = None
    return bm128_bn64, fn, ln, mask_packed, 128, 64, use_packed
