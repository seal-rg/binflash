"""Binary Block Masked Flash Attention — v102: single-pass while loop.

The two-loop scan was a breakthrough with for-loops + block_ptr (v092)
because separating passes let the compiler pipeline K/V without mask
block_ptr. But v100 switched to raw pointers (no block_ptr at all).
Without block_ptr staging, the two-loop benefit may be smaller.

This version tests a single-pass while loop that processes both full
and partial blocks in one iteration. Benefits: half the iterations,
half the bm_val loads. Risk: nested branch + mask registers in same loop.
"""
...
def _binflash_fwd_inner(
    ...
    while col_idx <= last_nz_col:
        ...
        if bm_val >= 1:  # non-empty block (full or partial)
            ...
            v = tl.load(v_ptrs)
            qk = tl.dot(q, k)
            if bm_val == 1:  # partial — apply mask
...
def binflash_attention(
    ...
    """Binary-block-masked flash attention (single-pass variant)."""
