"""Binary Block Masked Flash Attention — v124: per-row pass skip.

The current kernel (v114) always runs both passes, even when a Q-row has
NO full blocks (pass 1 wastes iterations) or NO partial blocks (pass 2
wastes iterations).

Example: sparse_10pct at density 0.19 — a block being ALL-True has
probability ~0, so essentially no full blocks exist → pass 1 iterates
[first_nz..last_nz] doing nothing but loading bm_val scalars. At N=8192
with scan_width~128, that's 128 loop iterations per Q-block wasted.

Fix: preprocess two per-row bool flags `has_full_per_row` and
`has_partial_per_row`. Kernel loads both scalars per program and wraps
pass 1 / pass 2 in runtime `if` conditions that skip the loop entirely
when the corresponding flag is false.

Preprocessing cost: one extra .any(dim=1) reduction — <50μs at all
relevant shapes. Cached alongside block_mask.

Kernel cost: 2 extra scalar i8 loads per program (~20 cycles total).

Win: skips potentially hundreds of while-loop iterations when a row has
no full blocks (common in random sparse patterns).
"""
...
def _fused_preproc(mask, block_m, block_n):
    ...
    # NEW: per-row flags for pass skip
    has_full_per_row = (block_mask == 2).any(dim=1).to(torch.int8)
    has_partial_per_row = (block_mask == 1).any(dim=1).to(torch.int8)
    ...
    return block_mask, first_nz, last_nz, has_full_per_row, has_partial_per_row, avg_gap_ratio, density
...
def _binflash_fwd_inner(
    ...
    first_nz_col, last_nz_col, has_full, has_partial,
    ...
    # ── Pass 1: full blocks (bm_val==2) — only if row has any full blocks ──
    if has_full:
    ...
    # ── Pass 2: partial blocks (bm_val==1) — only if row has any partial blocks ──
    if has_partial:
        col_idx = first_nz_col
        while col_idx <= last_nz_col:
            bm_val = tl.load(block_mask_ptr + start_m * bm_stride_row + col_idx * bm_stride_col)
...
@triton.jit
def _binflash_fwd(
    Q, K, V, sm_scale, Out, Mask, BlockMask, FirstNz, LastNz, HasFull, HasPartial, LSE,
    ...
    has_full = tl.load(HasFull + start_m)
    has_partial = tl.load(HasPartial + start_m)
    acc, l_i, m_i = _binflash_fwd_inner(
        ...
        first_nz, last_nz, has_full, has_partial,
...
def binflash_attention(
    ...
    """v114 + per-row has_full/has_partial pass-skip flags."""
    ...
    if block_mask is None:
        ...
        if key == _bm_cache_key:
            block_mask, first_nz, last_nz, has_full, has_partial, chosen_bm, chosen_bn = _bm_cache_val
        else:
            bm128_bn64, fn, ln, hf, hp, gap_ratio, density = _fused_preproc(mask, 128, 64)
            if D == 64 and gap_ratio > _GAP_RATIO_THRESHOLD and N % 64 == 0:
                block_mask, first_nz, last_nz, has_full, has_partial, _, _ = _fused_preproc(mask, 64, 64)
                chosen_bm, chosen_bn = 64, 64
            elif D == 64 and density > _DENSITY_THRESHOLD and N % 128 == 0:
                block_mask, first_nz, last_nz, has_full, has_partial, _, _ = _fused_preproc(mask, 128, 128)
                chosen_bm, chosen_bn = 128, 128
            else:
                block_mask, first_nz, last_nz, has_full, has_partial = bm128_bn64, fn, ln, hf, hp
                chosen_bm, chosen_bn = 128, 64
            _bm_cache_key = key
            _bm_cache_val = (block_mask, first_nz, last_nz, has_full, has_partial, chosen_bm, chosen_bn)
    else:
        nz = block_mask > 0
        nkb = nz.shape[1]
        col_indices = torch.arange(nkb, device=mask.device)
        ...
        first_nz = nz.to(torch.int32).argmax(dim=1).to(torch.int32)
        first_nz = torch.where(has_any_in_row, first_nz, torch.tensor(0, device=mask.device)).to(torch.int32)
        has_full = (block_mask == 2).any(dim=1).to(torch.int8)
        has_partial = (block_mask == 1).any(dim=1).to(torch.int8)
    ...
    _binflash_fwd[grid](
        q, k, v, sm_scale, o, mask, block_mask, first_nz, last_nz, has_full, has_partial, lse,
