"""Binary Block Masked Flash Attention — v148: extend packed mask to D=128 gap-heavy.

v147 (dead end #15 win) enabled packed mask for the BM=64 path (longformer
at D=64) unconditionally. At D=128, the same gap-heavy patterns
(longformer, cdw_sinks) can't use BM=64 (register pressure per dead end
#13) so they fall through to the default BM=128 BN=64 path. There,
packed mask is gated by `density > 0.8`, which excludes longformer
(density ~0.03) and cdw_sinks (density ~0.02).

v148 extends the default path dispatch to also enable packed mask when
`gap_ratio > 0.3` — mirroring v147's fix one level up.

Dead end #46 (v131 unconditional pack) did show longformer D=128
regressed -10.8%, but that was a blanket enable across all patterns.
This version is targeted: only gap-heavy patterns at D=128 get packed,
sparse_10pct/30pct continue to use the density-based trigger (still
packed), and everything else stays regular.

Risk: register pressure at D=128 with the (BM, BN_PACKED, 32) unpack
intermediate. For BM=128, the intermediate is 32KB — tight but probably
fits since Q=32KB and acc=64KB already live in registers and v132 works.
"""
...
def binflash_attention(
    ...
    """v147 + enable packed mask for gap-heavy patterns in the default path (D=128)."""
    ...
    if block_mask is None:
        ...
        if key == _bm_cache_key:
            ...
            if D == 64 and gap_ratio > _GAP_RATIO_THRESHOLD and N % 64 == 0:
                # v147 win: unconditional packed on BM=64 gap-heavy path
                ...
                # Default path BM=128 BN=64 — packed if (density > 0.8 OR gap_ratio > 0.3)
                # The gap_ratio branch captures D=128 longformer/cdw_sinks that can't
                # use BM=64 due to register pressure but still benefit from packed mask.
                block_mask, first_nz, last_nz = bm128_bn64, fn, ln
                use_packed = 1 if (density > _DENSITY_THRESHOLD_PACKED or gap_ratio > _GAP_RATIO_THRESHOLD) else 0
        ...
        raise NotImplementedError("External block_mask not supported in v148")
