"""v31 v26 + skip_full — v26's work-ratio gate + HAS_FULL constexpr skip.

This is the properly-stacked version: v26 (now prod) + v29's HAS_FULL idea.
Base: v26 promotes BM=128 BN=32 dQ at N>=8192 with work_ratio gate. v31 adds
HAS_FULL: tl.constexpr that skips the full-block loop body when n_full=0 for
every block (e.g. sparse_10/sparse_30 where every block is partial).

Expected: marginal gain over v26 (1-2% on sparse patterns) if register pressure
was the bottleneck. v29 (v22-based) would regress on gap-heavy patterns vs
v26; v31 preserves v26's gap-heavy wins.

Legacy v29 docstring below.
---
v29 skip_full — v22 + HAS_FULL constexpr skip of the full-block loop.

For patterns where every block is partial (sparse_10, sparse_30 — mask density
~1.0), n_full is always 0 for every Q-block and the full-block loop runs 0 iters.
The compiler still allocates registers/SMEM for the loop body. v29 adds a
compile-time `HAS_FULL: tl.constexpr` that wraps the full-block loop in an
`if HAS_FULL:` block. When HAS_FULL=False (sparse patterns), Triton compiles
a leaner kernel without the full-block body.

Python side: compute `has_full = (n_full > 0).any()` (one-shot D2H per preproc,
cached) and pass as constexpr. Triggers a new autotune cache entry keyed on
HAS_FULL (2 variants total: with/without full-block body).

Hypothesis: for sparse_10/sparse_30 patterns where dense wins already dominate
total runtime, a leaner partial-only kernel may free a few registers or improve
scheduling. Mid-single-digit gain possible if register pressure was the bottleneck.

Below is v22's docstring for reference.
---
v22 bm128_dq_gated — v19 with N-gated BM=128 dQ dispatch.

v19 full-matrix (bwd_v19.csv, 165 cells): aggregate −3% vs prod, but not clean:
- WINS at N=16384: causal -3.2%, log_tree -2.2%, longformer -3.6%, sparse_10 -3.3%,
  sparse_30 -6.5%, causal_doc_7 -4.9%, block_diag -4.4%
- LOSSES at N=4096: block_diag +2.8%, causal_doc_7 +1.6%, longformer +4.8%,
  causal_window +3.3%, cdw_sinks +1.9%, sliding_window mixed
- Mixed at N=8192

Hypothesis: BM=128 halves the number of dQ programs. At small N this reduces
occupancy below SM count → worse. At large N, occupancy is already saturated
so the bigger tile amortizes overhead better.

Gate: use BM=128 BN=32 for dQ if N >= 8192, else fall back to BM=64 BN=64 (v4
path). dK/dV stays at BM=BN=64 always (v19's dK/dV was unchanged already).

Preserves the extra (N/128, N/32) block mask; it's only used when the gate is
triggered, otherwise the BM=BN=64 path uses the regular bwd_block_mask.

v4 features kept (same as v19)."""
...
def _bwd_dkdv_inner(
    ...
    N_CTX: tl.constexpr,
    USE_PACKED: tl.constexpr,
    HAS_FULL: tl.constexpr,  # v29: skip full-block loop compile-time
):
    offs_d = tl.arange(0, HEAD_DIM)
    ...
    # Pass 1: full Q-blocks — no mask. v29: skipped when HAS_FULL=False.
    if HAS_FULL:
        for idx in tl.range(0, n_full, num_stages=3):
...
@triton.autotune(
    ...
    key=["N_CTX", "HEAD_DIM", "Z_TIMES_H", "BLOCK_M", "BLOCK_N", "USE_PACKED", "HAS_FULL"],
)
@triton.jit
def _bwd_dkdv_kernel(
    ...
    dk, dv = _bwd_dkdv_inner(
        ...
        BLOCK_M, BLOCK_N, HEAD_DIM, N_CTX, USE_PACKED, HAS_FULL,
    )
...
def _bwd_dq_inner(
    ...
    # Pass 1: full K-blocks — no mask. v29: skip body when HAS_FULL=False.
    if HAS_FULL:
        for idx in tl.range(0, n_full, num_stages=3):
...
def _binflash_bwd(do, q, k, v, o, lse, mask, sm_scale, preproc=None):
    ...
    # v29: HAS_FULL constexpr — skip compile of full-block loop when every
    # Q-block (or K-block) has n_full == 0 (e.g. sparse_10 / sparse_30 patterns
    # where every block is partial). Checked via .max() on the counts.
    has_full_bwd = bool(preproc.get("n_full_bwd_max", int(n_full_kt.max().item())) > 0) if "n_full_bwd_max" in preproc else bool(int(n_full_kt.max().item()) > 0)
    has_full_dq = bool(int(preproc["n_full_bwd"].max().item()) > 0)
    # Cache these so subsequent calls skip the .max() reduction
    if "n_full_bwd_max" not in preproc:
        preproc["n_full_bwd_max"] = int(n_full_kt.max().item())
        preproc["n_full_dq_max"] = int(preproc["n_full_bwd"].max().item())
        preproc["n_full_dq128_max"] = int(preproc["n_full_dq"].max().item())
    ...
    _bwd_dkdv_kernel[grid_dkdv](
        ...
        USE_PACKED=USE_PACKED,
        HAS_FULL=has_full_bwd,
    )

    # v31: v26 gate (N>=8192 AND work_ratio<=1.1) + HAS_FULL skip.
    ...
    if N >= 8192 and work_ratio <= 1.1:
        ...
        has_full_dq_dispatch = preproc["n_full_dq128_max"] > 0
    else:
        dq_bm, dq_bn = _BWD_BLOCK_M, _BWD_BLOCK_N  # 64, 64
        col_indices_dq = col_indices
        n_full_dq = n_full
        n_total_dq = n_total
        has_full_dq_dispatch = preproc["n_full_dq_max"] > 0
