...
def _gathered_fwd_inner(
    ...
    for idx in tl.range(0, n_full, num_stages=2):  # fwd_v_acc14: ns=2 (between 1 and 3)
        ...
        v = tl.load(v_ptrs)
        qk = tl.dot(q, k) * 1.44269504  # fwd_v_acc8: post-multiply by log2e in fp32 (q already scaled by sm_scale)
        ...
        l_i = l_i * alpha + l_ij
        acc = acc * alpha[:, None]
        # fwd_v_acc9: combine v_acc8 (postscale log2e) + v_acc5 (fp16 P@V)
        p = p.to(tl.float16)
        acc = tl.dot(p, v.to(tl.float16), acc)
        m_i = m_ij

    # Pass 2: partial blocks — iterate [n_full, n_total), with mask.
    for idx in tl.range(n_full, n_total, num_stages=2):  # fwd_v_acc14: ns=2
        col_idx = tl.load(col_indices_ptr + idx)
        start_n = col_idx * BLOCK_N
        k_ptrs = K_base + offs_d[:, None] * stride_kk + (start_n + offs_bn)[None, :] * stride_kn
        k = tl.load(k_ptrs)
        ...
        qk = tl.dot(q, k) * 1.44269504  # fwd_v_acc8: post-multiply by log2e in fp32
        qk += tl.where(mask_, 0.0, -1.0e6 * 1.44269504)  # adjust mask to log2 basis
        m_ij = tl.maximum(m_i, tl.max(qk, 1))
        qk -= m_ij[:, None]
        p = tl.math.exp2(qk)
        l_ij = tl.sum(p, 1)
...
def _gathered_fwd(
    ...
    q = tl.load(Q_bp)
    # fwd_v_acc8: pre-scale q by sm_scale only (no log2e). Avoids bf16 cast bias on (sm_scale*log2e) constant.
    # The log2e factor is applied post-matmul in fp32 inside _gathered_fwd_inner.
