...
def _bwd_dq_inner(
    ...
    # so its SMEM/register pressure is much less than dKdV. Add tl.range(num_stages=2, disallow_acc_multi_buffer=True)
    # for cross-iteration K, V load pipelining. dKdV's tl.range stays unrolled (v38).
    # Pass 1: full K-blocks — no mask.
    for idx in tl.range(0, n_full, num_stages=2, disallow_acc_multi_buffer=True):
    ...
    for idx in tl.range(n_full, n_total, num_stages=2, disallow_acc_multi_buffer=True):
        col_idx = tl.load(ci_base + idx)
        start_n = col_idx * BLOCK_N
        k_ptrs = K_base + offs_d[:, None] * stride_kk + (start_n + offs_bn)[None, :] * stride_kn
        k = tl.load(k_ptrs)
