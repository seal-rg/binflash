...
# v_acc71: extended autotune for fp16 k_scaled (ext autotune)
for _s in [1, 2, 3]:
    for _w in [2, 4, 8]:
...
def _bwd_fused_kernel(
    ...
    k = tl.load(K_bp)
    v = tl.load(V_bp)
    k_scaled = (k * (sm_scale * 1.44269504)).to(tl.float16)  # v_acc71: fp16 k_scaled (no fp16 dV/dK)
    ...
    for idx in tl.range(0, n_full):
        ...
        qk = tl.dot(q.to(tl.float16), tl.trans(k_scaled))  # v_acc71: cast q to fp16 to match k_scaled
...
def _bwd_dkdv_inner(
    ...
    for idx in tl.range(0, n_full):
        ...
        qk = tl.dot(q.to(tl.float16), tl.trans(k_scaled))  # v_acc71: cast q to fp16 to match k_scaled   # already log2-basis because k is pre-scaled
        p = tl.math.exp2(qk - lse_i[:, None])
...
def _bwd_dq_inner(
    ...
    for idx in tl.range(0, n_full, num_stages=1):  # v_acc80: ns=1
        ...
        qk = tl.dot(q_scaled, k.to(tl.float16))  # v_acc72: cast k to fp16 to match q_scaled fp16
        p = tl.math.exp2(qk - lse_i[:, None])
    ...
    for idx in tl.range(n_full, n_total, num_stages=1):  # v_acc80: ns=1
        col_idx = tl.load(ci_base + idx)
        start_n = col_idx * BLOCK_N
        k_ptrs = K_base + offs_d[:, None] * stride_kk + (start_n + offs_bn)[None, :] * stride_kn
        k = tl.load(k_ptrs)
...
def _bwd_dq_kernel(
    ...
    q = tl.load(Q_bp)
    do = tl.load(DO_bp)
    q_scaled = (q * (sm_scale * 1.44269504)).to(tl.float16)  # v_acc72: fp16 q_scaled too
