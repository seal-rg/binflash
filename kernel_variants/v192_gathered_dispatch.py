"""V192: Gathered block dispatch.

Replaces the scan-based inner loop with index-based iteration over
precomputed sorted block indices. During preprocessing, non-empty block
column indices are extracted per Q-row, sorted by type (full blocks
first, partial blocks second), and stored in a compact array.

The kernel iterates directly over this array:
  - Pass 1: iterate [0, n_full) — no mask load needed
  - Pass 2: iterate [n_full, n_total) — load mask

This eliminates:
  - Scan waste (no iterating over empty blocks)
  - Per-iteration bm_val checks
  - The hierarchical coarse mask (USE_HIER / COARSE_FACTOR_C)

The dispatch logic (path 1 BM=64, path 2 BN=128, default) is preserved.
"""
...
# ─── Gathered indices preprocessing (NEW in v192) ───


def _build_gathered_indices(block_mask):
    """Build sorted column index array from block_mask.

    Extracts non-empty block indices per Q-row, sorted so full blocks (==2)
    come before partial blocks (==1). Empty blocks (==0) are omitted.
    ...
    n_full = is_full.sum(dim=1).to(torch.int32)
    n_partial = is_partial.sum(dim=1).to(torch.int32)
    n_total = n_full + n_partial
    max_nnz = max(int(n_total.max().item()), 1)

    # Sort key: full=0, partial=1, empty=2 → argsort gives full indices first
    sort_key = torch.where(is_full, 0, torch.where(is_partial, 1, 2)).to(torch.int32)
    sorted_cols = sort_key.argsort(dim=1, stable=True)
    col_indices = sorted_cols[:, :max_nnz].contiguous().to(torch.int32)
