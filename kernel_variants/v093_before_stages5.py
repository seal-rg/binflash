"""v093 baseline — two-loop with stages=[1-4], warps=[4,8].

Two-loop scan: pass 1 processes full blocks (bm_val==2, no mask load),
pass 2 processes partial blocks (bm_val==1, with mask). Separating them
lets the compiler pipeline K/V loads independently of mask loads.

Q pre-scaled. Z_TIMES_H in autotune key. 3-valued block mask cache.
Before adding stages=5 to autotune search.
"""
