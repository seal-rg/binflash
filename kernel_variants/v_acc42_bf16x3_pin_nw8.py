...
# v_acc42: pin to (nw=8, ns=1) — what bf16x3 actually picks per inspection
_bwd_autotune_configs = [triton.Config({}, num_stages=1, num_warps=8)]
...
def _bwd_dkdv_inner(
    ...
    for idx in tl.range(0, n_full):
        ...
        dv = tl.dot(tl.trans(p), do.to(tl.float32), dv, input_precision='bf16x3')  # v_acc23: 3xTF32 emulation

        dp = tl.dot(do, tl.trans(v)).to(tl.float32)
        ds = p * (dp - d_i[:, None])
        dk = tl.dot(tl.trans(ds), q.to(tl.float32), dk, input_precision='bf16x3')  # v_acc23: 3xTF32 emulation
