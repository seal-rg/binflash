...
def _gathered_fwd_inner(
    ...
    for idx in tl.range(0, n_full, num_stages=TL_STAGES):
    ...
    for idx in tl.range(n_full, n_total, num_stages=TL_STAGES):
        col_idx = tl.load(col_indices_ptr + idx)
        start_n = col_idx * BLOCK_N
        k_ptrs = K_base + offs_d[:, None] * stride_kk + (start_n + offs_bn)[None, :] * stride_kn
        k = tl.load(k_ptrs)
    ...
    for _w in [4, 8]:
        for _mnr in [None, 168, 192]:
            for _ts in [1, 2, 3]:
                ...
                if _mnr is not None:
                    _kw["maxnreg"] = _mnr
                _autotune_configs.append(triton.Config({"TL_STAGES": _ts}, num_stages=_s, num_warps=_w, **_kw))
