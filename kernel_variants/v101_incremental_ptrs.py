"""Binary Block Masked Flash Attention — v101: incremental pointer advance.

v100 recomputes full pointer expressions per non-empty block (multiply +
broadcast). This version precomputes base pointers before loops and uses
incremental += stride per iteration, like block_ptr's tl.advance but with
raw pointers (for while-loop compatibility).

Trade-off: advances pointers for ALL blocks in [first_nz, last_nz] range
(not just non-empty), but each advance is a cheap broadcast add vs the
multiply+add of the current approach.
"""
...
def _binflash_fwd_inner(
    ...
    # Precompute base pointers at first_nz column
    start_n0 = first_nz_col * BLOCK_N
    k_ptrs = K_base + offs_d[:, None] * stride_kk + (start_n0 + offs_bn)[None, :] * stride_kn
    v_ptrs = V_base + (start_n0 + offs_bn)[:, None] * stride_vk + offs_d[None, :] * stride_vn

    # Strides per column step (scalars, broadcast-added)
    k_col_stride = BLOCK_N * stride_kn
    v_col_stride = BLOCK_N * stride_vk
    ...
    # Reset K/V pointers back to first_nz, init mask pointers
    k_ptrs = K_base + offs_d[:, None] * stride_kk + (start_n0 + offs_bn)[None, :] * stride_kn
    v_ptrs = V_base + (start_n0 + offs_bn)[:, None] * stride_vk + offs_d[None, :] * stride_vn
    mask_ptrs = Mask_base + offs_bm[:, None] * stride_mask_m + (start_n0 + offs_bn)[None, :] * stride_mask_n
    mask_col_stride = BLOCK_N * stride_mask_n
...
def binflash_attention(
    ...
    """Binary-block-masked flash attention with incremental pointer advance."""
