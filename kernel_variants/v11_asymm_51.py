"""v11 asymm_51 — more extreme asymmetry (5 full, 1 partial). Push v9's idea further.

Full-block inner loops (no mask load/branch) get num_stages=5; partial-block
loops (mask load + conditional + divergence) get num_stages=2. Hypothesis:
different loop bodies have different SMEM/compute balance → different optimal
pipeline depth. In v270 fwd both loops were 3, but the bwd kernels may benefit
from asymmetric tuning.
...
def _bwd_dkdv_inner(
    ...
    # Pass 1: full Q-blocks — no mask. v9: num_stages=5 (more pipelining on light body).
    for idx in tl.range(0, n_full, num_stages=5):
    ...
    # Pass 2: partial Q-blocks — packed or bool mask. v11: num_stages=1.
    for idx in tl.range(n_full, n_total, num_stages=1):
        row_idx = tl.load(ri_base + idx)
        m_cur = row_idx * BLOCK_M + offs_m
        q_ptrs = Q_base + m_cur[:, None] * stride_qm + offs_d[None, :] * stride_qk
        do_ptrs = DO_base + m_cur[:, None] * stride_dom + offs_d[None, :] * stride_don
...
def _bwd_dq_inner(
    ...
    # Pass 1: full K-blocks — no mask. v9: num_stages=5.
    for idx in tl.range(0, n_full, num_stages=5):
    ...
    # Pass 2: partial K-blocks — packed or bool mask. v11: num_stages=1.
    for idx in tl.range(n_full, n_total, num_stages=1):
        col_idx = tl.load(ci_base + idx)
        start_n = col_idx * BLOCK_N
        k_ptrs = K_base + offs_d[:, None] * stride_kk + (start_n + offs_bn)[None, :] * stride_kn
