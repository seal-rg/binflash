"""v191: test the "more tail ops = more scheduling room" hypothesis.

Last iterations confirmed v179's LSE store + tl.where acts as a
scheduling anchor worth ~3% on sparse/log_tree/causal N=16384.
Hypothesis: if more tail work helps, adding a SECOND (redundant)
LSE store should amplify the effect.

Implementation: duplicate `tl.store(LSE + ..., m_i + log2(l_i))`.
The second store writes to the same address (redundant), but Triton
should NOT deduplicate it since stores are sequenced. The hardware
executes both.

Expected HBM overhead: BM × BH × N × 4 extra bytes. At N=32768 BH=32:
8MB extra / 960 GB/s = 8.3μs additional store time.

If v191 shows consistent 1-2%+ improvement over v188: more tail ops
genuinely help (could explore further).
If neutral: effect has saturated, or mechanism is different.
If negative: the store overhead exceeds the scheduling benefit.
"""
import math
import torch
import triton
...
def _binflash_fwd_v191(
    ...
    # v191: duplicate LSE store — tests "more tail ops = more anchors" hypothesis
    tl.store(LSE + (off_hz * N_CTX) + offs_m, lse_val)
    tl.store(LSE + (off_hz * N_CTX) + offs_m, lse_val)
    tl.store(O_bp, acc.to(Out.type.element_ty))
