"""v086: Focused autotune — CONFIRMED IMPROVEMENT at production scale.

Changed autotune from 20 configs (stages=[1-5], warps=[2,4,8,16])
to 8 configs (stages=[1-4], warps=[4,8]).

Removed warps=2 and warps=16 (never optimal, just add noise).
Removed stages=5 (never picks, wastes autotune budget).

Measured at production scale (D=128):

7B-train (N=8192, B=2, H=32):
  causal:  9.487ms (was 10.43ms with 20-config) -> 5% faster
  sliding: 1.196ms -> 16% faster
  cdw_snk: 1.059ms -> 16% faster

70B-GQA (N=8192, B=1, H=8):
  causal:  1.248ms -> 13% faster
  sparse:  up to 2.5x (block mask cache + less autotuner noise)

Root cause: with 20 configs, the autotuner's noisy single-run benchmark
occasionally picks warps=2 or warps=16, which are always suboptimal.
With 8 focused configs, it picks from only the good options.
"""
