...
def _bwd_fused_kernel(
    ...
    for idx in tl.range(0, n_full):
        ...
        qk = tl.dot(q.to(tl.float32), tl.trans(k_scaled).to(tl.float32), input_precision='tf32x3')  # v_acc82: tf32x3 QK
...
def _bwd_dkdv_inner(
    ...
    for idx in tl.range(0, n_full):
        ...
        qk = tl.dot(q.to(tl.float32), tl.trans(k_scaled).to(tl.float32), input_precision='tf32x3')  # v_acc82: tf32x3 QK   # already log2-basis because k is pre-scaled
        p = tl.math.exp2(qk - lse_i[:, None])
...
def _bwd_dq_inner(
    ...
    for idx in tl.range(0, n_full, num_stages=1):  # v_acc76: ns=1 to reduce reg pressure
        ...
        qk = tl.dot(q_scaled.to(tl.float32), k.to(tl.float32), input_precision='tf32x3')  # v_acc82: tf32x3 in dQ
        p = tl.math.exp2(qk - lse_i[:, None])
    ...
    for idx in tl.range(n_full, n_total, num_stages=1):  # v_acc76: ns=1
        col_idx = tl.load(ci_base + idx)
        start_n = col_idx * BLOCK_N
        k_ptrs = K_base + offs_d[:, None] * stride_kk + (start_n + offs_bn)[None, :] * stride_kn
        k = tl.load(k_ptrs)
...
_BWD_BLOCK_M = 64
_BWD_BLOCK_N = 32  # v_acc83: smaller BN to fit fp32 QK inputs in SMEM
