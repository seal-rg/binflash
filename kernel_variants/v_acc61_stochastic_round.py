...
def _bwd_dkdv_inner(
    ...
    for idx in tl.range(0, n_full):
        ...
        # v_acc61: stochastic rounding via deterministic noise — add ±0.5 LSB random
        # before fp16 cast. Removes systematic bias from RTNE accumulation.
        # fp16 LSB at scale ~1 is 2^-10. We add noise at this scale * relative magnitude.
        seed = (off_hz.to(tl.uint32) * 16777619 + idx.to(tl.uint32) * 65537)
        offsets_p = (tl.arange(0, BLOCK_M)[:, None] * BLOCK_N + tl.arange(0, BLOCK_N)[None, :])
        noise_p = (tl.rand(seed, offsets_p) - 0.5) * (2.0**-10) * tl.abs(p)
        p_noisy = (p + noise_p).to(tl.float16)
        dv = tl.dot(tl.trans(p_noisy), do.to(tl.float16), dv)

        dp = tl.dot(do, tl.trans(v)).to(tl.float32)
        ds = p * (dp - d_i[:, None])
        # Stochastic rounding for ds too
        offsets_ds = offsets_p + 1
        noise_ds = (tl.rand(seed + 1, offsets_ds) - 0.5) * (2.0**-10) * tl.abs(ds)
        ds_noisy = (ds + noise_ds).to(tl.float16)
        dk = tl.dot(tl.trans(ds_noisy), q.to(tl.float16), dk)
