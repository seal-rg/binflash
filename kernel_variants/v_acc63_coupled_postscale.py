"""BinFlash backward pass — v_acc63: COUPLED postscale (fwd + bwd).

Builds on v_acc62 (fp16 dV+dK + bwd-side postscale) by also INLINING a postscale
fwd path so that the LSE produced by fwd and consumed by bwd share the SAME
exponent basis.

In v_acc62, the bwd's postscale (drop log2e from k/q prescale, multiply qk by
1.44269504 in fp32) was paired with prod's BIASED-LSE fwd (where q is
pre-scaled by sm_scale*log2e in bf16). That mismatch caused regressions on
sliding/longformer.

This file inlines the postscale fwd from past_attempts_acc/fwd/v_acc9 so that
both sides use the SAME basis: q and k pre-scaled by `sm_scale` only, with the
log2e factor applied post-matmul in fp32.
"""
...
# ===================================================================
# Inlined fwd preprocessing (from past_attempts_acc/fwd/v_acc9_postscale_plus_fp16.py)
# ===================================================================
...
# ===================================================================
# Inlined fwd kernel (from past_attempts_acc/fwd/v_acc9_postscale_plus_fp16.py)
# ===================================================================
...
def _gathered_fwd_inner(
    ...
    for idx in tl.range(0, n_full, num_stages=3):
        ...
        v = tl.load(v_ptrs)
        qk = tl.dot(q, k) * 1.44269504  # postscale log2e in fp32 (q already scaled by sm_scale)
    ...
    for idx in tl.range(n_full, n_total, num_stages=3):
        ...
        qk = tl.dot(q, k) * 1.44269504  # postscale log2e in fp32
        qk += tl.where(mask_, 0.0, -1.0e6 * 1.44269504)
        m_ij = tl.maximum(m_i, tl.max(qk, 1))
        qk -= m_ij[:, None]
        p = tl.math.exp2(qk)
        l_ij = tl.sum(p, 1)
...
for _s in [1, 2, 3, 4, 5, 6, 7]:
    for _w in [4, 8]:
        for _mnr in [None, 168, 192]:
            ...
            if _mnr is not None:
                _kw["maxnreg"] = _mnr
            _fwd_autotune_configs.append(triton.Config({}, num_stages=_s, num_warps=_w, **_kw))
...
def _gathered_fwd(
    ...
    q = tl.load(Q_bp)
    # Postscale: pre-scale q by sm_scale only (no log2e). Apply log2e post-matmul in fp32.
    ...
    acc, l_i, m_i = _gathered_fwd_inner(
        acc, l_i, m_i, q, K_base, V_base, Mask, MaskPacked,
        stride_kk, stride_kn, stride_vk, stride_vn,
        stride_mask_m, stride_mask_n, stride_mp_m, stride_mp_n,
        ci_base, n_full, n_total, start_m,
...
# ─── Dispatch (D=128 path only — needed by autograd) ───
...
# ===================================================================
# Bwd kernels (from v_acc62 — fp16 dV+dK + bwd-side postscale)
# ===================================================================
...
def _bwd_dkdv_inner(
    ...
    for idx in tl.range(0, n_full):
        ...
        qk = tl.dot(q, tl.trans(k_scaled)) * 1.44269504  # postscale log2e in fp32
    ...
    for idx in tl.range(n_full, n_total):
        ...
        qk = tl.dot(q, tl.trans(k_scaled)) * 1.44269504
        if USE_PACKED:
...
def _bwd_dkdv_kernel(
    ...
    k = tl.load(K_bp)
    v = tl.load(V_bp)
    k_scaled = (k * sm_scale).to(k.dtype)  # postscale: drop log2e from k
...
def _bwd_dq_inner(
    ...
    for idx in tl.range(0, n_full, num_stages=2):
        ...
        qk = tl.dot(q_scaled, k) * 1.44269504  # postscale log2e
        p = tl.math.exp2(qk - lse_i[:, None])
    ...
    for idx in tl.range(n_full, n_total, num_stages=2):
        ...
        qk = tl.dot(q_scaled, k) * 1.44269504
        if USE_PACKED:
            start_n_packed = (col_idx * BLOCK_N) // 32
            mp_ptrs = MaskPacked_base + offs_bm[:, None] * stride_mp_m + (start_n_packed + offs_bn_packed)[None, :] * stride_mp_n
            packed = tl.load(mp_ptrs)
...
def _bwd_dq_kernel(
    ...
    q = tl.load(Q_bp)
    do = tl.load(DO_bp)
    q_scaled = (q * sm_scale).to(q.dtype)  # postscale: drop log2e
...
# ─── Fwd that returns LSE (using LOCAL postscale fwd kernel) ───
...
class _BinFlashFunction(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, mask, sm_scale):
        ...
        D = q.shape[-1]
        preproc = _autograd_preproc(mask, D)
        # Calls LOCAL postscale _gathered_fwd via _binflash_fwd_with_lse above.
...
def binflash_attention_autograd(q, k, v, mask, sm_scale=None):
    """v_acc63: coupled postscale fwd + bwd. LSE shares same basis on both sides."""
