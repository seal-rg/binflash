"""Binary Block Masked Flash Attention — v114: D-aware dispatch fix.

v113 dispatch triggers BM=64 for longformer (gap_ratio > 0.3) regardless
of HEAD_DIM. This works at D=64 (+12-24%) but REGRESSES at D=128 (-16%
on longformer N=8192) due to higher register pressure with BM=64 at D=128.

Fix: Add D == 64 check to the BM=64 dispatch. At D=128, fall back to
default BM=128 kernel for longformer.

Both dispatch paths are now D=64 only:
  1. gap_ratio > 0.3 AND D=64 → BM=64
  2. density > 0.9 AND D=64 → BN=128
  3. else → default BM=128, BN=64
"""
...
def binflash_attention(
    ...
    """Binary-block-masked flash attention with D-aware adaptive dispatch."""
    ...
    if block_mask is None:
        ...
        if key == _bm_cache_key:
            ...
            # Both specialized paths are D=64 only — BM=64/BN=128 hurt at D=128
