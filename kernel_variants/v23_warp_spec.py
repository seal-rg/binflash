"""v23 warp_spec — v4 + tl.range(..., warp_specialize=True) on full-block bwd loops.

Triton supports warp specialization — splits the loop into producer warps (async
loads) and consumer warps (compute). On Hopper this maps to async wgmma; on Ada
(SM89, RTX6000 Ada) Triton still emits specialized code. Full-block loops are
the hottest, no divergence, load K/V every iter — natural candidates.

Applied only to full-block loops in _bwd_dq_inner and _bwd_dkdv_inner. Partial
loops (with mask divergence) kept as-is."""
