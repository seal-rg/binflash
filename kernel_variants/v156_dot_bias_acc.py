"""v156: fuse mask bias into tl.dot accumulator argument.

Hypothesis: dead end #64 (v153) speculated that `qk += bias` is better than
`qk = tl.where(mask, qk, -inf)` because "the whole expression fuses into a
single tensor-core-output epilogue". This version makes that fusion explicit
by passing the bias as the accumulator argument to tl.dot:

    bias = tl.where(mask, 0.0, -1.0e6)
    qk = tl.dot(q, k, bias)

Triton's tl.dot(a, b, acc) computes a @ b + acc in one tensor-core epilogue,
which should be at least as fast as the separate add and potentially faster.
"""
...
def _binflash_fwd_inner_v156(
    ...
    while col_idx <= last_nz_col:
        ...
        if bm_val == 1:
            ...
            # v156: pass bias as tl.dot accumulator — fuses into tensor-core epilogue
            bias = tl.where(mask, 0.0, -1.0e6)
            qk = tl.dot(q, k, bias)
            m_ij = tl.maximum(m_i, tl.max(qk, 1))
            qk -= m_ij[:, None]
...
def _binflash_fwd_v156(
    ...
    acc, l_i, m_i = _binflash_fwd_inner_v156(
        acc, l_i, m_i, q,
