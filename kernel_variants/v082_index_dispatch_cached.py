"""v082: Index dispatch + block mask cache — CONFIRMED IMPROVEMENT.

Two changes:
1. Single-entry cache for block mask + sorted indices + counts
2. Index dispatch kernel: iterates only over non-empty blocks using
   precomputed sorted column indices with delta-based tl.advance

Results vs v078 (scan-and-skip, no cache):

D=64 N=8K:
  blkdiag:  1.80x (0.334 -> 0.185ms)
  cdw_snk:  1.75x (0.378 -> 0.216ms)
  sliding:  1.70x (0.412 -> 0.242ms)
  longfmr:  1.28x (0.677 -> 0.530ms)
  csl_doc:  1.24x (0.854 -> 0.692ms)
  causal:   1.02x (no regression)

D=128 N=16K:
  blkdiag:  1.99x (1.364 -> 0.686ms)
  cdw_snk:  1.70x
  sliding:  1.59x
  longfmr:  1.35x
  causal:   1.01x
...
Sources of improvement:
  - Cache eliminates make_block_mask + argsort on repeated calls (~0.35ms)
  - Index dispatch skips empty blocks entirely (no scan overhead)
  - Delta-based tl.advance enables sequential-ish block pointer access

This file contains the baseline kernel (same as v082_before_index_dispatch)
with the cache. The index dispatch kernel optimization was later found to be
neutral at kernel level -- the big win was from the cache alone.
"""
