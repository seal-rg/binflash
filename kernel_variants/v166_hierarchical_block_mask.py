"""v166: 2-level hierarchical block mask. On top of the current fine
block_mask at (BM, BN), add a coarse level at (BM, 2*BN). Each coarse
entry summarizes 2 adjacent fine columns (is any non-empty).

The main kernel scans the COARSE mask in the outer while loop. When a
coarse cell is empty, the kernel advances by 1 coarse column, which
corresponds to 2 fine columns. When a coarse cell is non-empty, the
kernel does an inner static 2-iter loop over the fine sub-columns to
check their individual bm_vals.

Motivation: `_scan_overhead_probe.py` shows longformer/cdw_sinks spend
~95% and ~87% of their within-scan-range iterations on empty-block
skips. Every other pattern has 100% scan efficiency. A coarse level
halves the empty-skip cost for gap-heavy patterns at minimal cost to
dense patterns (1 extra scalar load per 2 iters, no branching change).

Preproc: compute coarse = (fine reshape (:, nkb/2, 2) > 0).any(dim=2)
cheaply in PyTorch after producing the fine mask. Negligible cost.

Kernel body structure is UNCHANGED from v164 except for the while-loop
outer rewrite. Correctness should be identical because the coarse mask
is a strict superset of the fine non-empty positions.
"""
...
# ─── Fused preprocessing (all kernels copied verbatim from v164) ───
...
def _make_coarse_l2(block_mask: torch.Tensor) -> torch.Tensor:
    """Build a (nqb, nkb/2) coarse-in-BN level-2 mask from the fine
    block_mask. A coarse cell is non-zero iff either of its 2 adjacent
    fine columns is non-zero. Stored as int8 (0 or 1)."""
    nqb, nkb = block_mask.shape
    assert nkb % 2 == 0, f"nkb={nkb} must be even for 2x coarse level"
    # Reshape to (nqb, nkb/2, 2) and reduce with any along the last axis.
    reshaped = block_mask.view(nqb, nkb // 2, 2)
    coarse = (reshaped > 0).any(dim=2).to(torch.int8)
    return coarse


# ─── Main attention kernel (v166 modifications) ───
...
def _binflash_fwd_inner(
    ...
    # ── Pass 1 (full blocks) ──
    if USE_HIER:
        coarse_first = first_nz_col // 2
        coarse_last = last_nz_col // 2
        ...
        while coarse_col <= coarse_last:
            ...
            if coarse_val > 0:
                base_col = 2 * coarse_col
                # Sub-column 0. No bounds check: col_idx outside
                # [first_nz_col, last_nz_col] has block_mask == 0 by
                # construction so the bm_val == 2 filter is enough.
                col_idx = base_col
                bm_val = tl.load(
                ...
                if bm_val == 2:
                        ...
                        acc = tl.dot(p, v, acc)
                        m_i = m_ij
                # Sub-column 1 (same no-bounds-check rationale).
                col_idx = base_col + 1
                bm_val = tl.load(
                    block_mask_ptr + start_m * bm_stride_row + col_idx * bm_stride_col
                )
    ...
    if USE_HIER:
        ...
        while coarse_col <= coarse_last:
            ...
            if coarse_val > 0:
                base_col = 2 * coarse_col
                for sub in tl.static_range(2):
                    col_idx = base_col + sub
                    # No bounds check (see pass 1 rationale).
                    bm_val = tl.load(
                        block_mask_ptr + start_m * bm_stride_row + col_idx * bm_stride_col
                    )
                    if bm_val == 1:
...
# v166: enable hierarchical outer loop only when the scan has large
# gaps. The probe in _scan_overhead_probe.py shows only longformer &
# cdw_sinks have significant skip-waste (>20% of scan iterations are
# empty). Those are already the path-1 gap-heavy patterns.
# Principled dispatch: enable hierarchical scan ONLY when there is real
# scan waste to eliminate. `gap_ratio > 0.3` is the exact threshold at
# which the BM=64 gap-heavy path already fires. Within that path, also
# require a minimum mean scan width so that small-N cases where the
# coarse-loop overhead > saved empty-skip iters don't regress.
...
def _dispatch_and_preprocess(mask: torch.Tensor, D: int):
    ...
    if D == 64 and gap_ratio > _GAP_RATIO_THRESHOLD and N % 64 == 0:
        # Path 1: gap-heavy (longformer, cdw_sinks). Real scan waste, but
        # at small N the coarse overhead dominates — gate on scan width.
        block_mask = out["block_mask_fine"]
        first_nz, last_nz = _reduce_for_fine(block_mask)
        coarse_l2 = _make_coarse_l2(block_mask)
        use_hier = 1 if _mean_scan_width(first_nz, last_nz) >= _HIER_MIN_SCAN_WIDTH else 0
        return block_mask, first_nz, last_nz, mask_packed, 64, 64, 1, use_hier, coarse_l2

    if D == 64 and density > _DENSITY_THRESHOLD_BN128 and N % 128 == 0:
        # Path 2: dense BN=128. No scan waste (density > 0.9 implies
        # nnz/scan ≈ 1). USE_HIER=0.
        block_mask = out["block_mask_bn128"]
        first_nz, last_nz = _reduce_for_fine(block_mask)
        coarse_l2 = _make_coarse_l2(block_mask)
        return block_mask, first_nz, last_nz, mask_packed, 128, 128, 1, 0, coarse_l2
    ...
    # Default path: only use hier when there is real gap waste to save.
    # (gap_ratio > 0 typically means path 1 would have fired — but for
    # D=128 the dispatch can't use BM=64, so gap-heavy D=128 patterns
    # take the default path and benefit from hier here.)
    ...
    if block_mask is None:
        ...
        if key == _bm_cache_key:
            (block_mask, first_nz, last_nz, mask_packed, chosen_bm, chosen_bn,
             use_packed, use_hier, coarse_mask) = _bm_cache_val
        else:
            cache_val = _dispatch_and_preprocess(mask, D)
            _bm_cache_key = key
            _bm_cache_val = cache_val
            (block_mask, first_nz, last_nz, mask_packed, chosen_bm, chosen_bn,
             use_packed, use_hier, coarse_mask) = cache_val
    else:
        nz = block_mask > 0
        nkb = nz.shape[1]
        ...
        use_packed = 0
        use_hier = 0
        coarse_mask = _make_coarse_l2(block_mask)
