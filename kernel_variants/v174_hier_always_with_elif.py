"""v174: retest v172's idea (enable USE_HIER=1 unconditionally on
default path) now that v173's elif branch split is in production.

v172 failed (dead end #71) on compute-bound dense patterns (causal
-12 to -18%, prefix_lm, log_tree, causal_doc_7 all -10 to -20%)
because v171's nested-if merged body had register pressure from
mixing full/partial paths. v173's explicit mutual-exclusion elif
body separates the register allocation per arm, so dense patterns
with mostly bm_val==2 blocks should only pay the cost of the full
arm, not the partial arm.

Only changes dispatch: `use_hier = 1` on default path always.
Kernel code path reuses production (v173-in-binflash_attention).
"""
import math
import torch
import triton
...
def _dispatch_and_preprocess(mask: torch.Tensor, D: int):
    ...
    # v174: force USE_HIER=1 unconditionally on the default path.
    use_hier = 1
    cf = _CF_OTHER
    coarse = _make_coarse_lK(block_mask, int(math.log2(cf)))
