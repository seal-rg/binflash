"""Binary Block Masked Flash Attention — v097: dense-row fast path.

Idea: For Q-block rows where ALL K-blocks are full (bm_val==2 everywhere),
use a dedicated tight inner loop with zero block mask checks or mask loads.
This extends the two-loop insight — structurally removing the block_mask_ptr
from the loop gives the compiler the cleanest possible code.

Benefit: eliminates ~N/BLOCK_N scalar loads + branches per dense row in both
passes. For causal ~50% of rows are all-full; for dense attention 100%.
"""
...
@triton.jit
def _dense_fwd_inner(
    acc, l_i, m_i, q,
    K_block_ptr, V_block_ptr,
    BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr, HEAD_DIM: tl.constexpr,
    N_CTX: tl.constexpr,
):
    """Process ALL blocks — no block mask checks, no mask loads."""
...
@triton.jit
def _binflash_fwd(
    Q, K, V, sm_scale, Out, Mask, BlockMask, RowDense, LSE,
    ...
    # Check if this row is entirely dense (all blocks full)
    is_dense = tl.load(RowDense + start_m)
    if is_dense:
        acc, l_i, m_i = _dense_fwd_inner(acc, l_i, m_i, q, K_bp, V_bp, BLOCK_M, BLOCK_N, HEAD_DIM, N_CTX)
    else:
        acc, l_i, m_i = _binflash_fwd_inner(acc, l_i, m_i, q, K_bp, V_bp, M_bp, BlockMask, stride_bm_row, stride_bm_col, start_m, BLOCK_M, BLOCK_N, HEAD_DIM, N_CTX)
...
def binflash_attention(
    ...
    """Binary-block-masked flash attention with dense-row fast path."""
    ...
    if block_mask is None:
        ...
        if key == _bm_cache_key:
            ...
            _bm_cache_key = key
            _bm_cache_val = block_mask

    # Per-row dense flag: True if ALL blocks in that row are full (bm_val==2)
    row_dense = (block_mask.min(dim=1).values == 2).to(torch.int8)
    ...
    _binflash_fwd[grid](
        q, k, v, sm_scale, o, mask, block_mask, row_dense, lse,
