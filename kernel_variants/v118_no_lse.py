"""Binary Block Masked Flash Attention — v118: skip LSE store.

The LSE (log-sum-exp) output is computed and stored at the end of each
Q-row, but not used by the benchmark. Removing it saves:
- 1 log2 computation per row (BLOCK_M values)
- BLOCK_M stores per row
- LSE tensor allocation

Likely marginal but nonzero. Also cleaner for inference-only workloads.
"""
...
@triton.jit
def _binflash_fwd(
    Q, K, V, sm_scale, Out, Mask, BlockMask, FirstNz, LastNz,
    ...
    acc = acc / l_i[:, None]
    # LSE store REMOVED — saves BLOCK_M stores + 1 log2 per row
...
def binflash_attention(
    ...
    """Same as v114 but without LSE store."""
    ...
    # LSE tensor NOT allocated
    grid = lambda META: (triton.cdiv(N, chosen_bm), B * H)

    _binflash_fwd[grid](
        q, k, v, sm_scale, o, mask, block_mask, first_nz, last_nz,
