"""v092: Two-loop scan — CONFIRMED on all patterns at production scale.

Pass 1: full blocks (bm_val==2, K/V only, no mask in loop body)
Pass 2: partial blocks (bm_val==1, K/V + mask load)

Separating the passes lets the compiler optimize the full-block loop
independently -- no mask block_ptr in the staging -> better K/V pipelining.

Production results vs v049 (all patterns, not just causal):
  7B-train N=8K:
    causal:  8.34ms (was 10.43ms) -> 1.25x
    sliding: 1.24ms -> 1.16x
    longfmr: 1.68ms -> 1.18x
    csl_doc: 2.76ms -> 1.20x
    cdw_snk: 1.13ms -> 1.13x
  70B-GQA N=8K:
    causal:  1.03ms -> 1.40x
    sliding: 0.16ms -> 1.90x
  D=64 N=8K:
    sliding: 0.28ms -> 1.89x
    blkdiag: 0.20ms -> 1.93x
    csl_doc: 0.65ms -> 1.76x

No regressions on any config or pattern.
"""
...
def _binflash_fwd_inner(
    ...
    Mask_block_ptr = tl.advance(Mask_block_ptr, (0, -N_CTX))
    for start_n in range(0, N_CTX, BLOCK_N):
        start_n = tl.multiple_of(start_n, BLOCK_N)
        col_idx = start_n // BLOCK_N
        bm_val = tl.load(block_mask_ptr + start_m * bm_stride_row + col_idx * bm_stride_col)
