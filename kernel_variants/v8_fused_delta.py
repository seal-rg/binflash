"""v8 fused_delta — v4 + inline delta computation inside dQ and dK/dV kernels.

Eliminates `_bwd_preprocess_kernel` launch. Each worker computes its own delta
inline by loading O (added as kernel arg) and reducing `sum(dO * O, dim=-1)`
for the Q-row range it processes.

Saves: 1 kernel launch + HBM write (N*B*H*4B delta) + HBM read of delta.
Adds: each dQ kernel loads O for its Q-block (+ 1 × BLOCK_M × HEAD_DIM bf16)
and each dK/dV inner iter loads O for the Q-block it visits (+ inner cost).

v4 (kept):
  - @triton.autotune over num_stages / num_warps / maxnreg
  - tl.range(num_stages=3) on inner loops (v270)
  - tl.assume bounds hints
  - make_block_ptr for coalesced Q / dO / O stores
  - pre-scale q once at load time (q *= sm_scale * log2e)
  - packed-mask partial-block path
The dQ kernel is Q-outer / K-inner (structurally identical to the fwd) with
gradient math (dp = dO·V^T, ds = P·(dp-D), dq += ds·K) added. The dK/dV
kernel is the transposed flavor (K-outer / Q-inner) using the transposed
row_indices built in v2. Preproc identical to v3."""
...
def _bwd_dkdv_inner(
    dk, dv,
    k_scaled, v,
    Q_base, DO_base, O_base,  # v8: O added
    ...
    for idx in tl.range(0, n_full, num_stages=3):
        ...
        o_ptrs = O_base + m_cur[:, None] * stride_om + offs_d[None, :] * stride_on
        q = tl.load(q_ptrs)
        do = tl.load(do_ptrs)
        o_tile = tl.load(o_ptrs)

        lse_i = tl.load(LSE + off_hz * N_CTX + m_cur)
        # v8: inline delta for this Q-block.
        d_i = tl.sum(do.to(tl.float32) * o_tile.to(tl.float32), axis=1)
...
def _bwd_dkdv_kernel(
    ...
    stride_oz, stride_oh, stride_om, stride_on,  # v8: O strides
    stride_mask_m, stride_mask_n,
    stride_mp_m, stride_mp_n,
    stride_ri_row,
    Z, H, N_CTX,
    ...
    qvk_offset_o = (off_hz // H).to(tl.int64) * stride_oz + (off_hz % H).to(tl.int64) * stride_oh
    ...
    dk, dv = _bwd_dkdv_inner(
        dk, dv, k_scaled, v,
        Q + qvk_offset_q, DO + qvk_offset_do, O + qvk_offset_o,
        Mask, MaskPacked,
        LSE,
        stride_qm, stride_qk,
        stride_dom, stride_don,
...
def _bwd_dq_kernel(
    Q, K, V, sm_scale,
    DO, DQ, O,  # v8: O added for inline delta
    Mask,
    MaskPacked,
    LSE,  # Delta removed — computed inline from O*dO
    ...
    # v8: inline delta — sum(dO * O) per row, converted to fp32 for the scalar reduce.
...
def _binflash_bwd(do, q, k, v, o, lse, mask, sm_scale, preproc=None):
    ...
    # v8: no separate delta tensor — computed inline per-block inside each kernel.
    ...
    # v8: skip `_bwd_preprocess_kernel` launch; delta is fused into dQ and dK/dV.

    grid_dkdv = (triton.cdiv(N, _BWD_BLOCK_N), B * H)
    _bwd_dkdv_kernel[grid_dkdv](
        q, k, v, sm_scale, do, dk, dv, o, mask, mask_packed, lse,
    ...
    _bwd_dq_kernel[grid_dq](
        q, k, v, sm_scale, do, dq, o, mask, mask_packed, lse,
