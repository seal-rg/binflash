"""v182: add maxnreg=128 and 144 back to v179's autotune space.

Dead end #53 tested maxnreg=128 without stages=6/7 and it was never
picked. v179 sparse_10pct picks stages=6 maxnreg=168 (aggressive end
of current range). With stages=6/7 now enabled, maxnreg=128 or 144
might be picked if the deep pipelining benefits from higher
occupancy enabled by lower register use.

Autotune space: 7 stages × 2 warps × 5 maxnreg = 70 configs
(vs v179's 42). 1.67x first-run autotune time.
...
for s in [1, 2, 3, 4, 5, 6, 7]:
    for w in [4, 8]:
        for mnr in [None, 128, 144, 168, 192]:
            kwargs = {}
            if mnr is not None:
                kwargs["maxnreg"] = mnr
