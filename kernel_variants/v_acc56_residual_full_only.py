...
def _bwd_dkdv_inner(
    ...
    # Pass 1: full Q-blocks — no mask. v_acc56: residual correction here only.
    for idx in tl.range(0, n_full):
        row_idx = tl.load(ri_base + idx)
        m_cur = row_idx * BLOCK_M + offs_m
        q_ptrs = Q_base + m_cur[:, None] * stride_qm + offs_d[None, :] * stride_qk
        ...
        # v_acc56: residual correction on FULL pass (deepest accumulation rows live here)
    ...
    # Pass 2: partial Q-blocks — packed or bool mask. (bf16, unchanged)
    for idx in tl.range(n_full, n_total):
        row_idx = tl.load(ri_base + idx)
        m_cur = row_idx * BLOCK_M + offs_m
        q_ptrs = Q_base + m_cur[:, None] * stride_qm + offs_d[None, :] * stride_qk
