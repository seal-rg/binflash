...
def _bwd_fused_kernel(
    ...
    k = tl.load(K_bp)
    v = tl.load(V_bp)
    k_scaled = (k * (sm_scale * 1.44269504)).to(k.dtype)  # v_acc8 placeholder; see below
    ...
    for idx in tl.range(0, n_full):
        ...
        qk = tl.dot(q.to(tl.float32), tl.trans(k_scaled))  # v_acc8: q→fp32 for fp32×fp32 matmul
...
def _bwd_dkdv_inner(
    ...
    for idx in tl.range(0, n_full):
        ...
        qk = tl.dot(q.to(tl.float32), tl.trans(k_scaled))  # v_acc8: q→fp32 for fp32×fp32 matmul   # already log2-basis because k is pre-scaled
        p = tl.math.exp2(qk - lse_i[:, None])
...
def _bwd_dkdv_kernel(
    ...
    k = tl.load(K_bp)
    v = tl.load(V_bp)
    # v_acc8: drop the bf16 cast on k_scaled. Keep fp32 multiply result, use mixed-precision
    # matmul (bf16 q × fp32 k_scaled). Loses speed (TF32 matmul) but preserves K precision.
    k_scaled_fp32 = k.to(tl.float32) * (sm_scale * 1.44269504)
    k_scaled = k_scaled_fp32  # passed as fp32 to inner
