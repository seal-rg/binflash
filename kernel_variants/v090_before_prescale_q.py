"""v090 baseline — full-block skip + focused autotune, before Q pre-scaling.

3-valued block mask (0=skip, 1=partial, 2=full). QK dot computed BEFORE mask
check: for bm_val==2, mask load is skipped entirely. Single loop with nested
`if bm_val == 1:` inside `if bm_val != 0:`.

Focused autotune: stages=[1-4], warps=[4,8]. Z_TIMES_H in key.
Block mask cache with 3-valued (has_any + has_all) computation.
"""
