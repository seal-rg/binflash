"""v171: revisit dead end #21 (single-pass merge) within the v168
hierarchical structure. The two while loops inside USE_HIER (pass 1
full + pass 2 partial) each scan the same coarse range. Merging them
halves the outer-scan overhead — e.g., for longformer N=32768 at CF=8,
the outer loop runs 64 iters instead of 128.

Dead end #21 (v102) merged pass 1+2 on the flat (non-hier) structure
and was 5-22% slower because the compiler couldn't optimize the
full-block path well when mixed with the partial-block path. The
hypothesis: with `tl.static_range(CF)` providing explicit unroll,
the compiler has enough static visibility to schedule instructions
across both full and partial paths efficiently.

Only the USE_HIER branches are merged. Non-hier (dense patterns, small
N) keeps the two-pass structure from v168.
"""
import math
import torch
import triton
...
def _binflash_fwd_inner_v171(
    ...
    if USE_HIER:
        # v171: SINGLE-PASS merged hier. One outer while loop scans the
        # coarse range and handles both full (bm_val==2) and partial
        # (bm_val==1) blocks within the same inner body.
        ...
        while coarse_col <= coarse_last:
            ...
            if coarse_val > 0:
                ...
                for sub in tl.static_range(COARSE_FACTOR_C):
                    ...
                    if bm_val > 0:  # non-empty (full OR partial)
                        ...
                        v = tl.load(v_ptrs)
                        qk = tl.dot(q, k)
                        # Apply mask only for partial blocks (bm_val == 1)
                        if bm_val == 1:
                            if USE_PACKED:
        ...
        # Non-hier two-pass (unchanged from v168)
...
def _binflash_fwd_v171(
    ...
    acc, l_i, m_i = _binflash_fwd_inner_v171(
        acc, l_i, m_i, q,
...
def binflash_attention(q, k, v, mask, sm_scale=None, block_m=128, block_n=None, block_mask=None):
    ...
    if block_mask is None:
        ...
        raise NotImplementedError("external block_mask path not implemented in v171 test file")
