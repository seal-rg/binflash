...
def _bwd_dkdv_inner(
    ...
    for idx in tl.range(0, n_full):
        ...
        dv = tl.dot(tl.trans(p), do.to(tl.float32), dv, input_precision='tf32x3')  # v_acc27: tf32x3 dV FULL

        dp = tl.dot(do, tl.trans(v)).to(tl.float32)
        ds = p * (dp - d_i[:, None])
        dk = tl.dot(tl.trans(ds), q.to(tl.float32), dk, input_precision='tf32x3')  # v_acc27: tf32x3 dK FULL
