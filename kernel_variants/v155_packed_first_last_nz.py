"""Binary Block Masked Flash Attention — v155: int64-packed first_nz/last_nz.

Current kernel performs 2 scalar loads per program:
    first_nz = tl.load(FirstNz + start_m)
    last_nz = tl.load(LastNz + start_m)

v155 packs both into a single int64 tensor at preprocessing time:
    fn_ln[i] = (last_nz[i] << 32) | (first_nz[i] & 0xFFFFFFFF)
Kernel does one load + unpack via shifts.

Savings: 1 fewer scalar load per program. Wall-clock impact is
negligible (<1μs for 4096 programs on 142 SMs), but this is a genuinely
untested micro-optimization. Recording the result either way.

Kernel body is otherwise identical to v149 production.
"""
...
def _fused_preproc(mask, block_m, block_n, want_packed=False):
    ...
    # NEW in v155: pack first_nz (low 32 bits) and last_nz (high 32 bits) into int64
    fn_i64 = first_nz.to(torch.int64) & 0xFFFFFFFF
    ln_i64 = last_nz.to(torch.int64) << 32
    first_last_packed = (fn_i64 | ln_i64).contiguous()
    mask_packed = _pack_mask(mask) if want_packed else None
    return block_mask, first_nz, last_nz, first_last_packed, avg_gap_ratio, density, partial_fraction, mask_packed
...
@triton.jit
def _binflash_fwd(
    Q, K, V, sm_scale, Out, Mask, MaskPacked, BlockMask, FirstLastPacked, LSE,
    ...
    # v155: single int64 load, unpack via shifts
    fn_ln = tl.load(FirstLastPacked + start_m)
    first_nz = (fn_ln & 0xFFFFFFFF).to(tl.int32)
    last_nz = (fn_ln >> 32).to(tl.int32)
...
def binflash_attention(
    ...
    """v149 with first_nz/last_nz packed into int64 for one-load setup."""
    ...
    if block_mask is None:
        ...
        if key == _bm_cache_key:
            block_mask, first_last_packed, mask_packed, chosen_bm, chosen_bn, use_packed = _bm_cache_val
        else:
            bm128_bn64, fn, ln, fl_packed, gap_ratio, density, partial_fraction, _ = _fused_preproc(
                mask, 128, 64, want_packed=False,
            )
            if D == 64 and gap_ratio > _GAP_RATIO_THRESHOLD and N % 64 == 0:
                block_mask, _, _, first_last_packed, _, _, _, mask_packed = _fused_preproc(
                ...
                block_mask, first_last_packed = bm128_bn64, fl_packed
                use_packed = 1 if (
            ...
            _bm_cache_key = key
            _bm_cache_val = (block_mask, first_last_packed, mask_packed, chosen_bm, chosen_bn, use_packed)
    else:
        raise NotImplementedError("External block_mask not supported in v155")
    ...
    _binflash_fwd[grid](
        q, k, v, sm_scale, o, mask, mp_tensor, block_mask, first_last_packed, lse,
