...
_bwd_autotune_configs = [
    # v_acc60: pinned to a small set of known-good configs to reduce lottery variance with extra residual matmul
    triton.Config({}, num_stages=1, num_warps=8, maxnreg=192),
    triton.Config({}, num_stages=2, num_warps=8, maxnreg=192),
    triton.Config({}, num_stages=1, num_warps=4, maxnreg=192),
]
...
def _bwd_fused_kernel(
    ...
    for idx in tl.range(0, n_full):
        ...
        # v_acc55: residual correction — main matmul in fp16, residual in bf16
        p_hi = p.to(tl.float16)
        p_lo = (p - p_hi.to(tl.float32)).to(do.dtype)
        dv = tl.dot(tl.trans(p_hi), do.to(tl.float16), dv)
        dv = tl.dot(tl.trans(p_lo), do, dv)
        dp = tl.dot(do, tl.trans(v)).to(tl.float32)
        ds = p * (dp - d_i[:, None])
        # v_acc55: residual correction for dK
        ds_hi = ds.to(tl.float16)
        ds_lo = (ds - ds_hi.to(tl.float32)).to(q.dtype)
        dk = tl.dot(tl.trans(ds_hi), q.to(tl.float16), dk)
        dk = tl.dot(tl.trans(ds_lo), q, dk)
        # dq via atomic_add
