"""v163: multi-granularity fused preproc. Extends the v161 fused
compute+pack kernel to also produce a fine-grained block_mask at
(BM=64, BN=64) from the same tile read. Path 1 (gap-heavy: longformer,
cdw_sinks) uses the fine output directly and SKIPS its second
`_fused_preproc` call entirely.

Motivation: after v162, the preproc_ms column shows path-1 patterns
still paying ~0.9ms at N=16384 because they make two `_fused_preproc`
calls (0.52ms + 0.38ms). Both calls read the full (N,N) mask from HBM.
This version reads the mask ONCE and produces both block_mask
granularities + pack in a single kernel launch.

Per program (BM_COARSE=128, BN_CHUNKS=2 at BLOCK_N=64, tile 16KB):
  - Coarse bm_vals: BN_CHUNKS (2) values — reduce tile over (BM=128, BN)
  - Fine bm_vals:   2 × BN_CHUNKS (4) values — reduce each BM=64 half
  - Packed words:   (128, 4) int32 — bit pack as before

No extra HBM reads — the fine reduction re-uses the tile already in
registers. Small added compute for the second sum axis.

Path 1 dispatches to the new multi-granularity kernel; default &
path 2 continue through v161's fused kernel that only produces the
coarse block_mask. Kernel body UNCHANGED.
"""
...
def _compute_and_pack_mask(
    ...
    """v161 fused compute+pack kernel (coarse block_mask only)."""
    row = tl.program_id(0)
    col_chunk = tl.program_id(1)
    offs_m = row * BLOCK_M + tl.arange(0, BLOCK_M)
...
def _compute_and_pack_mask_multi(
    Mask, BlockMaskCoarse, BlockMaskFine, Packed,
    stride_mask_m, stride_mask_n,
    stride_packed_m, stride_packed_n,
    nkb,
    ...
    """v163 multi-granularity fused kernel. Produces block_mask at two
    BM granularities + mask_packed from one tile read. Assumes
    BM_COARSE = 2 * BM_FINE.
    """
    ...
    # ─── Coarse block_mask (BM_COARSE, BN_CHUNKS) ───
    ...
    # ─── Fine block_mask (2 halves × BN_CHUNKS) ───
    tile_fine_4d = tl.reshape(tile_i32, (2, BM_FINE, BN_CHUNKS, BLOCK_N))
    sum_bm_f = tl.sum(tile_fine_4d, axis=1)  # (2, BN_CHUNKS, BLOCK_N)
    fine_sums = tl.sum(sum_bm_f, axis=2)  # (2, BN_CHUNKS)
    has_any_f = fine_sums > 0
    has_all_f = fine_sums == (BM_FINE * BLOCK_N)
    bm_fine = has_any_f.to(tl.int8) + has_all_f.to(tl.int8)  # (2, BN_CHUNKS)

    pair_idx = tl.arange(0, 2)
    fine_row_base = 2 * row_coarse + pair_idx  # (2,)
    fine_col = col_start  # (BN_CHUNKS,)
    fine_ptrs = (
        BlockMaskFine
        + fine_row_base[:, None] * nkb
        + fine_col[None, :]
    )  # (2, BN_CHUNKS)
    tl.store(fine_ptrs, bm_fine)

    # ─── Packed mask (unchanged from v161) ───
    WORDS: tl.constexpr = (BN_CHUNKS * BLOCK_N) // 32
...
def _fused_preproc_multigrain(mask, block_n):
    """v163: single-pass preproc producing BOTH (128, block_n) and
    (64, block_n) block_masks + mask_packed + metrics (measured at the
    COARSE granularity). Used when path 1 (gap-heavy, BM=64) is likely
    — we speculatively compute the fine output and only use it if the
    dispatch fires. Requires block_n divisible by 32 for packing."""
    N = mask.shape[0]
    assert block_n % 32 == 0
    ...
    _compute_and_pack_mask_multi[(nqb_coarse, nkb // bn_chunks)](
        mask, block_mask_coarse, block_mask_fine, mask_packed,
        mask.stride(0), mask.stride(1),
    ...
    # Metrics + first_nz/last_nz for COARSE (used by dispatch and default path).
...
def _reduce_for_fine(block_mask_fine):
    """Compute first_nz/last_nz for the fine block_mask (used when
    path 1 fires). Reuses _reduce_block_mask but discards the metrics.
    """
...
# ─── Main attention kernel (UNCHANGED from v149/.../v162) ───
...
def _dispatch_and_preprocess(mask: torch.Tensor, D: int):
    """v163: always run the multi-granularity fused kernel in call 1
    so path 1 (D=64 gap-heavy) can use the pre-computed fine block_mask
    directly and skip its second _fused_preproc call. Path 2 (D=64
    dense) still needs a separate BM=128 BN=128 call (BN granularity
    differs). Default path uses call 1's coarse output.
    """
    N = mask.shape[0]
    out = _fused_preproc_multigrain(mask, 64)  # coarse=(128,64), fine=(64,64)
    ...
    if D == 64 and gap_ratio > _GAP_RATIO_THRESHOLD and N % 64 == 0:
        # Path 1: reuse the fine block_mask from call 1. Only need to
        # compute first_nz/last_nz for the fine granularity.
    ...
    if D == 64 and density > _DENSITY_THRESHOLD_BN128 and N % 128 == 0:
        # Path 2: different BN granularity — still needs a separate call.
        block_mask, first_nz, last_nz, _, _, _, _ = _fused_preproc(
            mask, 128, 128, want_packed=False,
        )
        return block_mask, first_nz, last_nz, mask_packed, 128, 128, 1

    # Default path: use coarse outputs directly.
    use_packed = 1 if (
