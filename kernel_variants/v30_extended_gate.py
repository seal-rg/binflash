"""v30 extended_gate — v26 + extend BM=128 dQ dispatch to N=4096 for high-BH.

v26's gate is `N >= 8192 AND work_ratio <= 1.1`. At N=4096 with high BH
(llama3-8b: 128, 7B-batch8: 256, qwen3-32b: 512), total programs at BM=128
would be N/128 × BH = 32 × BH = 4096+ programs — plenty for 142 SMs. v30
extends the gate to fire at N=4096 too when BH is sufficient.

Gate: `use BM=128 if ceil(N/128) * BH >= 1024 AND work_ratio <= 1.1`.

- llama3-8b N=4096 BH=128: 4096 programs >= 1024 → BM=128 (new path)
- 7B-batch8 N=4096 BH=256: 8192 programs → BM=128 (new path)
- 27B-kv-heads N=4096 BH=64: 2048 → BM=128 (new path)
- qwen3-32b N=4096 BH=512: 16384 → BM=128 (new path)
- 70B-kv-heads N=4096 BH=16: 512 < 1024 → BM=64 (v4 path)
- All N>=8192 at any BH: gate fires.

Note: v30 originally tried BM=128 BN=64 — FAILED SMEM OOM (101380 > 101376,
4 bytes over on Ada). Stuck at BN=32.

Below is v26 docstring for reference.
---
...
_BWD_BLOCK_N = 64
_BWD_DQ_BLOCK_M = 128
_BWD_DQ_BLOCK_N = 32  # BN=64 at BM=128 fails SMEM (101380 > 101376 limit on Ada — 4 bytes over). Stick at 32.
...
def _binflash_bwd(do, q, k, v, o, lse, mask, sm_scale, preproc=None):
    ...
    # v30: extend gate to fire at N=4096 for high-BH. Formula: use BM=128 if
    # total programs at BM=128 (= ceil(N/128) * BH) is >= 1024 AND work_ratio OK.
    work_ratio = preproc.get("work_ratio_dq", 99.0)
    total_programs_bm128 = triton.cdiv(N, _BWD_DQ_BLOCK_M) * (B * H)
    if total_programs_bm128 >= 1024 and work_ratio <= 1.1:
