"""Binary Block Masked Flash Attention — v115: cache modifier for K/V loads.

K and V are loaded once per block and not reused (streaming pattern).
Using `cache_modifier='cs'` (streaming) hints to the hardware that these
loads shouldn't pollute L1 cache. This might free L1 for Q (which IS
reused across all blocks) and the block_mask scalar loads (hot).

Low-risk experiment — if the compiler/hardware doesn't benefit, neutral.
"""
...
def _binflash_fwd_inner(
    ...
    while col_idx <= last_nz_col:
        ...
        if bm_val == 2:
            ...
            k = tl.load(k_ptrs, cache_modifier=".cg")  # streaming
            v_ptrs = V_base + (start_n + offs_bn)[:, None] * stride_vk + offs_d[None, :] * stride_vn
            v = tl.load(v_ptrs, cache_modifier=".cg")  # streaming
            qk = tl.dot(q, k)
    ...
    while col_idx <= last_nz_col:
        ...
        if bm_val == 1:
            ...
            mask = tl.load(mask_ptrs, cache_modifier=".cg") != 0
            qk = tl.dot(q, k)
...
def binflash_attention(
    ...
    """Binary-block-masked flash attention with cache modifier hints."""
