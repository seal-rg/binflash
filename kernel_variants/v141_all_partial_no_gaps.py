"""Binary Block Masked Flash Attention — v141: ALL_PARTIAL_NO_GAPS constexpr.

v134 (dead end #48) tested `HAS_FULL` constexpr to elide pass 1 when no
full blocks exist. It was neutral because Triton already handled empty
while loops efficiently.

This version goes further: when (no full blocks) AND (no gaps in the
scan range), **both** pass 1 is elided and the `bm_val == 1` check
inside pass 2 is compile-skipped. Every iteration in the scan range is
known to be a partial block, so we process it unconditionally.

Targeted patterns: sparse_10pct/30pct at N≥8192 where block density is
~1.0 and every block in every row is partial. No other pattern matches:
causal has full blocks, longformer has gaps, etc.

Dispatch condition: `no_full_blocks AND gap_ratio == 0`. Computed in
preprocessing, cached. Passed as `ALL_PARTIAL_NO_GAPS: tl.constexpr`.

Kernel structure (USE_PACKED=1 branch):
  if ALL_PARTIAL_NO_GAPS:
      # Pass 1 skipped; pass 2 has no bm_val check
      col_idx = first_nz
      while col_idx <= last_nz:
          # Process partial block directly
          ...
  else:
      # v132 two-pass with bm_val check
      ...

Expected savings: 1 scalar load + 1 branch per iteration. For sparse_10pct
N=16384: ~256 iters/row * 4096 programs = 1M iterations. Tight inner
loop may also compile to better code with fewer branches.
"""
...
def _fused_preproc(mask, block_m, block_n, want_packed=False):
    ...
    # Check if all non-empty blocks are partial (no full blocks anywhere)
    has_full = (block_mask == 2).any().item()
    # Check if gap_ratio is 0 everywhere (all scan ranges are contiguous non-empty)
    zero_gaps = (gap_per_row == 0).all().item()
    all_partial_no_gaps = (not has_full) and zero_gaps
    mask_packed = _pack_mask(mask) if want_packed else None
    return block_mask, first_nz, last_nz, avg_gap_ratio, density, all_partial_no_gaps, mask_packed
...
def _binflash_fwd_inner(
    ...
    N_CTX: tl.constexpr, USE_PACKED: tl.constexpr, ALL_PARTIAL_NO_GAPS: tl.constexpr,
):
    ...
    if ALL_PARTIAL_NO_GAPS:
        # Pass 1 elided at compile time.
        # Pass 2: every iteration is a partial block (known at compile time),
        # skip the bm_val load + branch.
        ...
        # v132 two-pass structure, unchanged
        col_idx = first_nz_col
        while col_idx <= last_nz_col:
...
def _binflash_fwd(
    ...
    USE_PACKED: tl.constexpr, ALL_PARTIAL_NO_GAPS: tl.constexpr,
):
    ...
    acc, l_i, m_i = _binflash_fwd_inner(
        ...
        first_nz, last_nz,
        BLOCK_M, BLOCK_N, HEAD_DIM, N_CTX, USE_PACKED, ALL_PARTIAL_NO_GAPS,
    )
...
def binflash_attention(
    ...
    """v132 + ALL_PARTIAL_NO_GAPS constexpr for sparse-random patterns."""
    ...
    if block_mask is None:
        ...
        if key == _bm_cache_key:
            (block_mask, first_nz, last_nz, mask_packed, chosen_bm, chosen_bn,
             use_packed, all_partial_no_gaps) = _bm_cache_val
        else:
            bm128_bn64, fn, ln, gap_ratio, density, apng0, _ = _fused_preproc(mask, 128, 64, want_packed=False)
            if D == 64 and gap_ratio > _GAP_RATIO_THRESHOLD and N % 64 == 0:
                want = density > _DENSITY_THRESHOLD_PACKED
                block_mask, first_nz, last_nz, _, _, apng, mask_packed = _fused_preproc(
                    mask, 64, 64, want_packed=want,
                )
                chosen_bm, chosen_bn = 64, 64
                use_packed = 1 if want else 0
                all_partial_no_gaps = 1 if apng else 0
                ...
                all_partial_no_gaps = 1 if apng0 else 0
            _bm_cache_key = key
            _bm_cache_val = (block_mask, first_nz, last_nz, mask_packed, chosen_bm, chosen_bn,
                             use_packed, all_partial_no_gaps)
    else:
        raise NotImplementedError("External block_mask not supported in v141")
    ...
    _binflash_fwd[grid](
        ...
        USE_PACKED=use_packed, ALL_PARTIAL_NO_GAPS=all_partial_no_gaps,
    )
    return o
