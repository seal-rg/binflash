...
def _binflash_fwd_inner(
    ...
    for start_n in range(0, N_CTX, BLOCK_N):
        ...
        if bm_val != 0:
            ...
            mask = tl.load(Mask_block_ptr) != 0  # convert to bool
...
def binflash_attention(
    ...
    Args:
        ...
        block_mask: precomputed block mask (optional, computed from mask if not provided).
                    Shape depends on autotuned BLOCK_M/BLOCK_N — if provided, must match.
    """
    B, H, N, D = q.shape
    ...
    # Precompute block mask if not given — we don't know BLOCK_M/BLOCK_N until
    # autotuning runs, so we precompute for the largest block size (128x64)
    # and let the kernel index into it correctly.
    # With autotuning, we fix the block mask to match the autotuned config
    # by using a wrapper that creates it on-the-fly for the first call.
    if block_mask is None:
        # Use a fixed block size for the block mask that matches
        # the kernel's block pointer setup
        block_mask = make_block_mask(mask, 128, 64)
