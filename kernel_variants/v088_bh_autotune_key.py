"""v088: Added Z_TIMES_H (=B*H) to autotune key.

Allows autotuner to pick different configs for different occupancy levels:
- 7B (B*H=64): high occupancy, may prefer different warps/stages
- 70B-GQA (B*H=8): low occupancy

Measured effect: ~2% improvement on 7B-train causal (9.585->9.394ms).
Marginal but consistent, no regressions. Zero code change -- just key.
"""
