"""Binary Block Masked Flash Attention — v147: partial_fraction dispatch for BM=64 path.

v132's packed-mask dispatch uses `density > 0.8` for all paths. This
captures sparse_10pct/30pct (random sparse) where block-level density
is ~1.0, but MISSES longformer which has a legitimate case for packed
mask: longformer's BM=64 path processes only 8 non-empty blocks per
row on average, but EVERY ONE IS PARTIAL (diagonal band edges + sinks).

The correct criterion for packing is "fraction of NON-EMPTY blocks that
are partial" — call it partial_fraction = num_partial / num_nonempty.
When this is high (>0.5), nearly all processed blocks load a mask tile,
and packing gives a proportional bandwidth win.

For longformer: partial_fraction ≈ 1.0 (all non-empty blocks on the
diagonal band are partial) → should use packed mask.
For random sparse_10pct: gap_ratio = 0 so it never reaches the BM=64
path — this change only affects longformer-like gap patterns.

Expected effect: longformer D=64 N=16384 savings ~0.12ms (from 8KB-1KB
per block, ~8 blocks/row, 128 rows/head, 32 heads, 800GB/s ≈ 0.12ms).
On v132's 0.87ms longformer runtime, that's ~14% speedup.
"""
...
def _fused_preproc(mask, block_m, block_n, want_packed=False):
    ...
    # NEW: partial_fraction = fraction of non-empty blocks that are partial (bm_val==1)
    num_partial_total = (block_mask == 1).sum().item()
    num_nonempty_total = (block_mask > 0).sum().item()
...
_PARTIAL_FRACTION_THRESHOLD = 0.9  # NEW: for BM=64 gap-heavy path
...
def binflash_attention(
    ...
    """v132 + partial_fraction-based packed dispatch for BM=64 gap-heavy path."""
    ...
    if block_mask is None:
        ...
        if key == _bm_cache_key:
            ...
            bm128_bn64, fn, ln, gap_ratio, density, _, _ = _fused_preproc(mask, 128, 64, want_packed=False)
            if D == 64 and gap_ratio > _GAP_RATIO_THRESHOLD and N % 64 == 0:
                # BM=64 gap-heavy path: unconditionally enable packed mask.
                # Only longformer-like patterns hit this path (other benchmark
                # patterns have gap_ratio == 0), and longformer's partial_fraction
                # ~0.57 means most of the time is spent on mask-loading partial
                # blocks where packing reduces bandwidth ~4x.
        ...
        raise NotImplementedError("External block_mask not supported in v147")
