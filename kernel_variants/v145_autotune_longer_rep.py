"""Binary Block Masked Flash Attention — v145: autotune warmup=100 rep=500.

Triton's `triton.autotune` has optional `warmup` and `rep` kwargs that
control how long each candidate config is benchmarked during the
autotune search. Defaults are typically small (warmup~25, rep~100).
With small rep, per-config benchmark timings can be noisy, leading to
suboptimal picks when two configs are close.

This version explicitly sets longer `warmup=100, rep=500` for the
autotune benchmarks. Expected: the same config often wins, but in
cases where the winner is borderline, a more stable measurement may
pick a slightly better config.

Note: this makes autotune COMPILATION slower (~5x for each cached
shape). But warmup isn't part of the benchmark timing, so the
production cost is the same.

Kernel body is identical to v132.
"""
...
# Longer autotune benchmarks for more stable config selection
@triton.autotune(
    ...
    warmup=100,  # Default is ~25
    rep=500,     # Default is ~100
)
@triton.jit
def _binflash_fwd(
...
def binflash_attention(
    ...
    """v132 with longer autotune warmup=100 rep=500 for more stable config selection."""
    ...
    if block_mask is None:
        ...
        raise NotImplementedError("External block_mask not supported in v145")
