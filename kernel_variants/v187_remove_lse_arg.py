"""v187: fully remove LSE from kernel signature and tensor allocation.

v185 stopped computing `log2(l_i)` and storing to LSE, but:
  - `lse = torch.empty(B, H, N, dtype=fp32)` was still allocated
  - LSE pointer was still passed as kernel argument
  - 2 int64 register slots still reserved for the pointer

v187 removes both. The `lse` tensor allocation is dead weight (no
consumer). Removing saves:
  - 1 torch.empty call per kernel invocation (~5-10μs)
  - 1 kernel argument (2 int64 regs at kernel entry)
  - 1 autotune cache key dimension (implicitly, via kernel hash)

For fast kernels like cdw_sinks N=4096 (0.10ms), the allocation
overhead is ~5-10% of total time — should be measurable above the
<0.5% CV noise floor.
"""
import math
import torch
...
@triton.jit
def _binflash_fwd_v187(
    Q, K, V, sm_scale, Out, Mask, MaskPacked, BlockMask, CoarseMask, FirstNz, LastNz,
    stride_qz, stride_qh, stride_qm, stride_qk,
    stride_kz, stride_kh, stride_kn, stride_kk,
    stride_vz, stride_vh, stride_vk, stride_vn,
    stride_oz, stride_oh, stride_om, stride_on,
...
def binflash_attention(q, k, v, mask, sm_scale=None, block_m=128, block_n=None, block_mask=None):
    ...
    _binflash_fwd_v187[grid](
        q, k, v, sm_scale, o, mask, mp_tensor, block_mask, coarse_mask, first_nz, last_nz,
