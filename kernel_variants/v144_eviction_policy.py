"""Binary Block Masked Flash Attention — v144: eviction policy hints.

Dead end #30 tested `cache_modifier='.cg'` (cache global, bypass L1).
That's different from `eviction_policy` which controls how long lines
stay in L2 after access.

Hypothesis: for sparse_10pct-like patterns where many Q programs within
a head share the same K/V data, adding `eviction_policy='evict_last'`
on K/V loads tells the L2 controller to prefer retaining these lines
for subsequent programs. For single-use data (Q, O), 'evict_first'
encourages eviction to free L2 space.

Applied to:
  - K loads (pass 1, pass 2 both branches): evict_last
  - V loads (pass 1, pass 2 both branches): evict_last
  - Q load: evict_first (single use per program)
  - O store: (Triton may not expose eviction_policy on stores)
"""
...
def _binflash_fwd_inner(
    ...
    while col_idx <= last_nz_col:
        ...
        if bm_val == 2:
            ...
            # evict_last: keep K in L2 for other programs in same head
...
def _binflash_fwd(
    ...
    # evict_first for Q: single-use per program, don't pollute L2
    q = tl.load(Q_bp, eviction_policy='evict_first')
...
def binflash_attention(
    ...
    """v132 + eviction_policy hints (evict_last on K/V, evict_first on Q)."""
    ...
    if block_mask is None:
        ...
        raise NotImplementedError("External block_mask not supported in v144")
