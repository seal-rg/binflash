...
def _fused_preproc_multigrain(mask, block_n):
    ...
    # V264: transposed physical layout (N/32, N) but present as mp[m, n] via .T
    # So stride_m=1, stride_n=N. Reads of 128 consecutive m rows become 1 coalesced load.
...
def _dispatch_and_preprocess(mask: torch.Tensor, D: int):
    ...
    # V263: For D=128 (the common case), skip metrics sync entirely.
    # Hardcode use_packed=1 (same mask_packed is already computed; loading packed
    # bitmask is strictly-not-worse than raw bool in the kernel). This eliminates
    # the ~28μs CPU sync from `.tolist()` of density/gap_ratio/partial_fraction.
    if D == 128:
