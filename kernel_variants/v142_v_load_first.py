"""Binary Block Masked Flash Attention — v142: V load before K.

Dead end #10 (v085) tested V load deferral (moving V load to later in
the block body). Result: NEUTRAL — Triton's compiler optimizes load
scheduling regardless of source order. But the symmetric case — moving
V load EARLIER (before K) — was never explicitly tested.

Hypothesis: On Ada, load issue order may affect hardware scheduling.
Issuing V first (longer latency to hide since it's used only after
softmax) before K (needed immediately for QK matmul) could theoretically
start the V fetch earlier in the pipeline. But in practice, Triton's
multi-stage pipelining already issues async loads for both and handles
this.

This is the last remaining "load order" permutation to try. Expected
neutral. If neutral, we confirm load order is irrelevant and close this
design space.
"""
...
def _binflash_fwd_inner(
    ...
    # ── Pass 1: full blocks (bm_val==2) — V loaded first ──
    ...
    # ── Pass 2: partial blocks (bm_val==1) — V loaded first ──
    col_idx = first_nz_col
    while col_idx <= last_nz_col:
        bm_val = tl.load(block_mask_ptr + start_m * bm_stride_row + col_idx * bm_stride_col)
        if bm_val == 1:
...
def binflash_attention(
    ...
    """v132 + V load before K in inner kernel body."""
    ...
    if block_mask is None:
        ...
        raise NotImplementedError("External block_mask not supported in v142")
