...
def _bwd_dq_inner(
    ...
    for idx in tl.range(0, n_full, num_stages=2):
        ...
        qk = tl.dot(q_scaled, k.to(tl.float16))  # v_acc67: cast k to fp16 for matmul (q_scaled already fp16)
        p = tl.math.exp2(qk - lse_i[:, None])
...
def _bwd_dq_kernel(
    ...
    q = tl.load(Q_bp)
    do = tl.load(DO_bp)
    q_scaled = (q * (sm_scale * 1.44269504)).to(tl.float16)  # v_acc67: fp16 q_scaled only
