"""v165: fuse the coarse `_reduce_block_mask` into the multigrain kernel
via atomic_min/max/add, eliminating its separate launch. A tiny
`_compute_gap_sum` kernel (~15μs) finalizes the per-row gap_ratio
average from the atomic outputs. Saves ~0.03-0.04ms on the default
path. Kernel body UNCHANGED from v149/.../v164.
"""
...
@triton.jit
def _compute_gap_sum(
    FirstNz, LastNz, NumNonemptyRow,
    GapSumOut,
    nqb,
    NQB_POW2: tl.constexpr,
):
    offs = tl.arange(0, NQB_POW2)
    valid = offs < nqb
    first_nz = tl.load(FirstNz + offs, mask=valid, other=0)
    last_nz = tl.load(LastNz + offs, mask=valid, other=-1)
    num_nonempty = tl.load(NumNonemptyRow + offs, mask=valid, other=0)
    has_any = num_nonempty > 0
    scan_width = tl.where(has_any, last_nz - first_nz + 1, 1)
    gap_per_row = 1.0 - num_nonempty.to(tl.float32) / scan_width.to(tl.float32)
    gap_per_row = tl.where(valid, gap_per_row, 0.0)
    gap_sum = tl.sum(gap_per_row)
    tl.store(GapSumOut, gap_sum)
...
def _reduce_block_mask(
    ...
    """Legacy reduce kernel — still used for paths 1/2 fine/bn128 bounds."""
    row = tl.program_id(0)
...
def _compute_and_pack_mask_multi(
    ...
    FirstNzCoarse, LastNzCoarse, NumNonemptyCoarseRow, GlobalCounters,
    ...
    """v165 multi-granularity fused kernel: three block_mask outputs +
    mask_packed, plus atomic-fused coarse bounds/counts. Eliminates the
    separate `_reduce_block_mask` launch on the default path.

    The caller must pre-initialize:
      FirstNzCoarse[nqb_coarse] = nkb_bn64 (sentinel for min)
      LastNzCoarse[nqb_coarse]  = -1       (sentinel for max)
      NumNonemptyCoarseRow[nqb_coarse] = 0
      GlobalCounters[2] = [0, 0]
    """
    ...
    col_start_vec = col_chunk * BN_CHUNKS + tl.arange(0, BN_CHUNKS)
    tl.store(BlockMaskCoarse + row_coarse * nkb + col_start_vec, bm_coarse)

    # ── v165: fused bounds + counter atomics ──
    local_nonempty = has_any_c.to(tl.int32)
    local_partial = (has_any_c & (coarse_sums != (BM_COARSE * BLOCK_N))).to(tl.int32)
    num_nonempty_local = tl.sum(local_nonempty)
    num_partial_local = tl.sum(local_partial)
    col_fallback_first = tl.where(has_any_c, col_start_vec, nkb)
    local_first = tl.min(col_fallback_first, axis=0)
    col_fallback_last = tl.where(has_any_c, col_start_vec, -1)
    local_last = tl.max(col_fallback_last, axis=0)
    if num_nonempty_local > 0:
        tl.atomic_min(FirstNzCoarse + row_coarse, local_first)
        tl.atomic_max(LastNzCoarse + row_coarse, local_last)
    tl.atomic_add(NumNonemptyCoarseRow + row_coarse, num_nonempty_local)
    tl.atomic_add(GlobalCounters + 0, num_nonempty_local)
    tl.atomic_add(GlobalCounters + 1, num_partial_local)
    ...
    fine_ptrs = BlockMaskFine + fine_row[:, None] * nkb + col_start_vec[None, :]
...
def _fused_preproc_multigrain(mask, block_n):
    """v165: atomic-fused multigrain kernel — first_nz/last_nz +
    per-row num_nonempty + global counters emitted directly by the
    compute+pack kernel. Separate `_reduce_block_mask` is no longer
    needed on the default path; `_compute_gap_sum` finalizes the
    per-row gap_ratio average."""
    N = mask.shape[0]
    assert block_n == 64
    ...
    # Atomic targets — pre-initialized to sentinels.
    first_nz = torch.full((nqb_coarse,), nkb, dtype=torch.int32, device=mask.device)
    last_nz = torch.full((nqb_coarse,), -1, dtype=torch.int32, device=mask.device)
    num_nonempty_row = torch.zeros(nqb_coarse, dtype=torch.int32, device=mask.device)
    global_counters = torch.zeros(2, dtype=torch.int32, device=mask.device)
    ...
    _compute_and_pack_mask_multi[(nqb_coarse, nkb // bn_chunks)](
        ...
        first_nz, last_nz, num_nonempty_row, global_counters,
        mask.stride(0), mask.stride(1),
    ...
    # Normalize first_nz: rows with no nonempty blocks must have first_nz=0.
    first_nz = torch.where(num_nonempty_row > 0, first_nz, torch.zeros_like(first_nz))

    # Finalize gap_sum via tiny Triton kernel.
    gap_sum_out = torch.zeros(1, dtype=torch.float32, device=mask.device)
    nqb_pow2 = 1
    while nqb_pow2 < nqb_coarse:
        nqb_pow2 *= 2
    _compute_gap_sum[(1,)](
        first_nz, last_nz, num_nonempty_row,
        gap_sum_out,
        nqb_coarse,
        NQB_POW2=nqb_pow2,
    )

    metrics = torch.cat([global_counters.to(torch.float32), gap_sum_out])
    num_nonempty_total, num_partial_total, global_gap_sum = metrics.tolist()
    ...
    return {
        ...
        "first_nz_coarse": first_nz,
        "last_nz_coarse": last_nz,
