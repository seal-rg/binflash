"""v157: batch the 4 separate .item() calls in _fused_preproc into a single
CPU-GPU sync via torch.stack + .tolist().

Probe measurement showed _fused_preproc's total cost is ~973μs at N=16384 — a
significant fraction of kernel runtime on sparse/small patterns (longformer
kernel is 1669μs, so preproc is 58% of end-to-end time).

Current _fused_preproc code has 4 separate .item() calls:
...
Each .item() triggers a CPU-GPU synchronization (~50-200μs each on PCIe),
even though all four are independent reductions over the same (nqb, nkb)
block_mask tensor. We can stack them on-device and transfer in one shot.

Kernel code is UNCHANGED from v149 — only Python preprocessing differs.
"""
...
def _fused_preproc(mask, block_m, block_n, want_packed=False):
    ...
    # All three scalar metrics (gap_ratio, density, partial_fraction) are
    # computed on-device and concatenated into a single 3-element tensor,
    # which is transferred to CPU via one .tolist() call.
    num_nonempty = nz.to(torch.int32).sum(dim=1)  # (nqb,) int32
    scan_width = (last_nz - first_nz + 1).clamp(min=1)
    gap_per_row_mean = (1.0 - num_nonempty.float() / scan_width.float()).mean()  # scalar GPU
    density_tensor = nz.float().mean()  # scalar GPU
    num_nonempty_total_t = num_nonempty.sum().float()  # scalar GPU
    num_partial_total_t = (block_mask == 1).sum().float()  # scalar GPU
    # partial_fraction = num_partial / max(num_nonempty, 1)
    partial_fraction_tensor = num_partial_total_t / num_nonempty_total_t.clamp_min(1.0)

    # Single CPU-GPU sync: stack 3 scalars, transfer once
