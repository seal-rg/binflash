"""v173: within the v171 merged hier body, split the `if bm_val > 0`
nested branch into `if bm_val == 2 ... elif bm_val == 1`. The full-block
path loads only K, V and has no mask-related state; the partial-block
path loads K, V, mask. Hypothesis: explicit mutual-exclusion branches
let the compiler allocate registers separately for each case — the
full path never holds mask intermediates live.

Dead end #21 (v102) showed flat single-pass was slower than two-pass
flat. v171 revived the single-pass idea inside the hier branch only
because `tl.static_range(CF)` provides compile-time visibility. v173
tests whether an explicit elif split further helps the compiler
optimize each arm separately under static_range unroll.

Only the hier-branch body changes. Non-hier two-pass path unchanged.
The kernel functions are a local copy (not imported from v171), so a
fresh autotune cache is used — baseline comparison must tolerate
compilation variance between fresh caches.
"""
import math
import torch
import triton
        ...
        while coarse_col <= coarse_last:
            ...
            if coarse_val > 0:
                ...
                for sub in tl.static_range(COARSE_FACTOR_C):
                    ...
                    # v173: explicit mutual-exclusion split. Full branch has
                    # no mask state live; partial branch has its own.
                    if bm_val == 2:
...
def _binflash_fwd_v173(
    ...
    acc, l_i, m_i = _binflash_fwd_inner_v173(
        acc, l_i, m_i, q,
