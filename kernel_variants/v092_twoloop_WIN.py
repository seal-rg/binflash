"""v092: Two-loop structure — MAJOR IMPROVEMENT at all production scales.

Split inner loop into two passes:
  Pass 1: full blocks (bm_val==2) -- K/V only, no mask in loop body
  Pass 2: partial blocks (bm_val==1) -- K/V + mask

The compiler generates better code for pass 1 because the mask block_ptr
doesn't participate in loop-level staging. This enables better K/V
pipelining even with a single num_stages value.

Results vs v049 baseline:
  7B-train causal (N=8K): 8.091ms (was 10.1ms) -> 1.20x
  7B-train N=4K causal: 2.160ms -> 1.27x
  7B-batch8 N=2K causal: 2.523ms -> 1.20x
  70B-GQA N=8K causal: 1.005ms -> 1.39x (!!)
  D=64 N=8K causal: 2.123ms -> 1.60x (!!)

No regressions on any pattern or config.
"""
