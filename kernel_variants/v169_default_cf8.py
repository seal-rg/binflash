"""v169: extend v168's adaptive CF dispatch to the default path.

v168 applies CF=8 only on path 1 (D=64 gap-heavy, BM=64 BN=64).
D=128 longformer/cdw_sinks go through the default path (BM=128 BN=64)
because BM=64 is register-prohibitive at D=128. They currently use
CF=2 on the default path, missing out on the longformer-specific
wins v168 demonstrated at D=64.

This version mirrors v168's adaptive CF dispatch on the default path:
  - Default, gap_ratio > 0.7 (longformer-like): CF=8
  - Default, gap_ratio ∈ (0.3, 0.7] (cdw_sinks-like): CF=2 (unchanged)
  - Default, gap_ratio ≤ 0.3: CF=2, use_hier=0 (unchanged)

Only changes behavior for patterns with gap_ratio > 0.7 on the default
path, which in practice means D=128 longformer. cdw_sinks D=128 (gap
probably ~0.5-0.7) stays on CF=2 — we don't want to risk the small-N
regression we saw with CF=8-unconditional.

Imports unchanged helpers from production (v168) and redefines only
the dispatch + main kernel wrapper.
"""
import math
import torch
import triton
...
# ── begin inlined legacy _binflash_fwd_inner (v167, CF-parameterized) ──
@triton.jit
def _binflash_fwd_inner(
    ...
    return acc, l_i, m_i
# ── end inlined legacy _binflash_fwd_inner ──

# ── begin inlined legacy _binflash_fwd (v167, CF-parameterized) ──
@triton.autotune(
...
# ── end inlined legacy _binflash_fwd ──
...
# Reuse the production kernel directly — same signature, same behavior
...
def _dispatch_and_preprocess(mask: torch.Tensor, D: int):
    ...
    # Default path: v169 adds CF=8 for very-wide gaps
    use_packed = 1 if (
    ...
    # v169b: CF=4 for very wide gap (D=128 longformer). v169 initial
    # attempt with CF=8 regressed longformer by -17% due to register
    # spill at D=128 BM=128 (larger per-block fp32 acc state than
    # D=64 BM=64 path 1). CF=4 halves the inner unroll depth.
    if gap_ratio > _WIDE_GAP_THRESHOLD:
        cf = _CF_PATH1  # = 4
    else:
        cf = _CF_OTHER
    coarse = _make_coarse_lK(block_mask, int(math.log2(cf)))
...
def binflash_attention(q, k, v, mask, sm_scale=None, block_m=128, block_n=None, block_mask=None):
    ...
    if block_mask is None:
        ...
        raise NotImplementedError("external block_mask path not implemented in v169 test file")
