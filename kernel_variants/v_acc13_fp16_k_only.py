...
def _gathered_fwd_inner(
    ...
    for idx in tl.range(0, n_full, num_stages=3):
        ...
        v = tl.load(v_ptrs)
        qk = tl.dot(q, k.to(tl.float16)) * 1.44269504  # fwd_v_acc13: fp16 k only (q_scaled stays bf16)
    ...
    for idx in tl.range(n_full, n_total, num_stages=3):
        ...
        qk = tl.dot(q, k.to(tl.float16)) * 1.44269504  # fwd_v_acc13: fp16 k only (partial pass)
