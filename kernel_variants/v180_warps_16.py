"""v180: add num_warps=16 to v179's autotune space.

Dead end #24 (v106) tested stages=[1..6] × warps=[2,4,8,16] WITHOUT
maxnreg — warps=16 was never picked because the reg budget at 4096
threads (16 warps × 256) × 255 regs × 4 bytes = 4MB would spill
catastrophically.

With v177's maxnreg=192 limiting regs, warps=16 register budget at
maxnreg=192 is 16 × 32 × 192 × 4 = 393KB, comfortably fitting the
kernel's live state. Similar meta-lesson as v179: when a new knob
(maxnreg) is added, revisit related dead ends to see if they unlock.

Autotune space: 7 stages × 3 warps × 3 maxnreg = 63 configs
(vs v179's 42). 1.5x first-run autotune time.

Kernel body identical to v179 production.
"""
import math
import torch
import triton
...
for s in [1, 2, 3, 4, 5, 6, 7]:
    for w in [4, 8, 16]:  # v180: add warps=16
