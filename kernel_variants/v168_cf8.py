"""v168: CF=8 on path 1. Extends v167 which used CF=4 on the same
path. Math argument: total scan-loop overhead ≈ nkb/CF + non_empty*CF,
minimized at CF ≈ sqrt(nkb/non_empty). For longformer path 1 at
N=32768 (nkb=512, ~7 non-empty per row), CF* ≈ 8.5, so CF=8 should
outperform v167's CF=4.

File imports production helpers and redefines the main kernel and
dispatch with the wider unroll.
"""
import math
import torch
import triton
...
_CF_PATH1_WIDE = 8  # for gap_ratio > 0.7 (longformer-like)
_CF_PATH1 = 4       # for gap_ratio in (0.3, 0.7] (cdw_sinks-like)
_CF_OTHER = 2
_WIDE_GAP_THRESHOLD = 0.7


@triton.jit
def _binflash_fwd_inner_v168(
    acc, l_i, m_i, q,
...
def _binflash_fwd_v168(
    ...
    acc, l_i, m_i = _binflash_fwd_inner_v168(
        acc, l_i, m_i, q,
...
def _dispatch_and_preprocess(mask: torch.Tensor, D: int):
    ...
    if D == 64 and gap_ratio > _GAP_RATIO_THRESHOLD and N % 64 == 0:
        ...
        # v169 adaptive CF: very-wide gaps (longformer, gap>0.7) get CF=8,
        # moderate gaps (cdw_sinks, gap<=0.7) keep CF=4.
...
def binflash_attention(q, k, v, mask, sm_scale=None, block_m=128, block_n=None, block_mask=None):
    ...
    if block_mask is None:
        ...
        raise NotImplementedError("external block_mask path not implemented in v168 test file")
