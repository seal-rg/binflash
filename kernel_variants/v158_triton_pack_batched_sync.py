"""v158: combined preprocessing optimizations — Triton-based _pack_mask
(from v150) + batched CPU-GPU sync (from v157).

Dead end #61 (v150) recorded the Triton _pack_mask as "code quality
improvement, not a measurable speedup" because benchmark.py's kernel timing
caches preprocessing. However, direct measurement of _fused_preproc(want_packed=True)
shows v150's Triton pack is 3.3x faster than v149's PyTorch version
(~6.7ms → ~2.0ms at N=16384, D=any). This is a real ~5ms savings per unique
mask (cache miss path). Benchmark.py doesn't surface it because:
  (a) kernel timing caches preproc, so repeated calls pay no preproc cost
  (b) the `binflash_preproc_ms` column uses a simplified stub function that
      doesn't exercise _pack_mask at all

The improvement is real for production workloads where mask changes per
batch/layer. Kernel code is UNCHANGED from v149, so no kernel regression risk.

Combined with v157's batched .item() → .tolist() sync (marginal ~1% on
unpacked path, zero on packed path since packing dominates that case).
"""
...
def _pack_mask_kernel(
    ...
    Each program processes BLOCK_M rows and BLOCK_N_WORDS packed words
    (= 32*BLOCK_N_WORDS actual columns).
    """
...
def _pack_mask(mask: torch.Tensor) -> torch.Tensor:
    ...
    bit k of packed[i, j] == mask[i, j*32+k]. Avoids the 1GB int32
    intermediate that the PyTorch version allocated.
    """
    N = mask.shape[0]
...
def _fused_preproc(mask, block_m, block_n, want_packed=False):
    ...
    # v157: batch the 3 scalar metrics into a single CPU-GPU sync.
    num_nonempty = nz.to(torch.int32).sum(dim=1)
    scan_width = (last_nz - first_nz + 1).clamp(min=1)
    gap_per_row_mean = (1.0 - num_nonempty.float() / scan_width.float()).mean()
    density_tensor = nz.float().mean()
    num_nonempty_total_t = num_nonempty.sum().float()
    num_partial_total_t = (block_mask == 1).sum().float()
    partial_fraction_tensor = num_partial_total_t / num_nonempty_total_t.clamp_min(1.0)

    metrics = torch.stack([gap_per_row_mean, density_tensor, partial_fraction_tensor])
    avg_gap_ratio, density, partial_fraction = metrics.tolist()
...
# ─── Main attention kernel (UNCHANGED from v149) ───
