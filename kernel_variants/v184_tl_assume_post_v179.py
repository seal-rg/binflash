"""v184: retest tl.assume hints on loop bounds, post-v179.

Dead end #40 (v125) tested tl.assume hints on first_nz/last_nz pre-
hier (before v166+). Got neutral result. With current hier structure
(v173 elif + v177 maxnreg + v179 stages[1..7]), the compiler's
handling of the hier scan may differ and the hints might now help.

Adds at kernel entry:
  tl.assume(first_nz >= 0)
  tl.assume(last_nz < N_CTX // BLOCK_N)

Zero-risk probe. Kernel body otherwise byte-identical to v179.
"""
import math
import torch
import triton
...
def _binflash_fwd_inner_v184(
    ...
    # v184: tl.assume hints on loop bounds
    tl.assume(first_nz_col >= 0)
    tl.assume(last_nz_col < N_CTX // BLOCK_N)
    ...
    if USE_HIER:
        ...
        tl.assume(coarse_first >= 0)
        tl.assume(coarse_last < N_CTX // (BLOCK_N * COARSE_FACTOR_C))
...
def _binflash_fwd_v184(
    ...
    acc, l_i, m_i = _binflash_fwd_inner_v184(
        acc, l_i, m_i, q,
