"""v21 deep_stages — v4 + expand bwd autotune num_stages from [1..3] to [1..7].

The fwd (binflash_attention.py) autotunes num_stages ∈ [1..7] × maxnreg ∈
{None, 168, 192} (42 configs), and memory v179 documents real deep-stages wins
on several shapes. The current bwd is stuck at [1..3] based on a speculative
'won't fit in SMEM' claim. Triton auto-skips configs that OOM in SMEM, so
adding stages 4-7 costs nothing when they don't fit but opens potential wins
at small tile shapes (e.g. BLOCK_M=BLOCK_N=64, D=64 shapes, and also D=128
with maxnreg=192 freeing registers). No kernel body change."""
...
for _s in [1, 2, 3, 4, 5, 6, 7]:   # v21: match fwd's stages range; Triton skips OOM configs
    for _w in [4, 8]:
        for _mnr in [None, 168, 192]:
            _kw = {}
            if _mnr is not None:
