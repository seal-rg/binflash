"""v087: Grid transpose (B*H, N/BM) — MIXED results.

Swap program_id(0) and program_id(1) so batch-heads are scheduled
consecutively, improving L2 cache reuse for K/V data.

Results at production scale:
  7B-train causal: 13.0ms (was 9.5ms) -> 0.77x REGRESSION (30% slower)
  7B-train sparse: 1.05-1.20x improvement
  70B-GQA causal: 1.17x improvement
  70B-GQA sparse: 2.06-2.38x improvement
  D=64 all: 1.29-2.00x improvement

Root cause of causal regression: LOAD IMBALANCE.
Causal Q-blocks have varying work (Q-block 0: 2 KV blocks, Q-block 63: 128).
With transposed grid, light and heavy Q-blocks are in the same wave ->
heavy tail latency. With original grid, similar Q-blocks are grouped ->
better wave balance.

Conclusion: beneficial for sparse/GQA but catastrophic for dense causal at
large B*H (the dominant production workload). REVERTED.

This file contains the baseline kernel (same as v087_before_grid_transpose),
not the experimental transposed-grid variant which was reverted.
"""
