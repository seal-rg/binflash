"""v091: Raw mask pointers instead of block_ptr — NEUTRAL/WORSE.

Hypothesis: removing mask from block_ptr would let Triton double-buffer
only K/V at D=128 (64KB <= 100KB smem), enabling stages>1 pipelining.

Reality: autotuner still picks stages=1 at D=128. Raw pointer address
computation adds slight overhead at D=64. No pipelining improvement.

7B-train causal: 8.713ms (was 8.780ms) -- within noise
D=64 sparse: marginally worse (raw ptrs slower than block_ptr)

REVERTED -- block_ptr is cleaner and equal/faster.

This file contains the baseline kernel (same as v091_before_raw_mask_ptr),
not the experimental raw-pointer variant which was transient and reverted.
"""
