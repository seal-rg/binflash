"""Binary Block Masked Flash Attention — v140: lower packed-mask threshold to 0.5.

v132 (current production) uses `_DENSITY_THRESHOLD_PACKED = 0.8`, which
routes sparse_10pct/30pct (density ~1.0) through the packed-mask path
but keeps causal/prefix_lm (density ~0.5) on the regular mask path.

v131 (unconditional packing) regressed causal D=128 by -5% because the
pack code path was always active — with just 1 partial block per causal
row, the unpack overhead dominated the meager bandwidth savings.

Hypothesis: v132's constexpr-if architecture (`USE_PACKED: tl.constexpr`)
makes the packed and unpacked kernel specializations compile separately.
The register allocation for the packed specialization might differ from
v131's single-path kernel. Maybe causal routed through v132's packed
specialization doesn't have the same regression.

v140 tests this by lowering the threshold from 0.8 → 0.5. Patterns that
now dispatch to packed:
  - causal (density ~0.5) → packed
  - prefix_lm (density ~0.5) → packed
  - sparse_10pct/30pct → still packed (density ~1.0)
  - longformer → still regular (goes through BM=64 path first)
  - cdw_sinks, log_tree → still regular (density too low)

This is purely a Python-side dispatch threshold change. Kernel body is
identical to v132.
"""
...
_DENSITY_THRESHOLD_PACKED = 0.5  # LOWERED from 0.8 — tests whether causal/prefix_lm benefit from packed path
...
def binflash_attention(
    ...
    """v132 with lowered packed-mask dispatch threshold (0.8 → 0.5)."""
    ...
    if block_mask is None:
        ...
        raise NotImplementedError("External block_mask not supported in v140")
