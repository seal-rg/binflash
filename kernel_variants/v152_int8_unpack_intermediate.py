"""Binary Block Masked Flash Attention — v152: int8 unpack intermediate.

v151 attempted BM=64 BN=128 for gap-heavy and catastrophically regressed
cdw_sinks by -36%. Root cause analysis identified the unpack
intermediate as the register-pressure culprit: (BM, BN/32, 32) **int32**
values = 4 bytes each.

v152 casts the intermediate to **int8** immediately after the shift,
reducing storage 4x. Since each value is always 0 or 1, int8 is
sufficient.

  Old: bits_3d = ((packed[:,:,None] >> bit_offs[None,None,:]) & 1)  # int32
  New: bits_3d = ((packed[:,:,None] >> bit_offs[None,None,:]) & 1).to(tl.int8)

Applied at v149's existing BM=128 BN=64 packed path (the dominant one
for sparse_10pct, sparse_30pct, longformer D=128, etc.). If the int8
reduction helps, the register headroom could enable slightly better
pipelining or, at minimum, be neutral with cleaner code.

Kernel-body-only change, no dispatch change. Should be bit-identical
output (the subsequent reshape and `!= 0` still produce the same bool).
"""
    ...
    while col_idx <= last_nz_col:
        ...
        if bm_val == 1:
            ...
            if USE_PACKED:
                ...
                # v152: cast to int8 immediately to halve unpack intermediate register pressure
                bits_3d = ((packed[:, :, None] >> bit_offs[None, None, :]) & 1).to(tl.int8)
...
def binflash_attention(
    ...
    """v149 with int8 unpack intermediate for reduced register pressure."""
    ...
    if block_mask is None:
        ...
        raise NotImplementedError("External block_mask not supported in v152")
