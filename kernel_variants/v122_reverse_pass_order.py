"""Binary Block Masked Flash Attention — v122: reverse pass order.

Try running pass 2 (partial blocks) BEFORE pass 1 (full blocks).
Online softmax is order-independent mathematically, so the result is the
same. But compile-time, the compiler sees a different code structure and
may optimize differently.

Low risk, easy experiment.
"""
...
def _binflash_fwd_inner(
    ...
    # ── Pass 2 (partial blocks) FIRST — order swapped ──
...
def binflash_attention(
    ...
    """v114 + reverse pass order (pass 2 first, then pass 1)."""
