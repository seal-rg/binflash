...
# v52: EXPAND configs — add lower maxnreg ({128, 144}) and num_warps=2/16 to give the
# autotuner more options to escape the "always num_stages=1" limit on dKdV.

_bwd_autotune_configs = []
for _s in [1, 2, 3]:           # D=128 bwd won't fit >3 stages in SMEM
    for _w in [2, 4, 8]:        # v52: add num_warps=2
        for _mnr in [None, 128, 144, 168, 192]:  # v52: add maxnreg=128, 144
            _kw = {}
            if _mnr is not None:
                _kw["maxnreg"] = _mnr
