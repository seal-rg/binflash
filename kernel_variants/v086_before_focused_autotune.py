"""v086 baseline — 20-config autotune (stages=[1-5], warps=[2,4,8,16]) + cache.

The broad autotune was shown by compare_all_versions at production scale to
REGRESS D=128 causal by ~1ms (10.43ms vs 9.49ms for v006's 6-config search).
warps=2 and warps=16 are never optimal and just add noise.
"""
