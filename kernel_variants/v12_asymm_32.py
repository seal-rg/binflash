"""v12 asymm_32 — v4 with num_stages=3 full, num_stages=2 partial.

Middle ground between v4 (3, 3) and v9 (4, 2). Tests whether the partial-loop
reduction alone accounts for v9's win, separate from the full-loop increase.
...
def _bwd_dkdv_inner(
    ...
    # Pass 2: partial Q-blocks — packed or bool mask. v12: num_stages=2.
...
def _bwd_dq_inner(
    ...
    # Pass 2: partial K-blocks — packed or bool mask. v12: num_stages=2.
    for idx in tl.range(n_full, n_total, num_stages=2):
        col_idx = tl.load(ci_base + idx)
        start_n = col_idx * BLOCK_N
        k_ptrs = K_base + offs_d[:, None] * stride_kk + (start_n + offs_bn)[None, :] * stride_kn
