"""v7 block_n_32 — v4 + BLOCK_N=32 on both bwd kernels.

Rationale: v4 is stuck at num_stages=1 on D=128 because SMEM is tight. Halving
BLOCK_N (64→32) halves the per-iter p/ds working tensors (32KB → 16KB each),
freeing up SMEM for num_stages>=2 pipelining. Finer (N/64, N/32) block mask
also lets more blocks fall into the all-full or all-masked categories (easier
to skip fully)."""
...
_BWD_BLOCK_M = 64
_BWD_BLOCK_N = 32   # v7: halved to free SMEM for num_stages>=2
