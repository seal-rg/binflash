"""V197: V193 + cleanup. Remove unused first_nz/last_nz HBM stores.

V193's kernel doesn't use first_nz/last_nz — they were used by the old
scan-based kernel for loop bounds. The `_reduce_block_mask` kernel still
computes them internally (needed for gap_for_row) but stores to HBM are
wasted. Also skip the per-row tensor allocations.

Marginal savings (few KB HBM writes per call) but cleaner code.
"""
...
def _reduce_block_mask(
    ...
    """V197: removed FirstNz/LastNz output. V193's kernel doesn't use them."""
    row = tl.program_id(0)
