"""Binary Block Masked Flash Attention — v150: Triton _pack_mask kernel.

The current `_pack_mask` uses PyTorch ops:
    mask_reshaped = mask.view(N, N // 32, 32).to(torch.int32)
    packed = (mask_reshaped * weights).sum(dim=-1, dtype=torch.int32)

This allocates a 4*N² byte intermediate (1GB at N=16384) — wasteful.
A native Triton kernel computes the packed output directly from the
bool mask without intermediate allocation.

Not expected to show in the benchmark (preproc is cached in the main
kernel's warm-timed loop), but:
  1. Cleaner implementation, no 1GB temporary
  2. Faster first-call (which IS part of warmup, though not the timed portion)
  3. Removes a potential OOM trigger on memory-constrained systems
  4. Still a correctness-preserving change

Measured honestly even if neutral.
"""
...
def _pack_mask_kernel(
    ...
    BLOCK_N_WORDS: tl.constexpr,  # how many 32-bit packed words per program
):
    """Pack bool mask (N, N) into int32 (N, N//32).
    Each program processes BLOCK_M rows and BLOCK_N_WORDS packed words (= 32*BLOCK_N_WORDS actual columns).
    """
    ...
    offs_pw = packed_col_start + tl.arange(0, BLOCK_N_WORDS)  # (BLOCK_N_WORDS,)
    bit_offs = tl.arange(0, 32)  # (32,)

    # Compute actual mask column indices: offs_pw[j] * 32 + bit_offs[k]
    # Shape: (BLOCK_N_WORDS, 32)
    actual_cols = offs_pw[:, None] * 32 + bit_offs[None, :]  # (BLOCK_N_WORDS, 32)

    # Load mask tile: shape (BLOCK_M, BLOCK_N_WORDS * 32)
    # We use a 3D load effectively by indexing
    mask_ptrs = (
        Mask
        + offs_m[:, None, None] * stride_mask_m
        + actual_cols[None, :, :] * stride_mask_n
    )  # (BLOCK_M, BLOCK_N_WORDS, 32)
    mask_tile = tl.load(mask_ptrs)  # int8 values 0 or 1
    mask_i32 = mask_tile.to(tl.int32)

    # Pack: for each (row, word), sum(bit_k * 2^k)
    weights = 1 << bit_offs  # (32,)
    # weighted_bits: (BLOCK_M, BLOCK_N_WORDS, 32)
    weighted = mask_i32 * weights[None, None, :]
    # Sum over the 32 bits → (BLOCK_M, BLOCK_N_WORDS)
    packed = tl.sum(weighted, axis=2)

    # Store to Packed[row, packed_col]
    packed_ptrs = (
        Packed
...
def _pack_mask(mask: torch.Tensor) -> torch.Tensor:
    """Pack bool (N, N) into int32 (N, N//32) via a Triton kernel.

    bit k of packed[i, j] == mask[i, j*32+k]. No CPU-side intermediates.
    """
    N = mask.shape[0]
    ...
    # Pick block sizes that divide the problem
    BLOCK_M = min(64, N)
    BLOCK_N_WORDS = min(8, N // 32)  # 8 words = 256 cols per program
...
def _fused_preproc(mask, block_m, block_n, want_packed=False):
    ...
    num_partial_total = (block_mask == 1).sum().item()
    num_nonempty_total = int(num_nonempty.sum().item())
...
def binflash_attention(
    ...
    """v149 + Triton-based _pack_mask (cleanup — no 1GB intermediate)."""
    ...
    if block_mask is None:
        ...
        raise NotImplementedError("External block_mask not supported in v150")
