"""v178: probe finer maxnreg values around v177's 192 sweet spot.

v177 found causal-class patterns pick maxnreg=192 over None. The
optimum could be sharper — values in [192, 255] unexplored. Add
maxnreg values 200, 208, 216 alongside existing None/168/192.

If any shape prefers 200/208/216 over 192, there's further gain.
Zero-risk probe: just expands autotune configs.

Kernel body identical to v177 production (uses _binflash_fwd_inner).
"""
import math
import torch
import triton
...
for s in [1, 2, 3, 4, 5]:
    for w in [4, 8]:
        for mnr in [None, 168, 192, 200, 208, 216]:
            kwargs = {}
            if mnr is not None:
                kwargs["maxnreg"] = mnr
