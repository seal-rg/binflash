"""v3 packed_mask — v2 + use the 32-bit packed mask for partial blocks in the
dQ and dK/dV kernels. Saves 32x bandwidth vs loading the full (M, N) bool mask.
The fwd already builds the packed mask; we reuse its (N/32, N).T layout."""
...
def _bwd_dkdv_kernel(
    ...
    DO, DK, DV,
    Mask,
    MaskPacked,        # v3: (N, N/32) int32 bit-packed mask for partial-block path
    LSE, Delta,
    ...
    stride_mp_m, stride_mp_n,  # v3: packed mask strides
    stride_ri_row,
    Z, H, N_CTX,
    BLOCK_M: tl.constexpr,
    BLOCK_N: tl.constexpr,
    ...
    # Pass 2: partial Q-blocks — packed (v3) or bool mask fallback.
    for idx in range(n_full, n_total):
        ...
        qk_scaled = qk * (sm_scale * 1.44269504)
        if USE_PACKED:
            mp_ptrs = MaskPacked + m_cur[:, None] * stride_mp_m + (start_n_packed + offs_bn_packed)[None, :] * stride_mp_n
            packed = tl.load(mp_ptrs)
            bits_3d = (packed[:, :, None] >> bit_offs[None, None, :]) & 1
            mask_ = tl.reshape(bits_3d, (BLOCK_M, BLOCK_N)) != 0
        else:
...
def _bwd_dq_kernel(
    ...
    # Pass 2: partial blocks — packed (v3) or bool mask fallback.
    for idx in range(n_full, n_total):
        col_idx = tl.load(ci_base + idx)
        start_n = col_idx * BLOCK_N
        n_cur = start_n + offs_n
        ...
        if USE_PACKED:
            start_n_packed = (col_idx * BLOCK_N) // 32
            mp_ptrs = MaskPacked + offs_m[:, None] * stride_mp_m + (start_n_packed + offs_bn_packed)[None, :] * stride_mp_n
            packed = tl.load(mp_ptrs)
            bits_3d = (packed[:, :, None] >> bit_offs[None, None, :]) & 1
            mask_ = tl.reshape(bits_3d, (BLOCK_M, BLOCK_N)) != 0
        else:
...
def _binflash_bwd(do, q, k, v, o, lse, mask, sm_scale):
    ...
    # v3: build (N, N/32) bit-packed mask for partial-block path.
    # Mirrors the fwd's transposed .T layout for coalesced warp-wide reads across BLOCK_M rows.
    assert N % 32 == 0
    mask_packed = torch.empty((N // 32, N), dtype=torch.int32, device=mask.device).T
    _mask_i32 = mask.view(N, N // 32, 32).to(torch.int32)
    _bit_weights = (1 << torch.arange(32, device=mask.device, dtype=torch.int32))
    torch.sum(_mask_i32 * _bit_weights, dim=-1, out=mask_packed)
    USE_PACKED = 1
