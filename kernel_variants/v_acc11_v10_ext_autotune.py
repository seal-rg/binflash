...
def _gathered_fwd_inner(
    ...
    for idx in tl.range(0, n_full, num_stages=3):
        ...
        v = tl.load(v_ptrs)
        qk = tl.dot(q, k.to(tl.float16)) * 1.44269504  # fwd_v_acc10: fp16 k for matmul
    ...
    for idx in tl.range(n_full, n_total, num_stages=3):
        ...
        qk = tl.dot(q, k.to(tl.float16)) * 1.44269504  # fwd_v_acc10: fp16 k for matmul (partial pass)
...
_autotune_configs = []
# v_acc11: extended autotune for fp16-cast variant — more nw and maxnreg
...
def _gathered_fwd(
    ...
    q = (q * sm_scale).to(tl.float16)  # fwd_v_acc10: fp16 q_scaled (wider mantissa than bf16)
