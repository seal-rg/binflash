"""v186: remove the dead `tl.where(l_i == 0.0, 1.0, l_i)` safety check.

`l_i` is initialized to 1.0 at kernel entry:
    l_i = tl.zeros([BLOCK_M], dtype=tl.float32) + 1.0

In the online softmax inner body:
    l_i = l_i * alpha + l_ij
with alpha = exp2(m_i - m_ij) ∈ (0, 1] (since m_ij >= m_i always) and
l_ij = sum(p) >= 0. So l_i is monotonically non-decreasing and stays
≥ 1.0 throughout.

The only path to l_i == 0 would require the loop body to run on a
row with all-zero mask entries, producing p = exp2(-inf) = 0 → l_ij
= 0 → l_i stays at initial 1.0. Even then, l_i = 1.0, not 0.

Conclusion: `tl.where(l_i == 0.0, 1.0, l_i)` always selects l_i.
It's dead code. Remove it.

Savings per program: 1 compare + 1 select on BM floats = ~100 cycles.
Negligible on its own but applied across many programs could be
detectable against the newly calibrated noise floor (sparse/longformer
CV < 1%).

Kernel body otherwise byte-identical to v185 production.
"""
import math
import torch
import triton
...
def _binflash_fwd_v186(
    ...
    # v186: removed dead `tl.where(l_i == 0.0, 1.0, l_i)` — l_i is
    # initialized to 1.0 and monotonically non-decreasing, so the zero
