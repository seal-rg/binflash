"""Binary Block Masked Flash Attention — v132: packed mask with density dispatch.

v131 proved that int32 bit-packed mask gives +26-42% on dense-block
patterns (sparse_10pct/30pct at D=64 and D=128 large N) but regresses
longformer D=128 (-10.8%) and causal D=128 (-5.4%) because the unpack
overhead dominates when there are few partial blocks per row.

Fix: add an opt-in path. The kernel body has a `USE_PACKED: tl.constexpr`
flag. When True, pass 2 loads packed mask and unpacks; when False, loads
regular int8 mask (v114 behavior). Triton's constexpr-if compiles only
the selected branch.

Dispatch rule: USE_PACKED=1 iff block-density > 0.8. This captures:
  - sparse_10pct / sparse_30pct at any D, any N (density ≈ 1.0) → WIN
  - causal (density ≈ 0.5) → REGULAR (no regression)
  - longformer / cdw_sinks / log_tree (low density) → REGULAR
  - prefix_lm (density ~0.5) → REGULAR

The packed-mask variant is used only on patterns with long dense scans
where mask bandwidth actually matters. Others keep v114 behavior.
"""
...
def _binflash_fwd_inner(
    ...
    # ── Pass 2: partial blocks (bm_val==1) — K/V + mask (regular OR packed) ──
    col_idx = first_nz_col
    while col_idx <= last_nz_col:
        bm_val = tl.load(block_mask_ptr + start_m * bm_stride_row + col_idx * bm_stride_col)
        if bm_val == 1:
...
def binflash_attention(
    ...
    """v114 + density-dispatched int32 bit-packed mask for pass 2."""
    ...
    if block_mask is None:
        ...
        if key == _bm_cache_key:
            ...
            # Initial preproc at BM=128 BN=64 to get metrics; only pack if we'll need it
            bm128_bn64, fn, ln, gap_ratio, density, _ = _fused_preproc(mask, 128, 64, want_packed=False)
            # Decide dispatch path and whether packed
            if D == 64 and gap_ratio > _GAP_RATIO_THRESHOLD and N % 64 == 0:
                # BM=64 path (longformer-like gap pattern)
                ...
                # Default BM=128 BN=64 path
                block_mask, first_nz, last_nz = bm128_bn64, fn, ln
                # Use packed only if density > threshold (captures D=128 sparse patterns)
        ...
        raise NotImplementedError("External block_mask not supported in v132")
    ...
    # For the regular path, mask_packed may be None — pass a dummy zero-stride tensor
    if mask_packed is None:
        # Pass the original mask as a placeholder; it won't be read due to USE_PACKED=0
        mp_tensor = mask
