"""v189: diagnostic — isolate whether the LSE store's performance
benefit comes from the HBM store itself or from the log2+add compute.

v188 reverted v185/v186 and confirmed v179's epilogue (LSE store +
tl.where) is ~5-10% faster than the "clean" version on sparse/log_tree.
The mechanism is unclear. v189 tests the hypothesis:

- If v189 (store l_i directly, no log2) ≈ v188: it's the HBM store
  that acts as scheduling anchor. Compute doesn't matter.
- If v189 regresses toward v186 (17+ms on sparse_30pct N=16384):
  the log2 compute itself matters (creates dependency chain that
  fills a pipeline stall).

Kernel body otherwise byte-identical to v188 production.
"""
import math
import torch
import triton
...
def _binflash_fwd_v189(
    ...
    acc = acc / l_i[:, None]
    offs_m = start_m * BLOCK_M + tl.arange(0, BLOCK_M)
    # v189: store l_i directly, no log2+m_i compute. Tests if the HBM
    # store itself is the scheduling anchor (vs the compute).
    tl.store(LSE + (off_hz * N_CTX) + offs_m, l_i)
    tl.store(O_bp, acc.to(Out.type.element_ty))
