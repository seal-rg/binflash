"""Binary Block Masked Flash Attention — v119: manual hoist of loop-invariants.

The inner while loop has loop-invariant expressions:
  K_base + offs_d[:, None] * stride_kk   — doesn't depend on col_idx
  V_base + offs_d[None, :] * stride_vn   — doesn't depend on col_idx
  Mask_base + offs_bm[:, None] * stride_mask_m   — pass 2 only

While loops in Triton have less aggressive optimization than for loops.
Manually hoisting these invariant expressions outside the loop body might
give the compiler a clearer view.
"""
...
def _binflash_fwd_inner(
    ...
    # Hoisted loop invariants: row base pointers for K, V, Mask
    # K_row_base[d, j] = K_base + d * stride_kk
    # Each iteration adds col-dependent offset to this.
    K_row_base = K_base + offs_d[:, None] * stride_kk  # shape (HEAD_DIM, 1)
    V_col_base = V_base + offs_d[None, :] * stride_vn  # shape (1, HEAD_DIM)
    Mask_row_base = Mask_base + offs_bm[:, None] * stride_mask_m  # shape (BLOCK_M, 1)
    # Pre-computed block mask row pointer
...
def binflash_attention(
    ...
    """v114 + manual loop-invariant hoisting in inner kernel."""
