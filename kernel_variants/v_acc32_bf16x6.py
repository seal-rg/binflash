...
def _bwd_dkdv_inner(
    ...
    for idx in tl.range(0, n_full):
        ...
        dv = tl.dot(tl.trans(p), do.to(tl.float32), dv, input_precision='bf16x6')  # v_acc23: 3xTF32 emulation

        dp = tl.dot(do, tl.trans(v)).to(tl.float32)
        ds = p * (dp - d_i[:, None])
        dk = tl.dot(tl.trans(ds), q.to(tl.float32), dk, input_precision='bf16x6')  # v_acc23: 3xTF32 emulation
