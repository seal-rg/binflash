"""v183: eliminate the separate coarse_mask tensor. Compute coarse_val
on-the-fly inside the kernel by loading CF consecutive block_mask
cells and taking tl.max.

Rationale: coarse_mask is derived from block_mask via pair-OR
reduction. We can compute it in-place inside the kernel, saving the
preproc `_make_coarse_lK` op (~5-20μs) and the coarse_mask tensor
allocation. The block_mask values we load for the coarse check are
the SAME ones the inner static_range loop iterates, so they hit L1
on the second load (or the compiler fuses).

Only the hier branch body changes. Non-hier two-pass unchanged.
Dispatch still passes coarse_mask but the kernel ignores it (coarse
pointer args kept for API compatibility).
"""
import math
import torch
import triton
...
def _binflash_fwd_inner_v183(
    ...
    cf_offs = tl.arange(0, COARSE_FACTOR_C)
    ...
    if USE_HIER:
        ...
        while coarse_col <= coarse_last:
            # v183: compute coarse_val via CF-wide block_mask load + max reduction
            base_col = COARSE_FACTOR_C * coarse_col
            fine_slice = tl.load(
                block_mask_ptr + start_m * bm_stride_row + (base_col + cf_offs) * bm_stride_col
            )
            coarse_val = tl.max(fine_slice)
            if coarse_val > 0:
...
def _binflash_fwd_v183(
    ...
    acc, l_i, m_i = _binflash_fwd_inner_v183(
        acc, l_i, m_i, q,
...
def _dispatch_and_preprocess(mask: torch.Tensor, D: int):
    ...
    if D == 64 and gap_ratio > _GAP_RATIO_THRESHOLD and N % 64 == 0:
        ...
        # v183: no coarse_mask — kernel derives coarse_val from block_mask.
        return block_mask, first_nz, last_nz, mask_packed, 64, 64, 1, use_hier, cf

    if D == 64 and density > _DENSITY_THRESHOLD_BN128 and N % 128 == 0:
        block_mask = out["block_mask_bn128"]
        first_nz, last_nz = _reduce_for_fine(block_mask)
        cf = _CF_OTHER
        return block_mask, first_nz, last_nz, mask_packed, 128, 128, 1, 0, cf
    ...
    cf = _CF_OTHER
    return (block_mask, fnz, lnz, mask_packed, 128, 64, use_packed, use_hier, cf)
    ...
    if block_mask is None:
        ...
        if key == _bm_cache_key:
            (block_mask, first_nz, last_nz, mask_packed, chosen_bm, chosen_bn,
             use_packed, use_hier, cf) = _bm_cache_val
        else:
            cache_val = _dispatch_and_preprocess(mask, D)
            _bm_cache_key = key
            _bm_cache_val = cache_val
            (block_mask, first_nz, last_nz, mask_packed, chosen_bm, chosen_bn,
             use_packed, use_hier, cf) = cache_val
    else:
