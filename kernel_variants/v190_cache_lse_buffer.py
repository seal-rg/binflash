"""v190: cache the LSE buffer across calls to reduce Python dispatch
overhead on fast kernels.

Current `binflash_attention()` allocates `torch.empty(B, H, N, fp32)`
on every call. For large N this is a 2MB+ allocation — cheap but
non-zero due to PyTorch allocator bookkeeping (~5-10μs).

For fast kernels (block_diagonal N=4096 ~94μs, cdw_sinks N=4096
~104μs, sliding_window N=4096 ~116μs, longformer N=4096 ~178μs),
5-10μs of Python overhead is 5-10% of total kernel_ms. These are
on stable patterns (CV < 0.5%) so a real 5% gain is detectable.

This is not a kernel change — just Python-side dispatch
optimization. Same kernel body as v188 production.
"""
import math
import torch
import triton
...
_bm_cache_key = None
_bm_cache_val = None
# v190: module-level LSE buffer cache
_lse_buffer = None
_lse_buffer_size = 0
...
def binflash_attention(q, k, v, mask, sm_scale=None, block_m=128, block_n=None, block_mask=None):
    global _bm_cache_key, _bm_cache_val, _lse_buffer, _lse_buffer_size
    B, H, N, D = q.shape
    if block_n is None:
        block_n = min(D, 64)
    if sm_scale is None:
    ...
    # v190: reuse LSE buffer if big enough
    need = B * H * N
    if _lse_buffer is None or _lse_buffer_size < need or _lse_buffer.device != q.device:
        _lse_buffer = torch.empty(need, device=q.device, dtype=torch.float32)
        _lse_buffer_size = need
    lse = _lse_buffer[:need].view(B, H, N)
    grid = (triton.cdiv(N, chosen_bm), B * H)
