...
def _detect_mask_mode(mask: torch.Tensor, N: int) -> int:
    ...
    Only checks the block mask structure, not every element — O(block_rows * block_cols).
    """
    # Check if mask is all-True (full attention)
    if mask.all():
        return 2
    # Check if mask is lower-triangular (causal) by checking corners
    # A causal mask has: mask[0, N-1] = False, mask[N-1, 0] = True
    if N > 1 and not mask[0, N - 1].item() and mask[N - 1, 0].item():
        # Quick check: is it exactly causal? Sample a few points.
        # mask[i, j] should be True iff i >= j
        checks = min(N, 16)
        step = max(1, N // checks)
...
def binflash_attention(
    ...
    if mask_mode is None:
        mask_mode = _detect_mask_mode(mask, N)
