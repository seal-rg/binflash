"""Binary Block Masked Flash Attention — v151: BM=64 BN=128 for gap-heavy D=64.

Current v149/v150 production uses BM=64 BN=64 for the gap-heavy D=64
path (longformer, cdw_sinks). Hypothesis: BN=128 may give better
compute efficiency per matmul (larger inner-K dimension → more tensor
core parallelism) while halving the number of K-block iterations per
program.

For longformer at D=64:
  - Current scan: ~4 BN=64 blocks per row (window=128 → 2 on each side)
  - With BN=128: ~2 blocks per row
  - Fewer iterations means less loop overhead amortization across
    smaller work

Shmem per stage (BM=64 BN=128 D=64):
  - K (D, BN) = 64*128*2 = 16KB
  - V (BN, D) = 128*64*2 = 16KB
  - Mask packed = 64*4*4 = 1KB
  - ~33KB/stage → stages=2 fits in 100KB; stages=3 = 99KB borderline

Register usage:
  - Q (BM, D) = 8KB
  - acc (BM, D) F32 = 16KB
  - Unpack intermediate (BM, BN/32, 32) int32 = 32KB (doubled vs BN=64)
  - Total ~56KB — fits at 8 warps with ~224B/thread

Risk: the unpack intermediate is larger, may cause register pressure
that prevents deeper pipelining. Worth measuring.

If it wins, promote. If it loses or is neutral, the BN=64 choice was
correct.
"""
...
def binflash_attention(
    ...
    """v149 with BM=64 BN=128 for gap-heavy D=64 path (was BM=64 BN=64)."""
    ...
    if block_mask is None:
        ...
        if key == _bm_cache_key:
            ...
            # NEW in v151: BM=64 BN=128 for gap-heavy (was BM=64 BN=64).
            # Requires N divisible by BOTH 64 (BM) and 128 (BN) — still all benchmark shapes.
            if D == 64 and gap_ratio > _GAP_RATIO_THRESHOLD and N % 128 == 0:
                block_mask, first_nz, last_nz, _, _, _, mask_packed = _fused_preproc(
                    mask, 64, 128, want_packed=True,
                )
                chosen_bm, chosen_bn = 64, 128
                use_packed = 1
        ...
        raise NotImplementedError("External block_mask not supported in v151")
