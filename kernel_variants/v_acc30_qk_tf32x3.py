...
def _bwd_fused_kernel(
    ...
    for idx in tl.range(0, n_full):
        ...
        qk = tl.dot(q.to(tl.float32), tl.trans(k_scaled).to(tl.float32), input_precision='tf32x3')  # v_acc30
...
def _bwd_dkdv_inner(
    ...
    for idx in tl.range(0, n_full):
        ...
        qk = tl.dot(q.to(tl.float32), tl.trans(k_scaled).to(tl.float32), input_precision='tf32x3')  # v_acc30 inner-comment
        p = tl.math.exp2(qk - lse_i[:, None])
...
def _bwd_dq_inner(
    ...
    for idx in tl.range(0, n_full, num_stages=2):
        ...
        qk = tl.dot(q_scaled.to(tl.float32), k.to(tl.float32), input_precision='tf32x3')  # v_acc30
        p = tl.math.exp2(qk - lse_i[:, None])
