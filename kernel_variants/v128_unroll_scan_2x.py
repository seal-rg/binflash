"""Binary Block Masked Flash Attention — v128: manual loop unroll by 2.

Unroll the inner while-loop scan by 2 via tl.static_range, processing
two consecutive col_idx values per outer iteration:

    while col_idx + 1 <= last_nz_col:
        for k in tl.static_range(2):
            c = col_idx + k
            bm_val = tl.load(bm_row_base + c)
            if bm_val == 2:
                # process block at c
        col_idx += 2

    # Tail handler for odd count
    while col_idx <= last_nz_col:
        ...

Hypothesis: the compiler can schedule instructions across unrolled
iterations (two bm_val loads in flight, overlapping K/V loads, etc.),
hiding scalar and global memory latency. Dead end #36 (prefetch next
bm_val) tried latency hiding via explicit `if` branches but those added
control flow overhead. Static unroll is branch-free within the outer
iteration.

Risk: 2x code size per unrolled loop could hurt i-cache. Register
pressure from overlapping two K/V tiles might spill. Net effect unknown.
"""
...
@triton.jit
def _process_full_block(
    acc, l_i, m_i, q,
    K_base, V_base, col, offs_d, offs_bn,
...
@triton.jit
def _process_partial_block(
    acc, l_i, m_i, q,
    K_base, V_base, Mask_base, col, offs_d, offs_bn, offs_bm,
    stride_kk, stride_kn, stride_vk, stride_vn,
...
def _binflash_fwd_inner(
    ...
    # ── Pass 1: full blocks (bm_val==2), unrolled by 2 ──
    col_idx = first_nz_col
    while col_idx + 1 <= last_nz_col:
        # Load two bm_vals in quick succession
        bm_val_0 = tl.load(bm_row_base + col_idx * bm_stride_col)
        bm_val_1 = tl.load(bm_row_base + (col_idx + 1) * bm_stride_col)
        if bm_val_0 == 2:
            acc, l_i, m_i = _process_full_block(
                acc, l_i, m_i, q, K_base, V_base, col_idx,
                offs_d, offs_bn,
                stride_kk, stride_kn, stride_vk, stride_vn,
                BLOCK_N,
            )
        if bm_val_1 == 2:
            acc, l_i, m_i = _process_full_block(
                acc, l_i, m_i, q, K_base, V_base, col_idx + 1,
                offs_d, offs_bn,
                stride_kk, stride_kn, stride_vk, stride_vn,
                BLOCK_N,
            )
    ...
    # ── Pass 2: partial blocks (bm_val==1), unrolled by 2 ──
    col_idx = first_nz_col
    while col_idx + 1 <= last_nz_col:
        bm_val_0 = tl.load(bm_row_base + col_idx * bm_stride_col)
        bm_val_1 = tl.load(bm_row_base + (col_idx + 1) * bm_stride_col)
        if bm_val_0 == 1:
            acc, l_i, m_i = _process_partial_block(
                acc, l_i, m_i, q, K_base, V_base, Mask_base, col_idx,
                offs_d, offs_bn, offs_bm,
                stride_kk, stride_kn, stride_vk, stride_vn,
                stride_mask_m, stride_mask_n,
                BLOCK_N,
            )
        if bm_val_1 == 1:
            acc, l_i, m_i = _process_partial_block(
                acc, l_i, m_i, q, K_base, V_base, Mask_base, col_idx + 1,
                offs_d, offs_bn, offs_bm,
                stride_kk, stride_kn, stride_vk, stride_vn,
                stride_mask_m, stride_mask_n,
                BLOCK_N,
...
def binflash_attention(
    ...
    """v114 + manual 2x unroll of scan loops."""
    ...
    if block_mask is None:
        ...
        if key == _bm_cache_key:
            block_mask, first_nz, last_nz, chosen_bm, chosen_bn = _bm_cache_val
        else:
            ...
            _bm_cache_key = key
            _bm_cache_val = (block_mask, first_nz, last_nz, chosen_bm, chosen_bn)
    else:
        nz = block_mask > 0
        nkb = nz.shape[1]
        col_indices = torch.arange(nkb, device=mask.device)
