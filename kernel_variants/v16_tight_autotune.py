"""v16 tight_autotune — v4 with only 4 autotune configs (vs 18).

v10 showed BIGGER autotune space (75 configs) is SLOWER — Triton picks
suboptimal configs under noisy measurement. Opposite hypothesis: a tight
space with only "likely optimal" configs forces better picks.

v16 configs: num_stages ∈ {1, 2} × num_warps ∈ {4, 8} = 4 configs.
...
# v16: tight 4-config space — just stages ∈ {1, 2} × warps ∈ {4, 8}.
for _s in [1, 2]:
    for _w in [4, 8]:
        _bwd_autotune_configs.append(triton.Config({}, num_stages=_s, num_warps=_w))
