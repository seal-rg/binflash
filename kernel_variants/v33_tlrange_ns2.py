"""v33 tlrange_ns2 — v26 with tl.range(num_stages=2) instead of 3.

Fwd's v210 used tl.range num_stages=2 (promoted), then v270 moved to ns=3
with a 0.59% paired gain. The bwd copied v270 (ns=3 on both loops). But
bwd's SMEM profile differs from fwd — maybe ns=2 is a better fit. v33 tests
ns=2 on both full and partial bwd loops.

Less pipelining = less SMEM pressure = more room for autotune to pick other
configs (different num_warps / maxnreg). Might unlock a better overall picks.

Applied to _bwd_dkdv_inner and _bwd_dq_inner loops only. Fwd kept at ns=3.
---
