...
Two-loop scan: first processes full blocks (bm_val==2, no mask load),
then partial blocks (bm_val==1, with mask). Separating them lets the
compiler pipeline K/V loads independently of mask loads.
"""
...
def _binflash_fwd_inner(
    ...
    # Mask_block_ptr is still at origin (not advanced in pass 1)
...
def binflash_attention(
    ...
    Args:
        ...
        block_mask: precomputed block mask. If None, auto-computed and cached.
            Best performance: pass None (auto-computes 3-valued int8 mask).
            Also accepts bool (N//block_m, N//block_n) for compatibility.
    """
