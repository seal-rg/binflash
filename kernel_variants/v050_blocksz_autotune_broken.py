...
Single kernel: scan-and-skip with block mask.
Autotuning searches over BLOCK_M, BLOCK_N, num_stages, num_warps.
Block mask is recomputed per autotune config via pre_hook.
"""
...
# Pre-hook: recompute block mask when BLOCK_M/BLOCK_N change during autotuning
def _block_mask_pre_hook(nargs):
    """Called by Triton autotuner before each config test.
    Recomputes the block mask to match the current BLOCK_M/BLOCK_N."""
    bm = nargs["BLOCK_M"]
    bn = nargs["BLOCK_N"]
    mask = nargs["Mask"]
    N = nargs["N_CTX"]
    # Only recompute if shape doesn't match
    current_bm = nargs["BlockMask"]
    expected_rows = N // bm
    expected_cols = N // bn
    if current_bm.shape[0] != expected_rows or current_bm.shape[1] != expected_cols:
        new_bm = make_block_mask(mask, bm, bn).to(torch.int8)
        nargs["BlockMask"] = new_bm
        nargs["stride_bm_row"] = new_bm.stride(0)
        nargs["stride_bm_col"] = new_bm.stride(1)


@triton.autotune(
    configs=[
        triton.Config({"BLOCK_M": bm, "BLOCK_N": bn}, num_stages=s, num_warps=w,
                       pre_hook=_block_mask_pre_hook)
        for bm in [64, 128]
        for bn in [32, 64]
        for s in [1, 2, 3, 4, 5]
        for w in [4, 8]
...
def binflash_attention(
    ...
    Block sizes (BLOCK_M, BLOCK_N) are autotuned — the pre_hook recomputes
    the block mask to match each candidate config.
    """
    B, H, N, D = q.shape
    ...
    # Initial block mask at default size — pre_hook will recompute if autotuner picks different
    block_m, block_n = 128, min(D, 64)
    if N % block_m != 0:
        block_m = 64
