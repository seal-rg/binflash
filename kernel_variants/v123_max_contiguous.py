"""Binary Block Masked Flash Attention — v123: tl.max_contiguous hints.

Current production (v114) uses raw pointer arithmetic:
    k_ptrs = K_base + offs_d[:, None] * stride_kk + (start_n + offs_bn)[None, :] * stride_kn

Dead end #26 tested `tl.multiple_of` alone — NEUTRAL/MIXED. This version
adds `tl.max_contiguous` on top, which is a stronger hint: it tells the
compiler the offsets are CONTIGUOUS (not just aligned), enabling wider
vectorized loads (128-bit/256-bit) where possible.

Applied to all K/V/Mask loads in both passes.
"""
...
def _binflash_fwd_inner(
    ...
    while col_idx <= last_nz_col:
        ...
        if bm_val == 2:
            ...
            # Contiguity/alignment hint: BLOCK_N-contiguous, BLOCK_N-aligned
            n_offs = tl.max_contiguous(tl.multiple_of(start_n + offs_bn, BLOCK_N), BLOCK_N)
            k_ptrs = K_base + offs_d[:, None] * stride_kk + n_offs[None, :] * stride_kn
            k = tl.load(k_ptrs)
            v_ptrs = V_base + n_offs[:, None] * stride_vk + offs_d[None, :] * stride_vn
            v = tl.load(v_ptrs)
            qk = tl.dot(q, k)
    ...
    while col_idx <= last_nz_col:
        ...
        if bm_val == 1:
            ...
            v_ptrs = V_base + n_offs[:, None] * stride_vk + offs_d[None, :] * stride_vn
            v = tl.load(v_ptrs)
            mask_ptrs = Mask_base + offs_bm[:, None] * stride_mask_m + n_offs[None, :] * stride_mask_n
...
def binflash_attention(
    ...
    """v114 + tl.max_contiguous/tl.multiple_of hints on K/V/Mask loads."""
