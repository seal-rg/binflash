...
def _gathered_fwd_inner(
    ...
    for idx in tl.range(n_full, n_total, num_stages=2):
        ...
        if USE_PACKED:
            ...
            # Convert bits directly to float bias: bit=0 → -1e6, bit=1 → 0
            bias_3d = (bits_3d.to(tl.float32) - 1.0) * 1.0e6
            bias_tile = tl.reshape(bias_3d, (BLOCK_M, BLOCK_N))
        else:
            mask_ptrs = Mask_base + offs_bm[:, None] * stride_mask_m + (start_n + offs_bn)[None, :] * stride_mask_n
            mask_ = tl.load(mask_ptrs) != 0
            bias_tile = tl.where(mask_, 0.0, -1.0e6)
        qk = tl.dot(q, k)
        qk += bias_tile
        m_ij = tl.maximum(m_i, tl.max(qk, 1))
        qk -= m_ij[:, None]
