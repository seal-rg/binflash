...
Two dispatch strategies selected at the Python level:
- Scan-and-skip: for low sparsity (< 60%). Static loop bounds, Triton optimized.
- Index-based: for high sparsity (>= 60%). tl.advance with dynamic offsets.
  Sign-bit encoding: positive index = partial block (load mask), negative = full
  block (skip mask load — all entries are True).
"""
...
# ───────────────── Kernel 2: Index + full-block skip (high sparsity) ─────────
...
def _index_fwd_inner(
    ...
    """Index-based dispatch with sign-bit full-block encoding.

    Positive index: partial block → load mask, apply it.
    Negative index: full block → skip mask load (all entries True).
    Index value: abs(raw_idx) gives the actual column index.
    """
    prev_col = tl.zeros([], dtype=tl.int32)

    for idx in range(num_blocks):
        raw_idx = tl.load(kv_indices_ptr + idx)
        # Decode: column index is abs value, sign indicates full/partial
        is_partial = raw_idx >= 0
        col_idx = tl.where(is_partial, raw_idx, -raw_idx - 1)
        ...
        # Only load and apply mask for partial blocks
        if is_partial:
            mask = tl.load(Mask_block_ptr) != 0
            qk += tl.where(mask, 0.0, -1.0e6)
...
def _build_signed_kv_indices(mask: torch.Tensor, block_mask: torch.Tensor, block_m: int, block_n: int):
    """Build index array with sign-bit encoding for full/partial blocks.

    Positive index i: partial block at column i (need mask load)
    Negative index -(i+1): full block at column i (skip mask load)
    """
    ...
    # Build column indices: positive for partial, negative for full
    col_ids = torch.arange(num_cols, device=mask.device)
    signed_ids = torch.where(is_full, -(col_ids + 1), col_ids)  # full: -(col+1), partial: col

    # Pack non-zero indices to front (sort by non-zero status)
    kv_num_blocks = block_mask.sum(dim=1).to(torch.int32)
    sorted_order = block_mask.int().mul(-1).argsort(dim=1, stable=True)
    kv_indices = signed_ids.expand(num_rows, -1).gather(1, sorted_order).to(torch.int32)
...
def binflash_attention(
    ...
    if sparsity > _SPARSITY_THRESHOLD:
        kv_num_blocks, kv_indices = _build_signed_kv_indices(mask, block_mask, block_m, block_n)
        _index_fwd[grid](
