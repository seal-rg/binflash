"""V198: V193 + tl.range with loop_unroll_factor=2 hint.

Dead end #27 tested for-loop with runtime bounds (2-7% slower) but didn't
try the loop_unroll_factor hint. Triton 3.6's tl.range accepts this hint
which may let the compiler generate better unrolled code.

Only changes the two inner loops (Pass 1 and Pass 2) from while to
tl.range(end, loop_unroll_factor=2). Kernel body unchanged otherwise.
"""
...
def _gathered_fwd_inner(
    ...
    # V198: use tl.range with loop_unroll_factor=2 for better unrolled codegen.
    # Pass 1: full blocks — iterate [0, n_full), no mask load needed.
    for idx in tl.range(0, n_full, loop_unroll_factor=2):
    ...
    for idx in tl.range(n_full, n_total, loop_unroll_factor=2):
        col_idx = tl.load(col_indices_ptr + idx)
        start_n = col_idx * BLOCK_N
        k_ptrs = K_base + offs_d[:, None] * stride_kk + (start_n + offs_bn)[None, :] * stride_kn
        k = tl.load(k_ptrs)
