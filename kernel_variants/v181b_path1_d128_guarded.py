"""v181b: extend path 1 to D=128 ONLY when N >= 4096.

v181 tested unguarded D=128 path 1 and showed:
- 7B-train longformer N=4096: +4%, N=8192: +14% (WINS)
- 7B-batch8 longformer N=1024: -7% (REGRESSION)
- 7B-long longformer N=8192: -2% (small regression)

Root cause of small-N regression: BM=64 doubles program count vs
BM=128. At small N × large BH, per-program overhead dominates.

v181b adds `N >= 4096` guard: D=128 path 1 only fires at moderate/
long contexts where the BM=64 work per program amortizes overhead.
D=64 path 1 behavior unchanged.
"""
import math
import torch
import triton
...
_PATH1_D128_MIN_N = 4096  # guard for D=128 path 1
...
def _dispatch_and_preprocess(mask: torch.Tensor, D: int):
    ...
    # v181b: path 1 fires at D=64 (all N) OR D=128 with N >= 4096.
    # Small-N × large-BH at D=128 pays too much BM=64 program overhead.
    path1_ok = (D == 64) or (D == 128 and N >= _PATH1_D128_MIN_N)
    if path1_ok and gap_ratio > _GAP_RATIO_THRESHOLD and N % 64 == 0:
