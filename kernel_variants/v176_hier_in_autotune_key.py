"""v176: add USE_HIER to the autotune key.

Currently at the default path (BM=128, BN=64), both USE_HIER=1
(longformer/cdw_sinks at D=128) and USE_HIER=0 (causal/log_tree/
prefix_lm/etc. at D=64/128) share one autotune cache entry per
(N_CTX, HEAD_DIM, Z_TIMES_H, BLOCK_M, BLOCK_N, USE_PACKED). Whichever
pattern autotunes first picks the config; the second pattern is
stuck with it.

Since USE_HIER=1 and USE_HIER=0 compile to very different kernel
bodies (constexpr-if specialization), their optimal num_stages/
num_warps may differ. Adding USE_HIER to the autotune key lets each
control-flow path autotune independently. No kernel body changes.

This is a zero-risk experiment: the kernel IR is byte-identical per
(USE_HIER, USE_PACKED, BM, BN, D, N, BH) combination. Only the
autotune cache lookup is affected.

Trade-off: doubles the number of autotune cache entries, so first-run
compilation takes longer. For benchmark cached runs, no difference.
"""
import math
import torch
import triton
...
@triton.autotune(
    ...
    # v176: added USE_HIER to the key so constexpr-specialized bodies
    # autotune independently.
    key=["N_CTX", "HEAD_DIM", "Z_TIMES_H", "BLOCK_M", "BLOCK_N", "USE_PACKED", "USE_HIER"],
)
@triton.jit
def _binflash_fwd_v176(
    ...
    acc, l_i, m_i = _binflash_fwd_inner_v176(
        acc, l_i, m_i, q,
