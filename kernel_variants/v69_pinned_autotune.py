...
# ─── Autotune configs for bwd kernels (v69: PINNED to autotune-picked configs) ───
#
# Empirical inspection of dKdV autotune cache across 5 patterns × 3 N values on
# llama3-8b shape (B=4 H=32 D=128) showed Triton ALWAYS picks (nw=4, ns=1,
# maxnreg=None). The 18-config search space wastes compile time and introduces
# autotune lottery noise (different methods pick marginally different configs
# due to noisy per-config bench timing → 0.1-0.2% spread between functionally
# identical kernels in the 66-cell version-comparison bench).
#
# v69 pins dKdV to the single config Triton converges on. dQ is kept with the
# 3 configs the autotune actually picked across patterns (nw=4 ns=1, nw=4 ns=2,
# nw=8 ns=1) since dQ shows N-dependent preference.
#
# Expected impact:
# - Identical performance to v52b on shapes where autotune picks the same config
# - Eliminates autotune lottery between successive runs (deterministic)
# - Faster first-call autotune (1 config vs 18 for dKdV)

_bwd_dkdv_autotune_configs = [
    triton.Config({}, num_stages=1, num_warps=4),
]

_bwd_dq_autotune_configs = [
    triton.Config({}, num_stages=1, num_warps=4),
    triton.Config({}, num_stages=2, num_warps=4),
    triton.Config({}, num_stages=1, num_warps=8),
]

# Backward compatibility alias for old call sites
_bwd_autotune_configs = _bwd_dkdv_autotune_configs
...
@triton.autotune(
    configs=_bwd_dq_autotune_configs,  # v69: dQ-specific 3-config set
    key=["N_CTX", "HEAD_DIM", "Z_TIMES_H", "BLOCK_M", "BLOCK_N", "USE_PACKED"],
)
@triton.jit
def _bwd_dq_kernel(
