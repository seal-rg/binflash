"""Binary Block Masked Flash Attention — v136: selective hoist on USE_PACKED=0.

Dead end #34 (v119) showed manual loop-invariant hoisting of K/V/Mask
row bases produces **consistent wins on longformer D=128 +2-3% and
causal D=128 +0.8%** but **consistent losses on sparse_10pct D=128
-0.7 to -1.4%**. The mechanism is pattern-dependent and couldn't be
cleanly promoted on v114 because the regression was inseparable from
the win.

v132 introduced the USE_PACKED constexpr split: sparse_10pct/30pct now
use a separate code branch (USE_PACKED=1) that unpacks the bit-packed
mask. This means we can apply v119's hoisting **only** to the
USE_PACKED=0 code path (used by longformer/causal/log_tree/cdw_sinks at
D=128) without touching the USE_PACKED=1 path where sparse_10pct lives.

Hoisted invariants (USE_PACKED=0 branch of pass 2 only):
  - K_row_base = K_base + offs_d[:, None] * stride_kk
  - V_col_base = V_base + offs_d[None, :] * stride_vn
  - Mask_row_base = Mask_base + offs_bm[:, None] * stride_mask_m
  - bm_row_base = block_mask_ptr + start_m * bm_stride_row

Pass 1 is NOT hoisted because changing its code-gen could affect the
USE_PACKED=1 path's register allocation (pass 1 code is compiled even
when no full blocks exist — just the body doesn't execute).

Expected: small wins on longformer/causal/cdw_sinks at D=128 from the
restricted hoist. sparse_10pct/30pct unchanged because their code path
is untouched.
"""
...
def _binflash_fwd_inner(
    ...
    # ── Pass 1: full blocks — unchanged (not hoisted to avoid touching USE_PACKED=1 path) ──
    ...
    if USE_PACKED:
        # USE_PACKED=1 path: unchanged from v132 (sparse_10pct/30pct)
        ...
        # USE_PACKED=0 path: HOISTED loop-invariants per v119
        K_row_base = K_base + offs_d[:, None] * stride_kk      # (HEAD_DIM, 1)
        V_col_base = V_base + offs_d[None, :] * stride_vn      # (1, HEAD_DIM)
        Mask_row_base = Mask_base + offs_bm[:, None] * stride_mask_m  # (BLOCK_M, 1)
        bm_row_base = block_mask_ptr + start_m * bm_stride_row
        col_idx = first_nz_col
        while col_idx <= last_nz_col:
            bm_val = tl.load(bm_row_base + col_idx * bm_stride_col)
            if bm_val == 1:
                start_n = col_idx * BLOCK_N
                n_offs = start_n + offs_bn
                k_ptrs = K_row_base + n_offs[None, :] * stride_kn
                k = tl.load(k_ptrs)
                v_ptrs = V_col_base + n_offs[:, None] * stride_vk
                v = tl.load(v_ptrs)
                mask_ptrs = Mask_row_base + n_offs[None, :] * stride_mask_n
...
def binflash_attention(
    ...
    """v132 + hoisted K/V/Mask row-bases in USE_PACKED=0 branch of pass 2."""
    ...
    if block_mask is None:
        ...
        raise NotImplementedError("External block_mask not supported in v136")
