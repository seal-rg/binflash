"""Binary Block Masked Flash Attention — v154: add num_warps=2 to autotune.

Dead end #6 (v086) removed num_warps=2 from autotune years ago because
"never optimal" at that time. But the kernel architecture has changed
significantly since then (v132 packed mask, v147 BM=64 unconditional
packing, v149 partial_fraction dispatch).

The BM=64 path (longformer, cdw_sinks at D=64) has smaller tiles:
  - Q (64, 64) BF16 = 8KB
  - acc (64, 64) F32 = 16KB
  - Unpack intermediate (64, 2, 32) int32 = 16KB
Total per program: ~40KB of registers

At num_warps=2 (64 threads), that's 640 bytes/thread — well below the
Ada register budget, and higher occupancy per SM than warps=4 or 8.

Adding num_warps=2 to the autotune space lets the autotuner pick it if
beneficial for any shape. If "never optimal" still holds, v154 is
neutral (autotune picks 4/8 as before). Low risk.

Config space: stages=[1..5] × warps=[2, 4, 8] = 15 configs (was 10).
"""
...
# v154: add num_warps=2 to autotune space (was [4, 8])
@triton.autotune(
    configs=[triton.Config({}, num_stages=s, num_warps=w)
             for s in [1, 2, 3, 4, 5] for w in [2, 4, 8]],
...
def binflash_attention(
    ...
    """v149 + num_warps=2 added to autotune space."""
    ...
    if block_mask is None:
        ...
        raise NotImplementedError("External block_mask not supported in v154")
