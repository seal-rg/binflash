"""v17 bm128_manual_ptr — retry BLOCK_M=128 on dQ but via manual pointer math.

v5 failed with SMEM OOM at BLOCK_M=128 BN=64. Triton's make_block_ptr may
allocate Q/dO in SMEM (64KB for bf16 BM=128 D=128). Maybe switching to
manual tl.load with explicit ptrs lets Triton keep them in registers,
freeing 64KB of SMEM — enough to fit the kernel.

v4 features (kept, except make_block_ptr in dQ):
...
def _bwd_dq_kernel(
    ...
    # v17: manual pointer arithmetic (instead of make_block_ptr) to hint at register allocation.
    offs_m = start_m * BLOCK_M + tl.arange(0, BLOCK_M)
    offs_d_q = tl.arange(0, HEAD_DIM)
    q_ptrs = Q + qvk_offset_q + offs_m[:, None] * stride_qm + offs_d_q[None, :] * stride_qk
    do_ptrs = DO + qvk_offset_do + offs_m[:, None] * stride_dom + offs_d_q[None, :] * stride_don
    q = tl.load(q_ptrs)
    do = tl.load(do_ptrs)
    ...
    # v17: manual pointer store.
    dq_ptrs = DQ + qvk_offset_dq + offs_m[:, None] * stride_dqm + offs_d_q[None, :] * stride_dqk
...
_BWD_BLOCK_M = 64
_BWD_BLOCK_N = 64
_BWD_DQ_BLOCK_M = 128   # v17: try BM=128 for dQ only
_BWD_DQ_BLOCK_N = 64
...
def _binflash_bwd(do, q, k, v, o, lse, mask, sm_scale, preproc=None):
    ...
    # v17: dQ uses BLOCK_M=128 with fwd's (N/128, N/64) col_indices.
    col_indices_dq = preproc["col_indices_fwd"]
    n_full_dq = preproc["n_full_fwd"]
    n_total_dq = preproc["n_total_fwd"]
    assert preproc["chosen_bm"] == _BWD_DQ_BLOCK_M and preproc["chosen_bn"] == _BWD_DQ_BLOCK_N, \
        f"v17 requires fwd BM={_BWD_DQ_BLOCK_M} BN={_BWD_DQ_BLOCK_N}; got {preproc['chosen_bm']}/{preproc['chosen_bn']}"

    grid_dq = (triton.cdiv(N, _BWD_DQ_BLOCK_M), B * H)
    _bwd_dq_kernel[grid_dq](
        ...
        B, H, N, B * H,
        BLOCK_M=_BWD_DQ_BLOCK_M, BLOCK_N=_BWD_DQ_BLOCK_N, HEAD_DIM=D,
        USE_PACKED=USE_PACKED,
    )
