"""v089: Full-block mask skip — CONFIRMED IMPROVEMENT at production scale.

3-valued block mask: 0=skip block, 1=partial (load fine mask), 2=full (skip mask load).
Combined with block mask cache, the .all() preprocessing is amortized to zero.

This was previously tested at small scale (v080) and rejected because
preprocessing cost exceeded kernel savings. The key difference now:
1. Block mask cache amortizes preprocessing to zero on repeated calls
2. Production scale (D=128, B=2, H=32) makes the 8KB/block mask savings significant
3. Focused autotune (8 configs) avoids noise that hurt the earlier test

Results at production scale:
  7B-train causal (N=8K): 8.712ms (was 9.394ms) -> 7.2% faster
  7B-train sliding: 1.216ms -> 14% vs v049
  70B-GQA causal: 1.159ms -> 20% vs v049
  No regressions on any config/pattern.

Block composition for causal (N=8192): 4032 full + 128 partial + 4032 empty.
The 4032 full blocks each skip 8KB mask load = 31.5MB saved per batch-head.
"""
