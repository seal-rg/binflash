"""v080: 3-valued block mask — SLOWER.

Idea: use int8 block mask with values 0=skip, 1=partial, 2=full.
Skip mask tile load (8KB) for fully-True blocks.

Results: preprocessing make_block_mask_3val costs 1.766ms (int16.sum route)
or 0.252ms (any+all route), vs 0.165ms for current make_block_mask.
Kernel savings from skipping mask load on full blocks: ~33us for causal.
Net: WORSE across all patterns.

Block composition for N=8192, BM=128, BN=64:
  causal: full=4032, partial=128, empty=4032
  sliding: full=128, partial=252, empty=7812
  blkdiag: full=128, partial=0, empty=8064

Conclusion: The ~87us extra preprocessing cost exceeds the ~33us kernel savings.
The current bool block mask with make_block_mask(.any()) is optimal.

This file contains the baseline kernel (same as v080_before_3val_blockmask),
not the experimental 3-valued variant which was transient and reverted.
"""
