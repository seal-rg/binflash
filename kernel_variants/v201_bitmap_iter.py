"""V201: sequential multi-head per program (2 heads).

Each program processes H_CHUNK=2 heads sequentially for same Q-row-block.
Shares: col_indices load, n_full/n_total, kernel launch.
Different: Q, K, V, acc per head iteration.

Target: reduce kernel launches 2x on all workloads.
"""
...
@triton.autotune(
    ...
    key=["N_CTX", "HEAD_DIM", "Z_TIMES_H", "BLOCK_M", "BLOCK_N", "USE_PACKED", "H_CHUNK"],
)
@triton.jit
def _gathered_fwd(
    Q,
    ...
    # V201: each program processes H_CHUNK heads sequentially for same Q-row-block.
    # Loads col_indices/n_full/n_total ONCE (shared across heads).
    start_m = tl.program_id(0)
    pid_pair = tl.program_id(1)

    # Load per-row gathered metadata ONCE (same for all heads — mask is shared).
    n_full = tl.load(NFull + start_m)
    n_total = tl.load(NTotal + start_m)
    ci_base = ColIndices + start_m * stride_ci_row

    # Loop over heads in this chunk
    for head_idx in tl.static_range(H_CHUNK):
        off_hz = pid_pair * H_CHUNK + head_idx
...
def binflash_attention(
    ...
    # V201: H_CHUNK=2 heads per program
    H_CHUNK = 2
    total_hz = B * H
    if total_hz % H_CHUNK != 0:
        H_CHUNK = 1
    grid = (triton.cdiv(N, chosen_bm), total_hz // H_CHUNK)
