...
Two dispatch strategies, selected at the Python level based on sparsity:
- Scan-and-skip (low sparsity): iterate all blocks, check block mask, skip zeros.
  Triton optimizes the static loop bounds well.
- Index-based (high sparsity): precompute non-zero block indices, iterate only
  those, use tl.advance with dynamic deltas.
"""
...
_SPARSITY_THRESHOLD = 0.6  # Use index dispatch above this block sparsity
...
def binflash_attention(
    ...
    Auto-selects between scan-and-skip (dense) and index-based (sparse) dispatch.
    """
    B, H, N, D = q.shape
