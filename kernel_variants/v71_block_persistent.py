"""v71 block-partitioned persistent dKdV — fix v70's L2 locality regression.

v70 used STRIDED partitioning: pid stride = N_PERSISTENT means consecutive
work_ids in one program are far apart (different off_hz, same start_n). For
large B*H this jumps across (B,H) tensors → no L2 reuse on K, V loads.

v71 uses BLOCK partitioning: each program handles a contiguous range
[pid*chunk, (pid+1)*chunk) of work_ids. Adjacent work_ids in one program have
ADJACENT start_n (within one off_hz) — preserves K-block L2 adjacency.

Trade-off:
- Big workloads (e.g. B*H=128, n_total_work=16384): N_PERSISTENT=128 → 128
  programs each doing 128 sequential K-blocks. Mostly fills 142 SMs.
- Small workloads (e.g. B*H=8): would have most programs with 0-1 work items.

v71 GATES: use block-persistent only when n_total_work >= 256. Otherwise
falls back to standard grid (B*H << num_sms case)."""
...
def _bwd_dkdv_kernel(
    ...
    # v71 block partition: each program handles work_ids [pid*chunk, (pid+1)*chunk).
    # Adjacent work_ids in one program have adjacent start_n → good K-block L2 adjacency.
    pid = tl.program_id(0)
    chunk = tl.cdiv(n_total_work, N_PERSISTENT)
    work_start = pid * chunk
    work_end = tl.minimum(work_start + chunk, n_total_work)
    work_id = work_start
    while work_id < work_end:
        ...
        work_id += 1  # v71: contiguous range, increment by 1
