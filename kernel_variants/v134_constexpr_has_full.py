"""Binary Block Masked Flash Attention — v134: constexpr HAS_FULL dispatch.

v132 uses `USE_PACKED: tl.constexpr` to compile-select the mask-loading
path. This version extends that pattern with `HAS_FULL: tl.constexpr`
that compile-elides pass 1 entirely when the preprocessing determines
no full blocks exist anywhere in the block mask.

For patterns where every non-empty block is partial (e.g. sparse_10pct
random with per-entry density 19% — a block is "all True" with
probability ~10^-1900), pass 1's while loop runs zero processing but
still iterates nkb bm_val loads per Q program wastefully. Eliding
removes that iteration overhead.

Dead end #39 (v124 per-row has_full/has_partial) tried this with a
runtime `if` branch and was dominated by autotune variance. Using a
constexpr instead of a runtime scalar means Triton specializes the
kernel body for the two cases — no branching, no autotune perturbation
from extra control flow.

Dispatch: `HAS_FULL = 1 if (block_mask == 2).any() else 0`. The
autotune key includes HAS_FULL so both specializations get their own
optimum config.
"""
...
def _fused_preproc(mask, block_m, block_n, want_packed=False):
    ...
    has_full_any = (block_mask == 2).any().item()  # bool
    mask_packed = _pack_mask(mask) if want_packed else None
    return block_mask, first_nz, last_nz, avg_gap_ratio, density, has_full_any, mask_packed
...
def _binflash_fwd_inner(
    ...
    N_CTX: tl.constexpr, USE_PACKED: tl.constexpr, HAS_FULL: tl.constexpr,
):
    ...
    # ── Pass 1: full blocks (bm_val==2) — compile-elided when HAS_FULL=0 ──
    if HAS_FULL:
    ...
    # ── Pass 2: partial blocks (bm_val==1) — always runs ──
    col_idx = first_nz_col
    while col_idx <= last_nz_col:
        bm_val = tl.load(block_mask_ptr + start_m * bm_stride_row + col_idx * bm_stride_col)
        if bm_val == 1:
...
def _binflash_fwd(
    ...
    USE_PACKED: tl.constexpr, HAS_FULL: tl.constexpr,
):
...
def binflash_attention(
    ...
    """v132 + constexpr HAS_FULL to compile-elide pass 1 when no full blocks."""
    ...
    if block_mask is None:
        ...
        if key == _bm_cache_key:
            block_mask, first_nz, last_nz, mask_packed, chosen_bm, chosen_bn, use_packed, has_full = _bm_cache_val
        else:
            bm128_bn64, fn, ln, gap_ratio, density, hf0, _ = _fused_preproc(mask, 128, 64, want_packed=False)
            if D == 64 and gap_ratio > _GAP_RATIO_THRESHOLD and N % 64 == 0:
                want = density > _DENSITY_THRESHOLD_PACKED
                block_mask, first_nz, last_nz, _, _, hf, mask_packed = _fused_preproc(
                    mask, 64, 64, want_packed=want,
                )
                chosen_bm, chosen_bn = 64, 64
                use_packed = 1 if want else 0
                has_full = 1 if hf else 0
                ...
                has_full = 1 if hf0 else 0
            _bm_cache_key = key
            _bm_cache_val = (block_mask, first_nz, last_nz, mask_packed, chosen_bm, chosen_bn, use_packed, has_full)
    else:
        raise NotImplementedError("External block_mask not supported in v134")
    ...
    _binflash_fwd[grid](
        ...
        USE_PACKED=use_packed, HAS_FULL=has_full,
    )
    return o
