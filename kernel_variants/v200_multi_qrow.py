"""V200: Process K_CHUNK=2 Q-row-blocks per program.

Target: overhead-bound patterns (block_diagonal at 27% TC peak,
sliding_window at 55%, causal_window at 60%). Each program does
K_CHUNK sequential Q-row-block computations, amortizing kernel
launch + Q-load overhead by 2x.

Cons: doubles per-program register state (2 sets of m_i, l_i, acc),
which may cause spills. Also halves parallelism — but benchmark grid
is 2-4x oversaturated, so parallelism reduction is OK.

If this helps overhead-bound patterns without hurting compute-bound,
it's a net win.
"""
...
@triton.autotune(
    ...
    key=["N_CTX", "HEAD_DIM", "Z_TIMES_H", "BLOCK_M", "BLOCK_N", "USE_PACKED", "K_CHUNK"],
)
@triton.jit
def _gathered_fwd(
    Q,
    ...
    # V200: each program sequentially processes K_CHUNK Q-row-blocks.
    ...
    # Loop over K_CHUNK Q-row-blocks assigned to this program.
    for chunk_idx in tl.static_range(K_CHUNK):
        start_m = pid_m * K_CHUNK + chunk_idx

        # Recompute Q and O block pointers for this start_m
        ...
        # Fresh softmax state per Q-row-block
...
def binflash_attention(
    ...
    # V200: K_CHUNK Q-row-blocks per program.
    K_CHUNK = 2
    nqb = triton.cdiv(N, chosen_bm)
    if nqb % K_CHUNK != 0:
        # Fall back to K_CHUNK=1 if nqb isn't divisible
        K_CHUNK = 1
    grid = (nqb // K_CHUNK, B * H)
