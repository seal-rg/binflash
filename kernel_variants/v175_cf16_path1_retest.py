"""v175: retest dead end #70 (CF=16 on path 1 wide-gap) now that
v173's explicit elif branch split is in production. The original
v170 test (v168 body, nested ifs) showed -5.6 to -6.1% regression at
longformer N=8192/16384 because 16-wide static_range unroll hit
register pressure with the merged full/partial body.

v173's per-arm register allocation SHOULD reduce register pressure —
the full arm doesn't hold mask intermediates live, so the 16-wide
unroll of the full arm is less costly. Worth retesting.

Only changes _CF_PATH1_WIDE from 8 to 16. Everything else (dispatch,
kernel code) unchanged — reuses production binflash_attention._binflash_fwd.
"""
import math
import torch
import triton
...
# v175: CF=16 on wide-gap path 1 (was 8)
_CF_PATH1_WIDE = 16
_CF_PATH1 = 4
_CF_OTHER = 2
