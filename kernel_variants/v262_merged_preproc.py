...
def _dispatch_and_preprocess(mask: torch.Tensor, D: int):
    ...
    # V262: Use the COARSE block_mask (main dispatch path) to compute metrics
    # via the merged _build_gathered_indices kernel. For D=64 paths, we fall
    # back to separate gathering (but still skip _reduce_block_mask).
    ...
    if D == 64 and gap_ratio > _GAP_RATIO_THRESHOLD and N % 64 == 0:
        # Path 1: gap-heavy → BM=64 BN=64; need to build col_indices on fine mask
    ...
    if not use_packed:
        mask_packed = None
    # col_indices/n_full/n_total already computed above for the coarse path
