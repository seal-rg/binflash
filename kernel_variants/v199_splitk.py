"""V199: Split-K attention on top of v193's gathered dispatch.

Radical refactor: split each Q-row-block's K iterations across S=2 programs.
Each program computes partial (m, l, acc) for its slice. A second 'combine'
kernel merges the S partials via online softmax.

Target: sparse patterns at large N where long K-scans limit TC efficiency
(sparse_10pct at N=32768 is 62% TC peak vs 77% at N=16384).

Memory overhead: partial_acc is (S, B, H, N, D) bf16 ≈ 1GB at large N.
partial_m/l: tiny.

Implementation: S=2 always (no dispatch decision). Simpler to validate.
"""
...
def _gathered_fwd_inner(
    ...
    # V199: split-K. Each program handles a slice [slice_start, slice_end) of col_indices.
    # slice_start and slice_end are passed as additional kernel args (see _splitk_fwd).
    # The slice may span the full/partial boundary (n_full), so we still need the two-pass
    # structure, but bounded by slice bounds.
    #
    # Pass 1: full blocks within this slice — iterate [slice_start, min(slice_end, n_full))
    idx = slice_start
    full_end = tl.minimum(slice_end, n_full)
    while idx < full_end:
    ...
    # Pass 2: partial blocks within this slice — iterate [max(idx, n_full), slice_end)
    # idx is wherever pass 1 left off. If slice started in partial region, idx == slice_start.
    while idx < slice_end:
        col_idx = tl.load(col_indices_ptr + idx)
        start_n = col_idx * BLOCK_N
        k_ptrs = K_base + offs_d[:, None] * stride_kk + (start_n + offs_bn)[None, :] * stride_kn
...
def _splitk_fwd(
    ...
    V,
    sm_scale,
    PartialOut,    # (S, B, H, N, D) bf16 — partial accumulator output
    PartialM,      # (S, B, H, N) fp32 — partial max
    PartialL,      # (S, B, H, N) fp32 — partial sum
    Mask,
    MaskPacked,
    ColIndices,
    NFull,
    ...
    stride_po_m,
    stride_po_d,
    stride_pml_s,  # shared by PartialM and PartialL
    stride_pml_z,
    stride_pml_h,
    stride_pml_m,
    stride_mask_m,
    ...
    z = off_hz // H
    h = off_hz % H
    qvk_offset = z.to(tl.int64) * stride_qz + h.to(tl.int64) * stride_qh
    ...
    # Compute this program's slice of col_indices.
    # Split evenly: slice s covers [s * n_total // S, (s+1) * n_total // S).
    slice_start = (s_idx * n_total) // S
    slice_end = ((s_idx + 1) * n_total) // S
    ...
    # Write partial output (unnormalized acc — combine kernel normalizes).
    # Store partial_m and partial_l as scheduling anchors (like main v193 kernel).
    offs_m = start_m * BLOCK_M + tl.arange(0, BLOCK_M)
    offs_d = tl.arange(0, HEAD_DIM)

    # Partial base offsets
    pml_offset = (
        s_idx.to(tl.int64) * stride_pml_s
        + z.to(tl.int64) * stride_pml_z
        + h.to(tl.int64) * stride_pml_h
    )
    po_offset = (
        s_idx.to(tl.int64) * stride_po_s
        + z.to(tl.int64) * stride_po_z
        + h.to(tl.int64) * stride_po_h
    )

    # Store partial m, l
    tl.store(PartialM + pml_offset + offs_m * stride_pml_m, m_i)
    tl.store(PartialL + pml_offset + offs_m * stride_pml_m, l_i)

    # Store partial acc (NOT normalized — combine kernel handles that)
    acc_ptrs = (
        PartialOut + po_offset
        + offs_m[:, None] * stride_po_m
        + offs_d[None, :] * stride_po_d
    )
    tl.store(acc_ptrs, acc.to(PartialOut.type.element_ty))


@triton.jit
def _splitk_combine(
    PartialOut,    # (S, B, H, N, D) bf16
    PartialM,      # (S, B, H, N) fp32
    PartialL,      # (S, B, H, N) fp32
    Out,           # (B, H, N, D) bf16
    LSE,           # (B, H, N) fp32
    stride_oz,
    stride_oh,
    ...
    for s in tl.static_range(S):
        ...
        m_s = tl.load(PartialM + pml_offset + offs_m * stride_pml_m)
        l_s = tl.load(PartialL + pml_offset + offs_m * stride_pml_m)
        acc_s = tl.load(
            PartialOut + po_offset
            + offs_m[:, None] * stride_po_m
            + offs_d[None, :] * stride_po_d
        ...
        # m_new = max(m_i, m_s)
        # alpha_i = exp2(m_i - m_new); alpha_s = exp2(m_s - m_new)
        # l_new = l_i * alpha_i + l_s * alpha_s
        # acc_new = acc * alpha_i + acc_s * alpha_s
        m_new = tl.maximum(m_i, m_s)
        alpha_i = tl.math.exp2(m_i - m_new)
        alpha_s = tl.math.exp2(m_s - m_new)
        l_i = l_i * alpha_i + l_s * alpha_s
        acc = acc * alpha_i[:, None] + acc_s * alpha_s[:, None]
        m_i = m_new
    ...
    lse_offset = (
        z.to(tl.int64) * (H * N_CTX)
        + h.to(tl.int64) * N_CTX
    )
    tl.store(LSE + lse_offset + offs_m, m_i + tl.math.log2(l_i))

    out_offset = z.to(tl.int64) * stride_oz + h.to(tl.int64) * stride_oh
...
def binflash_attention(
    ...
    S = 2  # split factor for split-K
    o = torch.empty_like(q)
    lse = torch.empty(B, H, N, device=q.device, dtype=torch.float32)

    # Partial buffers: allocated per call (could cache but benchmark call count is small)
    partial_out = torch.empty(S, B, H, N, D, device=q.device, dtype=torch.bfloat16)
    partial_m = torch.empty(S, B, H, N, device=q.device, dtype=torch.float32)
    partial_l = torch.empty(S, B, H, N, device=q.device, dtype=torch.float32)

    grid = (triton.cdiv(N, chosen_bm), B * H, S)
    ...
    # Combine kernel
    combine_grid = (triton.cdiv(N, chosen_bm), B * H)
    _splitk_combine[combine_grid](
        partial_out,
        partial_m,
        partial_l,
        o,
