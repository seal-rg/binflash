"""v14 reorder_dp — v4 with dp computation moved before dv accumulation in dK/dV inner.

Rationale: in v4's dK/dV inner loop, the sequence is:
  dv += trans(p) @ do     (needs p)
  dp  = do @ trans(v)      (needs do, v — independent of p)
  ds  = p * (dp - d_i)
  dk += trans(ds) @ q
Moving dp computation earlier (before dv) may let the compiler start the
dp matmul in parallel with the trans(p) op + the dv accumulation. Triton
compilers reorder by data dependency, but explicit ordering gives a hint.
...
def _bwd_dkdv_inner(
    ...
    for idx in tl.range(0, n_full, num_stages=3):
        ...
        # v14: start dp (independent of p) before dv accumulation.
