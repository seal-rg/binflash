"""Binary Block Masked Flash Attention — v133: grid transpose for dense-partial patterns.

Dead end #12 found that swapping the kernel grid from (N/BM, B*H) to
(B*H, N/BM) gives ~2x speedup on sparse/GQA workloads because adjacent
programs in a wave now process the SAME head with different Q blocks,
enabling L2 cache reuse of K/V tiles across programs. But it regressed
causal D=128 by -30% due to load imbalance: causal rows near the
diagonal have very different scan widths, so one wave has heterogeneous
work per SM.

v132's insight: for dense-partial patterns (density > 0.8), block-scan
widths are uniform across rows → no load imbalance. We can apply grid
transpose only to those patterns, capturing the K/V reuse win without
the causal regression.

Combined with v132's packed mask: patterns with density > 0.8 get BOTH
(1) int32 bit-packed mask and (2) transposed grid. The expected effect
stacks the mask-bandwidth win (+28-44% from v132) with K/V-reuse win.

Key change: `GRID_TRANSPOSED: tl.constexpr` controls program_id
assignment. When True, `off_hz = tl.program_id(0)` and
`start_m = tl.program_id(1)`; when False, the reverse (v132 default).
"""
...
@triton.autotune(
    ...
    key=["N_CTX", "HEAD_DIM", "Z_TIMES_H", "BLOCK_M", "BLOCK_N", "USE_PACKED", "GRID_TRANSPOSED"],
)
@triton.jit
def _binflash_fwd(
    ...
    USE_PACKED: tl.constexpr, GRID_TRANSPOSED: tl.constexpr,
):
    if GRID_TRANSPOSED:
...
def binflash_attention(
    ...
    """v132 + grid transpose for the packed-mask-dispatch (dense-partial) path."""
    ...
    if block_mask is None:
        ...
        raise NotImplementedError("External block_mask not supported in v133")

    if N % chosen_bm != 0 or N % chosen_bn != 0:
        raise ValueError(f"N must be divisible")

    # Grid transpose when packed-mask dispatch is active (dense-partial patterns)
    grid_transposed = use_packed
    o = torch.empty_like(q)
    lse = torch.empty(B, H, N, device=q.device, dtype=torch.float32)
    if grid_transposed:
        grid = lambda META: (B * H, triton.cdiv(N, chosen_bm))
    else:
    ...
    _binflash_fwd[grid](
        ...
        USE_PACKED=use_packed, GRID_TRANSPOSED=grid_transposed,
    )
    return o
