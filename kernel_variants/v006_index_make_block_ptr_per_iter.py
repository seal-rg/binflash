...
2. In the flash-attention inner loop, iterate only over non-zero blocks using the
   index list, creating block pointers at the right offsets each iteration.
...
def _binflash_fwd_inner(
    ...
    for idx in range(num_blocks):
        ...
        # Create block pointers at the exact offset for this non-zero block
...
def _build_kv_indices(block_mask: torch.Tensor):
    ...
    Cheap preprocessing: just sorts each row of the block mask to pack non-zero
    column indices to the front. Runs in microseconds on the tiny block mask.
    """
    num_rows, num_cols = block_mask.shape
    kv_num_blocks = block_mask.sum(dim=1).to(torch.int32)
    # argsort on negated mask puts True positions first (stable sort preserves order)
...
def binflash_attention(
    ...
    # Build index-based dispatch structures (cheap — operates on tiny block mask)
