...
def _bwd_fused_kernel(
    ...
    k = tl.load(K_bp)
    v = tl.load(V_bp)
    k_scaled = (k * sm_scale).to(k.dtype)  # v_acc62: drop log2e prescale (postscale qk in fp32)
    ...
    for idx in tl.range(0, n_full):
        ...
        qk = tl.dot(q, tl.trans(k_scaled)) * 1.44269504  # v_acc62: postscale log2e in fp32
    ...
    for idx in tl.range(n_full, n_total):
        ...
        qk += tl.where(mask_, 0.0, -1.0e6 * 1.44269504)  # v_acc62: mask in log2 basis after postscale
        p = tl.math.exp2(qk - lse_i[:, None])
        dv = tl.dot(tl.trans(p.to(tl.float16)), do.to(tl.float16), dv)  # v_acc51: fp16 dV+dK
        dp = tl.dot(do, tl.trans(v)).to(tl.float32)
        ds = p * (dp - d_i[:, None])
...
def _bwd_dq_inner(
    ...
    for idx in tl.range(0, n_full, num_stages=2):
        ...
        qk = tl.dot(q_scaled, k) * 1.44269504  # v_acc62: postscale log2e
        p = tl.math.exp2(qk - lse_i[:, None])
...
def _bwd_dq_kernel(
    ...
    q = tl.load(Q_bp)
    do = tl.load(DO_bp)
    q_scaled = (q * sm_scale).to(q.dtype)  # v_acc62: drop log2e prescale
