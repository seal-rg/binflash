"""v089 baseline — focused autotune + Z_TIMES_H key + cache, before full-block mask skip.

Focused 8-config autotune: stages=[1-4], warps=[4,8].
Autotune key includes Z_TIMES_H (=B*H) for occupancy-aware config selection.
Bool block mask with single-entry cache. Single loop, all blocks load mask.
"""
