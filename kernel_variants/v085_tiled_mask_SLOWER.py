"""v085: Tiled mask layout — SLOWER.

Idea: Pre-tile the (N,N) bool mask to (nqb*nkb*BM, BN) contiguous layout so
each (BM,BN) tile is a contiguous 8KB read instead of 128 scattered row-reads.

v1 (raw pointers): 0.51-0.79x SLOWER at D=64.
v2 (tl.make_block_ptr): 0.51-0.84x SLOWER at D=64, 0.81-1.02x at D=128.

Why slower: tl.make_block_ptr already optimizes strided 2D loads from row-major
tensors. The compiler generates efficient coalesced load instructions regardless
of whether the data is physically contiguous. The tiled layout adds preprocessing
cost (.permute().contiguous() copies the full NxN mask) with no kernel benefit.

Key insight: on modern GPUs with tl.make_block_ptr, the physical memory layout
of small tile loads (8KB) doesn't matter -- the memory controller and L2 cache
handle strided access patterns efficiently.

This file contains the baseline kernel (same as v085_before_tiled_mask),
not the experimental tiled layout variant which was transient and reverted.
"""
