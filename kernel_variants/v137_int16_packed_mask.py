"""Binary Block Masked Flash Attention — v137: int16 bit-packed mask.

v132 packs the mask into int32 (32 bits per word). Inside the kernel,
the unpack broadcasts the packed tensor to (BM, BN/32, 32) int32, then
reshapes to (BM, BN) bool. For BM=128 BN=64, this intermediate is
128*2*32 = 8192 int32 values = **32KB of register space per program**.
Combined with Q (32KB) + acc (64KB at D=128) this approaches the
256-thread register budget of ~128KB.

This version uses int16 packing (16 bits per word): intermediate is
(BM, BN/16, 16) = 8192 int16 = **16KB**, halving register pressure
during unpack. Same mask bandwidth (still 1 bit per entry), just a
different on-chip representation.

Expected effect: if register pressure is the bottleneck limiting
autotune's stages picks (v135 showed stages=3 for sparse_10pct D=128
packed, below the theoretical max of 5-7), the freed registers might
unlock deeper pipelines and therefore better compute/memory overlap.
"""
...
def _pack_mask_i16(mask: torch.Tensor) -> torch.Tensor:
    """Pack bool (N, N) into int16 (N, N//16). bit k of packed[i, j] == mask[i, j*16+k]."""
    N = mask.shape[0]
    assert N % 16 == 0, f"N={N} must be divisible by 16 for int16 bit packing"
    # Compute as int32 then cast (avoid int16 arithmetic issues)
    weights = (1 << torch.arange(16, device=mask.device, dtype=torch.int32))
    mask_reshaped = mask.view(N, N // 16, 16).to(torch.int32)
    packed_i32 = (mask_reshaped * weights).sum(dim=-1, dtype=torch.int32)
    # Values fit in uint16; cast to int16 (bit pattern preserved)
    return packed_i32.to(torch.int16)
...
def _fused_preproc(mask, block_m, block_n, want_packed=False):
    ...
    mask_packed = _pack_mask_i16(mask) if want_packed else None
...
def _binflash_fwd_inner(
    ...
    BN_PACKED: tl.constexpr = BLOCK_N // 16  # int16 packing → 16 bits per word
    offs_bn_packed = tl.arange(0, BN_PACKED)
    bit_offs = tl.arange(0, 16)
    ...
    while col_idx <= last_nz_col:
        ...
        if bm_val == 1:
            ...
            v = tl.load(v_ptrs)
            if USE_PACKED:
                # int16 packed mask: (BM, BN/16)
                start_n_packed = (col_idx * BLOCK_N) // 16
                mp_ptrs = MaskPacked_base + offs_bm[:, None] * stride_mp_m + (start_n_packed + offs_bn_packed)[None, :] * stride_mp_n
                packed = tl.load(mp_ptrs)  # (BM, BN_PACKED) int16
                # Promote to int32 for shift arithmetic (Triton may emit i16 ops otherwise)
                packed_i32 = packed.to(tl.int32)
                bits_3d = (packed_i32[:, :, None] >> bit_offs[None, None, :]) & 1  # (BM, BN/16, 16)
...
def binflash_attention(
    ...
    """v132 with int16 bit-packed mask (halved unpack intermediate)."""
    ...
    if block_mask is None:
        ...
        if key == _bm_cache_key:
            ...
            if D == 64 and gap_ratio > _GAP_RATIO_THRESHOLD and N % 64 == 0:
                ...
                mask_packed = _pack_mask_i16(mask) if use_packed else None
                chosen_bm, chosen_bn = 128, 64
            _bm_cache_key = key
            _bm_cache_val = (block_mask, first_nz, last_nz, mask_packed, chosen_bm, chosen_bn, use_packed)
    else:
        raise NotImplementedError("External block_mask not supported in v137")
