...
def _gathered_fwd_inner(
    ...
    stride_mp_m,
    stride_mp_n,
    bm_row,  # (NKB_POW2,) int8 — block_mask row for this start_m
    n_full,
    n_total,
    start_m,
    ...
    offs_kb = tl.arange(0, NKB_POW2)

    # Derive per-category cumsum ranks (once, from bm_row in registers)
    is_full = (bm_row == 2)
    is_partial = (bm_row == 1)
    full_cumsum = tl.cumsum(is_full.to(tl.int32), axis=0)  # 1-based rank, 0 elsewhere
    partial_cumsum = tl.cumsum(is_partial.to(tl.int32), axis=0)

    # Pass 1: full blocks — select the (idx+1)-th full bit from bm_row via reduction
    for idx in tl.range(0, n_full, num_stages=2):
        target = idx + 1
        match_full = is_full & (full_cumsum == target)
        col_idx = tl.sum(tl.where(match_full, offs_kb, 0).to(tl.int32), axis=0)
    ...
    # Pass 2: partial blocks — select (idx - n_full + 1)-th partial bit
    for idx in tl.range(n_full, n_total, num_stages=2):
        target = idx - n_full + 1
        match_partial = is_partial & (partial_cumsum == target)
        col_idx = tl.sum(tl.where(match_partial, offs_kb, 0).to(tl.int32), axis=0)
        start_n = col_idx * BLOCK_N
        k_ptrs = K_base + offs_d[:, None] * stride_kk + (start_n + offs_bn)[None, :] * stride_kn
        k = tl.load(k_ptrs)
        v_ptrs = V_base + (start_n + offs_bn)[:, None] * stride_vk + offs_d[None, :] * stride_vn
...
def _gathered_fwd(
    ...
    Mask,
    MaskPacked,
    BlockMask,  # (nqb, nkb) int8 — now replaces ColIndices input
    NFull,
    NTotal,
    LSE,
    stride_qz,
    ...
    # Load block_mask row into registers (NKB_POW2 int8s)
    offs_kb_load = tl.arange(0, NKB_POW2)
    nkb_runtime = N_CTX // BLOCK_N
    bm_row = tl.load(
        BlockMask + start_m * stride_bm_row + offs_kb_load,
        mask=offs_kb_load < nkb_runtime,
        other=0,
    )
...
def binflash_attention(
    ...
    """V260 fused popcount: col_idx derived from bm_row via in-kernel cumsum."""
    ...
    if key == _bm_cache_key:
        ...
        # Need block_mask tensor + n_full/n_total, but NOT col_indices
        out_preproc = _fused_preproc_multigrain(mask, 64)
        density = out_preproc["density"]
        gap_ratio = out_preproc["gap_ratio"]
        partial_fraction = out_preproc["partial_fraction"]
        mask_packed = out_preproc["mask_packed"]
        if D == 64 and gap_ratio > _GAP_RATIO_THRESHOLD and N % 64 == 0:
            block_mask = out_preproc["block_mask_fine"]
            chosen_bm, chosen_bn, use_packed = 64, 64, 1
        elif D == 64 and density > _DENSITY_THRESHOLD_BN128 and N % 128 == 0:
            block_mask = out_preproc["block_mask_bn128"]
            chosen_bm, chosen_bn, use_packed = 128, 128, 1
        else:
            use_packed = 1 if (density > _DENSITY_THRESHOLD_PACKED or gap_ratio > _GAP_RATIO_THRESHOLD or partial_fraction > _PARTIAL_FRACTION_THRESHOLD) else 0
            if not use_packed:
                mask_packed = None
            block_mask = out_preproc["block_mask_coarse"]
            chosen_bm, chosen_bn = 128, 64
        # Compute n_full, n_total only (no col_indices needed)
        n_full, n_total = _build_counts_only(block_mask)
        cache_val = (block_mask, n_full, n_total, mask_packed, chosen_bm, chosen_bn, use_packed)
        _bm_cache_key = key
...
@triton.jit
def _build_counts_only_kernel(BlockMask, NFull, NTotal, stride_bm_row, stride_bm_col, nkb, NKB_POW2: tl.constexpr):
    row = tl.program_id(0)
    offs = tl.arange(0, NKB_POW2)
    valid = offs < nkb
    bm_row = tl.load(BlockMask + row * stride_bm_row + offs * stride_bm_col, mask=valid, other=0)
    is_full = (bm_row == 2) & valid
    is_partial = (bm_row == 1) & valid
    n_full = tl.sum(is_full.to(tl.int32))
    n_total = n_full + tl.sum(is_partial.to(tl.int32))
    tl.store(NFull + row, n_full)
    tl.store(NTotal + row, n_total)


def _build_counts_only(block_mask):
    nqb, nkb = block_mask.shape
    nkb_pow2 = 1
    while nkb_pow2 < nkb:
        nkb_pow2 *= 2
    n_full = torch.empty(nqb, dtype=torch.int32, device=block_mask.device)
    n_total = torch.empty(nqb, dtype=torch.int32, device=block_mask.device)
    _build_counts_only_kernel[(nqb,)](
        block_mask, n_full, n_total,
        block_mask.stride(0), block_mask.stride(1),
        nkb, NKB_POW2=nkb_pow2,
    )
