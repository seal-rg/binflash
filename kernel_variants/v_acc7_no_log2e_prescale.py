...
def _gathered_fwd_inner(
    ...
    for idx in tl.range(0, n_full, num_stages=3):
        ...
        qk -= m_ij[:, None]
        p = tl.math.exp(qk)  # fwd_v_acc7: exp() instead of exp2()
        l_ij = tl.sum(p, 1)
        alpha = tl.math.exp(m_i - m_ij)  # fwd_v_acc7: exp() instead of exp2()
        l_i = l_i * alpha + l_ij
        acc = acc * alpha[:, None]
        p = p.to(v.dtype)
        acc = tl.dot(p, v, acc)
...
def _gathered_fwd(
    ...
    q = tl.load(Q_bp)
    q = (q * sm_scale).to(q.dtype)  # fwd_v_acc7: skip log2e prescale, use exp() not exp2()
