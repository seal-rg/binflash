...
def _bwd_dkdv_inner(
    ...
    for idx in tl.range(0, n_full):
        ...
        dv = tl.dot(tl.trans(p), do.to(tl.float32), dv, input_precision='tf32x3')  # v_acc23: 3xTF32 emulation

        dp = tl.dot(do, tl.trans(v)).to(tl.float32)
        ds = p * (dp - d_i[:, None])
        dk = tl.dot(tl.trans(ds), q.to(tl.float32), dk, input_precision='tf32x3')  # v_acc23: 3xTF32 emulation
    ...
    for idx in tl.range(n_full, n_total):
        ...
        dv = tl.dot(tl.trans(p.to(do.dtype)), do, dv)  # v_acc26: bf16 cast for PARTIAL pass

        dp = tl.dot(do, tl.trans(v)).to(tl.float32)
        ds = p * (dp - d_i[:, None])
        dk = tl.dot(tl.trans(ds.to(q.dtype)), q, dk)  # v_acc26: bf16 cast for PARTIAL pass
