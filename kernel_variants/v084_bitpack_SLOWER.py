"""v084: Bit-packed mask — SLOWER.

Idea: pack bool mask into int64 (64 bits per value). Load 1KB vs 8KB per tile.
Extract bits via int64 shifts: ((packed >> bit_idx) & 1) != 0.

Results at D=64 (where bit-packing is used):
  ALL patterns 0.06-0.59x -- catastrophically slower.
  Cause: NVIDIA GPUs have ~1/64 int64 throughput vs int32/fp32.
  The 8192 int64 shifts per block dominate kernel time.

Analysis: even with int32 packing (full throughput), the net savings
would be ~5ns/block (7.3ns bandwidth saved - 2.2ns ALU cost).
For causal (4096 blocks): 20us out of 2.5ms = 0.8%. Not worth the
complexity of two kernel paths + packing preprocessing.

Also: D=128 OOMs because int64 intermediate (128x64x8 = 64KB) exceeds
register budget alongside Q/K/V/acc tiles.

Conclusion: bit-packing the mask is not viable on current NVIDIA hardware.
The bool mask load (8KB per tile, 33% of per-block data at D=64) is not
the bottleneck -- tensor core matmuls and K/V loads dominate.

This file contains the baseline kernel (same as v084_before_bitpack),
not the experimental bit-packed variant which was transient and reverted.
"""
...
def binflash_attention(
    ...
    _binflash_fwd[grid](
        q, k, v, sm_scale, o, mask, block_mask, lse,
