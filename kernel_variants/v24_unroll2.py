"""v24 unroll2 — v4 + loop_unroll_factor=2 on full-block bwd loops.

Triton's IR-level loop unroller can inline 2 iterations into one body. This
should help the compiler schedule 2 rounds of load/compute as if one iter,
reducing loop overhead and exposing more ILP. Applied only to the full-block
loops (no mask divergence) in both _bwd_dq_inner and _bwd_dkdv_inner. Partial
loops kept as-is to avoid doubling mask-load pressure.

Same v4 features otherwise."""
...
def _bwd_dkdv_inner(
    ...
    for idx in tl.range(0, n_full, num_stages=3, loop_unroll_factor=2):
