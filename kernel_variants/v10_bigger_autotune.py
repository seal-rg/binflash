"""v10 bigger_autotune — v4 with expanded autotune space.

Old space (18 configs): stages ∈ {1,2,3} × warps ∈ {4,8} × maxnreg ∈ {None,168,192}.
New space (75 configs): stages ∈ {1..5} × warps ∈ {4,8,16} × maxnreg ∈ {None,144,168,192,216}.

Cost: one-time autotune compilation penalty per unique kernel key (larger set).
Benefit: Triton may find a faster config that was missing from the old space.
Triton skips OOM configs automatically.
...
# ─── Autotune configs for bwd kernels — v10: expanded space ───

_bwd_autotune_configs = []
for _s in [1, 2, 3, 4, 5]:             # v10: extended to 5 (OOM configs skipped by Triton)
    for _w in [4, 8, 16]:              # v10: added num_warps=16
        for _mnr in [None, 144, 168, 192, 216]:   # v10: more maxnreg granularity
            _kw = {}
            if _mnr is not None:
                _kw["maxnreg"] = _mnr
