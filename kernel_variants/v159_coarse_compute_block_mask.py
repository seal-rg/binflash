"""v159: coarser-grained _compute_block_mask with BN_CHUNKS columns per
program. The current kernel uses a 2D grid of (nqb, nkb) = 32768 tiny
programs at N=16384 BM=128 BN=64, each loading an 8KB tile, doing one
sum-reduction, and storing 1 byte. The per-program work is tiny compared
to the grid scheduling cost.

This version loads (BM, BN_CHUNKS * BN) per program, reshapes to
(BM, BN_CHUNKS, BN), reduces over BM and BN axes, and stores BN_CHUNKS
bm_vals. BN_CHUNKS=4 or 8 cuts the program count 4-8x with the same
total memory bandwidth.

Also keeps v158's Triton _pack_mask and batched sync.
"""
...
def _compute_block_mask(
    ...
    """Grid is (nqb, nkb // BN_CHUNKS). Each program handles BM rows ×
    (BN_CHUNKS * BN) columns and writes BN_CHUNKS bm_val entries.
    """
    row = tl.program_id(0)
    ...
    tile = tl.load(tile_ptrs)  # (BM, BN_CHUNKS * BN) int8

    # Reshape to (BM, BN_CHUNKS, BN) then reduce over axes (0, 2) -> (BN_CHUNKS,)
    tile_3d = tl.reshape(tile, (BLOCK_M, BN_CHUNKS, BLOCK_N))
    tile_i32 = tile_3d.to(tl.int32)
    # Sum over BM axis first (axis=0) -> (BN_CHUNKS, BN)
    chunk_sums_per_col = tl.sum(tile_i32, axis=0)
    # Then over BN axis -> (BN_CHUNKS,)
    chunk_sums = tl.sum(chunk_sums_per_col, axis=1)

    has_any = chunk_sums > 0
    has_all = chunk_sums == (BLOCK_M * BLOCK_N)
    bm_vals = has_any.to(tl.int8) + has_all.to(tl.int8)  # (BN_CHUNKS,)
...
def _fused_preproc(mask, block_m, block_n, want_packed=False):
    ...
    # Pick BN_CHUNKS to divide nkb and keep tile size reasonable.
    # Goal: fewer programs. BN_CHUNKS * BN <= 512 (shmem comfort).
    bn_chunks = 8
    while nkb % bn_chunks != 0 and bn_chunks > 1:
        bn_chunks //= 2
    # Also ensure BN_CHUNKS * BLOCK_N * BLOCK_M fits reasonably (here: 128*64*8=64KB int8 -> OK)
...
# ─── Main attention kernel (UNCHANGED from v149/v158) ───
