"""v179: expand stages range from [1..5] to [1..7] while keeping
v177's maxnreg={None,168,192}.

Dead end #49 (v135) tested stages=[1..7] WITHOUT maxnreg and found
stages=7 was picked in probe but didn't translate to benchmark wins
(the deeper pipelining couldn't fit without register cap). With v177's
maxnreg enabled, deeper pipelining at stages=6 or 7 MIGHT now fit and
give additional latency hiding. This is a genuine retest since dead
end #49 was before the maxnreg discovery.

Autotune space: 7 stages × 2 warps × 3 maxnreg = 42 configs
(vs v177's 5×2×3 = 30). 1.4x first-run autotune time.
Zero risk — autotune picks the winner; 6/7 not being picked just
confirms v177's 1-5 is enough.

Kernel body identical to v177.
"""
import math
import torch
import triton
