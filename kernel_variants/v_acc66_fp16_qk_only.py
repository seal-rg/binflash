...
def _bwd_fused_kernel(
    ...
    for idx in tl.range(0, n_full):
        ...
        qk = tl.dot(q.to(tl.float16), tl.trans(k_scaled).to(tl.float16))  # v_acc66: fp16 QK matmul
...
def _bwd_dq_inner(
    ...
    for idx in tl.range(0, n_full, num_stages=2):
        ...
        qk = tl.dot(q_scaled.to(tl.float16), k.to(tl.float16))  # v_acc66: fp16 QK matmul (dQ side)
        p = tl.math.exp2(qk - lse_i[:, None])
