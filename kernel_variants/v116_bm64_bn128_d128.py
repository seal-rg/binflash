"""Binary Block Masked Flash Attention — v116: BM=64 BN=128 at D=128 for dense.

At D=128, the default BM=128 BN=64 has stages=1 forced and ~28% compute/load
overlap. Dead end #5 says BN=128 doesn't fit at BM=128 D=128 (shared memory).
But at BM=64: Q=16KB, K/V per stage ≈ 64KB combined, acc=32KB register,
qk=32KB register. Total register ~80KB = 160 regs/thread < 255 Ada limit.

Hypothesis: for DENSE patterns at D=128 (where BM=64 doesn't hurt matmul
efficiency as much because the workload is compute-bound anyway), BM=64
BN=128 might enable better pipelining or tensor core utilization.

Dispatch: D=128 AND density > 0.9 → BM=64 BN=128 (new path)
Others: unchanged from v114.
"""
...
def binflash_attention(
    ...
    """Adds D=128 dense path (BM=64 BN=128) to v114's dispatch."""
    ...
    if block_mask is None:
        ...
        if key == _bm_cache_key:
            ...
            elif D == 128 and density > _DENSITY_THRESHOLD and N % 128 == 0 and N % 64 == 0:
                # NEW: D=128 dense → BM=64 BN=128 (exploits lower register pressure)
                block_mask, first_nz, last_nz, _, _ = _fused_preproc(mask, 64, 128)
