...
def _bwd_fused_kernel(
    ...
    for idx in tl.range(0, n_full):
        ...
        # dq via atomic_add
        dq_partial = tl.dot(ds.to(tl.float16), k.to(tl.float16)).to(tl.float32) * sm_scale  # v_acc52: fp16 dQ
...
def _bwd_dq_inner(
    ...
    for idx in tl.range(0, n_full, num_stages=2):
        ...
        dp = tl.dot(do, tl.trans(v)).to(tl.float32)
        ds = p * (dp - d_i[:, None])
        dq = tl.dot(ds.to(tl.float16), tl.trans(k).to(tl.float16), dq)  # v_acc52: fp16 dQ too
