"""Binary Block Masked Flash Attention — v153: single-op mask bias via tl.where.

v149 production uses:
    qk += tl.where(mask, 0.0, -1.0e6)
which compiles to: where(mask, 0.0, -1e6) → temp → add to qk. Two ops.

Alternative:
    qk = tl.where(mask, qk, -1.0e6)
which is one op: select between qk (unmasked) and -1e6 (masked).

Functionally equivalent for softmax — masked positions become exp2(-1e6)
≈ 0 after the softmax normalization, regardless of whether the value is
-1e6 exactly or qk - 1e6. (Both are << any real qk value, so both yield
a vanishing exponential contribution.)

Whether this actually generates fewer instructions depends on Triton's
optimizer — may fuse both forms to identical code, or may not. Measure.
"""
...
def _binflash_fwd_inner(
    ...
    while col_idx <= last_nz_col:
        ...
        if bm_val == 1:
            ...
            qk = tl.dot(q, k)
            # v153: single where instead of where+add
            qk = tl.where(mask, qk, -1.0e6)
            m_ij = tl.maximum(m_i, tl.max(qk, 1))
            qk -= m_ij[:, None]
            p = tl.math.exp2(qk)
...
def binflash_attention(
    ...
    """v149 with single-op mask bias (tl.where replaces where+add)."""
    ...
    if block_mask is None:
        ...
        raise NotImplementedError("External block_mask not supported in v153")
