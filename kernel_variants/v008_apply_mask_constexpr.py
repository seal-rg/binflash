...
1. Index-based dispatch: precompute non-zero block indices, skip empty blocks entirely.
2. Full/partial block separation: skip mask load for blocks where all entries are True.
3. Block pointer loads: use tl.make_block_ptr for hardware-optimized memory access.
"""
...
def _process_block_masked(
    ...
    N_CTX: tl.constexpr, APPLY_MASK: tl.constexpr,
):
    """Process a single KV block. APPLY_MASK=False skips the mask load (full block)."""
...
def _binflash_fwd(
    ...
    # Phase 1: partial blocks (need fine-grained mask)
    for idx in range(num_partial):
        col_idx = tl.load(partial_ptr + idx)
        kv_offset = col_idx * BLOCK_N
        acc, l_i, m_i = _process_block_masked(
            acc, l_i, m_i, q, K_base, V_base, Mask,
            kv_offset, start_m,
            stride_kn, stride_kk, stride_vk, stride_vn,
            stride_mask_m, stride_mask_n,
    ...
    # Phase 2: full blocks (all entries True — skip mask load)
    for idx in range(num_full):
        col_idx = tl.load(full_ptr + idx)
        kv_offset = col_idx * BLOCK_N
        acc, l_i, m_i = _process_block_masked(
            acc, l_i, m_i, q, K_base, V_base, Mask,
            kv_offset, start_m,
...
def _build_dispatch_indices(mask: torch.Tensor, block_mask: torch.Tensor, block_m: int, block_n: int):
    ...
    # Full blocks: every entry in the (block_m x block_n) tile is True
