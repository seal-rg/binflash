"""v1 dq_blockmask_skip — self-contained. dQ kernel now iterates only the
block-mask's non-empty K-blocks using the forward preproc's col_indices.
Pass 1 = full blocks (no mask), Pass 2 = partial blocks (inline bool mask).
dK/dV kernel still iterates all Q-blocks (unchanged from v0)."""
...
def _bwd_dkdv_kernel(
    Q, K, V, sm_scale,
    DO, DK, DV,
    Mask,              # (N, N) bool — inline fine mask check
    LSE, Delta,
    ...
    # Iterate all Q-blocks (no skipping yet — optimization target).
    num_m_blocks = tl.cdiv(N_CTX, BLOCK_M)
    for m_idx in range(0, num_m_blocks):
        m_cur = m_idx * BLOCK_M + offs_m
        ...
        lse_i = tl.load(LSE + off_hz * N_CTX + m_cur)   # already in log2 base from fwd
        d_i = tl.load(Delta + off_hz * N_CTX + m_cur)

        # S = Q K^T * scale  (natural log space? fwd used log2; keep log2 here.)
        qk = tl.dot(q, tl.trans(k))  # (BLOCK_M, BLOCK_N)
        qk_scaled = qk * (sm_scale * 1.44269504)  # log2-scaled scores
        # Apply fine mask: -inf for masked-out positions
        ...
        # dS = P * (dP - D)   (natural-log gradient; we're in log2 so rescale later)
        ds = p * (dp - d_i[:, None])
        # Store dS back with sm_scale (not log2-scaled); convert from log2 grad
        # Since p used log2 basis (exp2), dS_log2 = P * (dP - D), and chain through
        # d/dS_nat = d/dS_log2 * (1/ln2). But here we keep log2 internally and multiply
        # sm_scale at the end.
        # dK += dS^T @ Q * sm_scale
        dk += tl.dot(tl.trans(ds.to(q.dtype)), q) * sm_scale

    # Scale dV down by ln(2) to convert log2-gradient to natural-exp gradient
    # Actually: p = exp2(qk_scaled - lse) where qk_scaled = S * log2(e).
    # softmax_nat(S) = exp(S - lse_nat). lse here = lse_nat * log2(e) so
    # exp2(S*log2(e) - lse_nat*log2(e)) = exp(S - lse_nat) = softmax. Good — P is the real softmax.
    # Therefore dV and dK need no rescaling (the scale was absorbed via fwd/bwd symmetry).
...
def _binflash_bwd(do, q, k, v, o, lse, mask, sm_scale):
    ...
    # v1: build bwd-grain (64, 64) block mask + gathered col_indices for dQ.
