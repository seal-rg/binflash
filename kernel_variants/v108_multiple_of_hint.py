"""Binary Block Masked Flash Attention — v108: tl.multiple_of alignment hint.

v092 (for-loop kernel) had `start_n = tl.multiple_of(start_n, BLOCK_N)` as
an alignment hint. v100 (while-loop) dropped this because start_n is computed
as `col_idx * BLOCK_N`. But the compiler may not deduce the alignment without
the hint — adding it back could unlock better address generation.
"""
...
def _binflash_fwd_inner(
    ...
    while col_idx <= last_nz_col:
        ...
        if bm_val == 2:
            ...
            start_n = tl.multiple_of(start_n, BLOCK_N)  # alignment hint
...
def binflash_attention(
    ...
    """Binary-block-masked flash attention with alignment hints."""
