"""BinFlash attention — production kernel.

Current architecture (v193, promoted 2026-04-17):
  - Gathered block dispatch: each Q-row-block precomputes a sorted list of
    non-empty block column indices (full blocks first, then partial blocks).
    The kernel iterates directly over this list instead of scanning the
    block mask with bm_val checks. Eliminates scan waste entirely and
    removes the hierarchical coarse-mask machinery.
  - Fused Triton preproc: `_build_col_indices_kernel` produces sorted
    indices + per-row counts in one pass. Replaces the prior PyTorch
    argsort (which cost ~0.2ms/call).
  - Dispatch paths (data-driven, not pattern-named):
      Path 1: D=64 gap_ratio>0.3 → BM=64 BN=64 USE_PACKED=1
      Path 2: D=64 density>0.9  → BM=128 BN=128 USE_PACKED=1
      Default: BM=128 BN=64, USE_PACKED varies

Measured vs prior production (v188, scan+hier):
  Kernel-only mean +4.4%, total mean +4.8% across 165 configs (5 workloads
  × 11 patterns × 3 seq_lens). Biggest wins: longformer +11%, cdw_sinks +9%.
"""
