"""Binary Block Masked Flash Attention — v117: BM=64 BN=64 at D=128 for dense.

v116 tried BM=64 BN=128 at D=128 → REGRESSED dense (-19%).
v110/v111 showed BM=64 at D=64 regresses dense patterns too (matmul efficiency).
v114 excludes BM=64 for longformer at D=128 (register pressure with gaps).

This tests: does BM=64 BN=64 at D=128 help DENSE patterns (not gap-heavy)?
At D=128 BM=64:
- Q: 16KB (half of BM=128)
- acc: 32KB (half of BM=128 → frees registers)
- K/V shared per stage: 32KB (unchanged)

Lower register pressure might enable stages=2 at BM=64 D=128 (where stages=1
is forced at BM=128). If the autotuner finds stages=2, we might beat BM=128.

For dense (density > 0.9) at D=128: try BM=64.
"""
...
def binflash_attention(
    ...
    """Adds D=128 dense path (BM=64 BN=64) to v114's dispatch."""
    ...
    if block_mask is None:
        ...
        if key == _bm_cache_key:
            ...
            elif D == 128 and density > _DENSITY_THRESHOLD and N % 64 == 0:
                # NEW: D=128 dense → BM=64 BN=64 (lower register pressure, maybe stages=2)
                block_mask, first_nz, last_nz, _, _ = _fused_preproc(mask, 64, 64)
                chosen_bm, chosen_bn = 64, 64
            else:
