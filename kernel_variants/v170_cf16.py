"""v170: try CF=16 on path 1 for wide-gap patterns (longformer-like).

v168 found CF=8 works better than CF=4 for longformer on path 1.
The formula CF* ≈ sqrt(nkb/nnz) suggests CF=8.5 for longformer
N=32768 path 1. Let me try CF=16 to empirically test whether the
true optimum is higher than CF=8.

Register budget estimate at BM=64 BN=64 D=64 CF=16:
- Static unroll 16 fine bm_val checks → 16 inline copies of the
  check body. Compiler may share K/V registers across copies but
  bm_val registers need to be held briefly.
- Inner unroll depth 16 might exceed what the compiler handles well.
- Likely outcome: either a small win (true optimum at CF=16) or
  regression (CF=16 register pressure).
"""
import math
import torch
import triton
...
_CF_PATH1_WIDE = 16  # v170: was 8 in v168
_CF_PATH1 = 4
_CF_OTHER = 2
...
def binflash_attention(q, k, v, mask, sm_scale=None, block_m=128, block_n=None, block_mask=None):
    ...
    if block_mask is None:
        ...
        raise NotImplementedError("external block_mask path not implemented in v170 test file")
