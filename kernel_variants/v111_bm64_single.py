"""Binary Block Masked Flash Attention — v111: BM=64 single-row.

v110 showed +18-22% win on longformer with BM=64 dual-row, but losses on
other patterns from the dual-row overhead. Hypothesis: the longformer win
comes from FINER block-mask granularity (BM=64 vs BM=128), not K/V sharing.

Test this by using BM=64 with SINGLE-row processing — same as v100 but
with halved block size in the Q dimension. If longformer still wins,
confirms the granularity hypothesis.
"""
...
def binflash_attention(
    ...
    block_m: int = 64,  # v111: force BM=64
    block_n: int | None = None,
    block_mask: torch.Tensor | None = None,
) -> torch.Tensor:
    """Same as v100 but with BLOCK_M=64 to test finer granularity hypothesis."""
