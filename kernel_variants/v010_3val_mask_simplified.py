...
Scan-and-advance kernel with three-valued block mask:
- 0: skip block entirely (all mask entries False)
- 1: partial block — load and apply fine-grained mask
- 2: full block — all mask entries True, skip mask load

Uses tl.make_block_ptr + tl.advance for hardware-optimized block loads.
Static loop bounds (N_CTX / BLOCK_N) for optimal Triton compilation.
"""
...
def _binflash_fwd_inner(
    ...
    for start_n in range(0, N_CTX, BLOCK_N):
        ...
        if bm_val != 0:
            ...
            # Only load mask for partial blocks (bm_val == 1)
            # Full blocks (bm_val == 2) skip mask load entirely
            if bm_val == 1:
...
def _build_3val_block_mask(mask: torch.Tensor, block_mask: torch.Tensor, block_m: int, block_n: int):
    """Build a three-valued block mask: 0=skip, 1=partial, 2=full."""
    ...
    bm_3val = torch.zeros_like(block_mask, dtype=torch.int8)
    bm_3val[block_mask & ~is_full] = 1  # partial
    bm_3val[is_full] = 2  # full
    return bm_3val
...
def binflash_attention(
    ...
    bm_3val = _build_3val_block_mask(mask, block_mask, block_m, block_n)
