"""Binary Block Masked Flash Attention — v121: prefetch next bm_val.

Each iteration of the inner while loop loads bm_val, branches, then does
heavy work (matmul + softmax). The bm_val load has some latency. Prefetching
the NEXT iteration's bm_val in parallel with current iteration's compute
might hide the load latency.

Low risk: just changes load order. Worst case neutral.
"""
...
def _binflash_fwd_inner(
    ...
    while col_idx <= last_nz_col:
        # Prefetch NEXT bm_val early
        next_col = col_idx + 1
        if next_col <= last_nz_col:
            bm_val_next = tl.load(block_mask_ptr + start_m * bm_stride_row + next_col * bm_stride_col)
        else:
            bm_val_next = tl.zeros([], dtype=tl.int8)
...
def binflash_attention(
    ...
    """v114 + prefetch next bm_val in inner loop."""
