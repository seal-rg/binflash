"""V194: V193 + int16 col_indices (half the HBM bandwidth for index loads).

Col indices up to nkb = N/BN. At N=32768 BN=64: max = 512, fits in int16
(range -32768..32767). Halves the per-iteration index load size.
"""
...
def _build_col_indices_kernel(
    ...
    # Scatter column indices to their target ranks (as int16).
    is_nonempty = is_full | is_partial
    ci_ptrs = ColIndices + row * stride_ci_row + target_rank
    tl.store(ci_ptrs, offs.to(tl.int16), mask=is_nonempty)
...
def _build_gathered_indices(block_mask):
    ...
    col_indices = torch.empty(nqb, nkb, dtype=torch.int16, device=block_mask.device)
...
def _gathered_fwd_inner(
    ...
    while idx < n_full:
        col_idx = tl.load(col_indices_ptr + idx).to(tl.int32)
