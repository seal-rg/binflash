"""BinFlash: block-mask flash attention (forward + backward Triton kernels).

This single module hosts the complete kernel stack.

Forward
-------
- ``_compute_and_pack_mask_multi`` / ``_fused_preproc_multigrain``: fused
  Triton reductions that turn the (N, N) bool mask into a coarse block mask
  (any-zero / all-one / mixed), a finer-grained variant, a bn128 variant,
  and a 32-bit-packed mask for the inner kernel to consume.
- ``_build_gathered_indices`` / ``_build_gathered_indices_merged``: sort
  partial/full K-block ids per Q row so the inner kernel iterates over a
  compact list instead of probing every block.
- ``_gathered_fwd_inner`` / ``_gathered_fwd``: the flash-attention inner
  loop that consumes ``(col_indices, n_full, n_total)``, runs full blocks
  without a mask load and partial blocks with the packed mask, and writes
  ``Out`` and ``LSE``.
- ``_dispatch_and_preprocess``: selects between BM=64/128 and BN=64/128
  tiles (with or without the packed-mask path) by computing density and
  gap metrics on the coarse block mask.

Backward
--------
- ``_bwd_preprocess_kernel``: per-row ``delta = sum(dO * O)``. Autotuned
  over ``BLOCK_M``, ``num_warps``, ``num_stages``.
- ``_bwd_dkdv_kernel``: 64x64 tile dKdV pass over Q chunks. Inner loop is
  unrolled — ``tl.range`` cross-iteration pipelining conflicts with the
  autotune-level pipelining and regresses SMEM allocation.
- ``_bwd_dq_kernel``: dQ pass over K chunks. Dispatched to ``BM=64 BN=64``
  or ``BM=128 BN=32`` depending on sequence length and a work-ratio gate
  that catches gap-heavy patterns (longformer, sliding, cdw_sinks) where
  BM=128 would otherwise add K-block visits. Inner loop uses
  ``tl.range(num_stages=2)`` (lower SMEM pressure than dKdV).

Public API
----------
``binflash_attention(q, k, v, mask, sm_scale=None)`` at the bottom of this
module is the autograd-wrapped entry point. Forward runs the
gathered-dispatch kernel and saves LSE; backward reuses LSE plus a
per-mask cached preproc bundle to run the three bwd kernels above.
"""
...
def _gathered_fwd_inner(
    ...
    APPROX_SOFTMAX: tl.constexpr,
):
    # PRECISE=True (v_acc9 path): q has been pre-scaled by sm_scale only (no
    # log2e baked in); we apply log2e post-matmul in fp32 to avoid a bf16 cast
    # bias on the (sm_scale * log2e) constant. P @ V also runs in fp16 (not
    # the caller's bf16) for a more accurate Tensor Core path.
    #
    # APPROX_SOFTMAX=True (BLASST, arXiv:2512.12087): after computing qk for a
    # K-block, if the block's per-row max stays below the running max by more
    # than log2(threshold), the block's softmax contribution rounds to < threshold
    # of the dominant term so we skip the V load and the P@V matmul.
    # log2_threshold = log2(softmax_threshold) is a runtime float; qk is in log2
    # basis here (whether via prescale or PRECISE postscale), so no further
    # conversion is needed.
    LOG2E: tl.constexpr = 1.44269504
    MASK_NEG: tl.constexpr = -1.0e6 * LOG2E if PRECISE else -1.0e6
    ...
    for idx in tl.range(0, n_full, num_stages=3):
        ...
        if PRECISE:
            qk = qk * LOG2E
        m_local = tl.max(qk, 1)
        # BLASST skip: every Q-row in this tile sees its softmax contribution
        # from this K-block bounded by 2^(m_local - m_i) < threshold.
        # When APPROX_SOFTMAX=False the static `False` makes the branch dead-code.
        if APPROX_SOFTMAX and tl.max(m_local - m_i, axis=0) < log2_threshold:
            pass  # skip: contribution rounds to < threshold of running max
        else:
    ...
    for idx in tl.range(n_full, n_total, num_stages=3):
        ...
        if PRECISE:
            qk = qk * LOG2E
        qk += tl.where(mask_, 0.0, MASK_NEG)
        m_local = tl.max(qk, 1)
        if APPROX_SOFTMAX and tl.max(m_local - m_i, axis=0) < log2_threshold:
            pass  # skip: contribution rounds to < threshold of running max
        else:
...
@triton.autotune(
    ...
    key=["N_CTX", "HEAD_DIM", "Z_TIMES_H", "BLOCK_M", "BLOCK_N", "USE_PACKED", "PRECISE", "APPROX_SOFTMAX"],
)
@triton.jit
def _gathered_fwd(
    Q,
    ...
    q = tl.load(Q_bp)
    if PRECISE:
        # v_acc9: scale only by sm_scale; log2e is applied post-matmul in fp32.
...
def _bwd_dkdv_inner(
    ...
    # PRECISE=True (v_acc51 path): cast P/dO and dS/Q to fp16 for the dV and dK
    # matmuls (instead of using the caller's bf16). fp16 has 10 mantissa bits vs
    # bf16's 7 — cheaper than fp32 but tighter than bf16 on the dKdV path.
    #
    # APPROX_SOFTMAX=True (BLASST): skip Q-blocks whose softmax weight against
    # the saved LSE is below threshold for every row in the K-tile. Uses lse
    # (final) as the running max — strictly tighter than the fwd's m_i, so this
    # skips a superset of the fwd's skipped set.
    offs_d = tl.arange(0, HEAD_DIM)
    ...
    for idx in tl.range(0, n_full):
        ...
        if PRECISE:
            qk = qk * 1.44269504  # v_acc9: postmultiply log2e in fp32 to match fwd basis
        # else: qk is already log2-basis because k was pre-scaled by sm_scale * log2e

        # BLASST: skip block if all rows' softmax weights are below threshold.
        if APPROX_SOFTMAX and tl.max(tl.max(qk, axis=1) - lse_i, axis=0) < log2_threshold:
            pass  # skip: p < threshold everywhere → contribution to dv/dk is negligible
        else:
            ...
            if PRECISE:
                dv = tl.dot(tl.trans(p.to(tl.float16)), do.to(tl.float16), dv)
            else:
            ...
            ds = p * (dp - d_i[:, None])
            if PRECISE:
                dk = tl.dot(tl.trans(ds.to(tl.float16)), q.to(tl.float16), dk)
            else:
    ...
    for idx in tl.range(n_full, n_total):
        ...
        qk = tl.dot(q, tl.trans(k_scaled))
        if PRECISE:
            qk = qk * 1.44269504  # v_acc9 postscale; mask sentinel scaled to match
            MASK_NEG = -1.0e6 * 1.44269504
        else:
            MASK_NEG = -1.0e6
        if USE_PACKED:
...
def _bwd_dkdv_kernel(
    ...
    v = tl.load(V_bp)
    if PRECISE:
        # v_acc9 basis: bake only sm_scale into k_scaled; postmultiply qk by
        # log2e inside the inner loop so the qk basis matches the fwd LSE.
        k_scaled = (k * sm_scale).to(k.dtype)
    else:
...
def _bwd_dq_inner(
    ...
    # PRECISE=True (v_acc9): q_scaled has only sm_scale baked in (no log2e).
    # We postmultiply qk by log2e in fp32 so the qk basis matches the fwd LSE.
    # APPROX_SOFTMAX=True (BLASST): skip K-blocks whose softmax weight against
    # the saved LSE is below threshold for every Q-row in the tile.
    LOG2E_DQ: tl.constexpr = 1.44269504
    MASK_NEG_DQ: tl.constexpr = -1.0e6 * LOG2E_DQ if PRECISE else -1.0e6
    offs_d = tl.arange(0, HEAD_DIM)
    offs_bn = tl.arange(0, BLOCK_N)
    offs_bm = start_m * BLOCK_M + tl.arange(0, BLOCK_M)
    BN_PACKED: tl.constexpr = BLOCK_N // 32
    ...
    for idx in tl.range(0, n_full, num_stages=2):
        ...
        if APPROX_SOFTMAX and tl.max(tl.max(qk, axis=1) - lse_i, axis=0) < log2_threshold:
            pass  # skip: block's softmax weight is below threshold for every row
        else:
            v_ptrs = V_base + (start_n + offs_bn)[:, None] * stride_vk + offs_d[None, :] * stride_vn
            v = tl.load(v_ptrs)
            p = tl.math.exp2(qk - lse_i[:, None])
    ...
    for idx in tl.range(n_full, n_total, num_stages=2):
        ...
        if USE_PACKED:
            ...
            mask_ptrs = Mask_base + offs_bm[:, None] * stride_mask_m + (start_n + offs_bn)[None, :] * stride_mask_n
            mask_ = tl.load(mask_ptrs) != 0
        qk += tl.where(mask_, 0.0, MASK_NEG_DQ)
...
def _bwd_dq_kernel(
    ...
    do = tl.load(DO_bp)
    if PRECISE:
        # v_acc9 basis: bake only sm_scale into q_scaled; postmultiply qk by
        # log2e inside the inner loop so the qk basis matches the fwd LSE.
        q_scaled = (q * sm_scale).to(q.dtype)
    else:
...
def _binflash_fwd_with_lse(
    ...
    # BLASST threshold is consumed in log2 basis (qk inside the kernel is in
    # log2 basis after prescale or PRECISE postscale). When the option is off,
    # the kernel branch is static-dead-code and the value is ignored.
    log2_threshold = math.log2(softmax_threshold) if approximate_softmax else 0.0

    o = torch.empty_like(q)
    lse = torch.empty(B, H, N, device=q.device, dtype=torch.float32)
    grid = lambda META: (triton.cdiv(N, chosen_bm), B * H)  # noqa: E731, ARG005
...
class _BinFlashFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, q, k, v, mask, sm_scale, precise, approximate_softmax, softmax_threshold):
        if sm_scale is None:
        ...
        ctx.precise = precise
        ctx.approximate_softmax = approximate_softmax
...
def binflash_attention(
    ...
    Args:
        q, k, v: (B, H, N, D) tensors. D in {16, 32, 64, 128}.
        mask: (N, N) bool tensor — ``True`` means attend, ``False`` means skip.
        sm_scale: softmax scale; defaults to ``1 / sqrt(D)``.
        precise: when True, opt into a higher-precision kernel variant.
            Forward keeps ``log2e`` out of the q prescale (avoids a lossy bf16
            cast of the ``sm_scale * log2e`` constant) and runs P@V in fp16
            (10 mantissa bits vs the caller's bf16 7). The same basis is used
            in dKdV and dQ so the bwd qk stays consistent with the saved LSE,
            and dV+dK matmuls also cast to fp16. Across our 11-pattern test
            grid (D=64 and D=128, N up to 16k) precise=True lowers
            max-error roughly 25% on the forward output and 10-12% on dK/dV
            median; dQ is roughly tied. Costs a few percent in latency.
        approximate_softmax: when True, opt into BLASST-style content-based
            block skipping (arXiv:2512.12087). After computing ``qk`` for a
            K-block, if every row of the tile would receive a softmax
            contribution smaller than ``softmax_threshold`` times the running
            max, the V load and P@V matmul for that block are skipped. The
            backward pass applies the same check against the saved LSE — this
            skips a *superset* of the forward's blocks, so gradients flow
            consistently through what was actually computed. This is an
            *approximate* op: small contributions are dropped on purpose.
        softmax_threshold: float in (0, 1), default 1e-4. Threshold for the
            ``approximate_softmax`` skip. Smaller values skip fewer blocks
            (closer to exact); larger values skip more (faster, less precise).
            Ignored when ``approximate_softmax=False``.

    Returns:
        (B, H, N, D) tensor, same dtype as ``q``.

    Backward populates ``q.grad``, ``k.grad``, ``v.grad`` (no grad through
